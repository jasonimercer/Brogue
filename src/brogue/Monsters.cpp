...
void mutateMonster(Creature* monst, int mutationIndex)
{
...
// prepends it to the list of creatures, and returns a pointer to that creature.
// Note that the creature is not given a map location here!
Creature* generateMonster(int monsterID, bool itemPossible, bool mutationPossible)
{
  int itemChance, mutationChance, i, mutationAttempt;
  Creature* monst;
  ...
  if (mutationPossible && !(monst->info.flags & MONST_NEVER_MUTATED) &&
      !(monst->info.abilityFlags & MA_NEVER_MUTATED) && rogue.depthLevel > 10)
  {
  ...
  monst->creatureState =
      (((monst->info.flags & MONST_NEVER_SLEEPS) || rand_percent(25)) ? MONSTER_TRACKING_SCENT : MONSTER_SLEEPING);
  ...
  if (ITEMS_ENABLED && itemPossible && (rogue.depthLevel <= AMULET_LEVEL) && monsterItemsHopper->nextItem &&
      rand_percent(itemChance))
  {
    ...
    monst->carriedItem->nextItem = nullptr;
...
bool monsterHiddenBySubmersion(const Creature* monst, const Creature* observer)
{
  ...
  {
    if (observer && (terrainFlags(observer->xLoc, observer->yLoc) & T_IS_DEEP_WATER) &&
...
bool canSeeMonster(Creature* monst)
{
  ...
  if (!monsterIsHidden(monst, &player) && (playerCanSee(monst->xLoc, monst->yLoc) || monsterRevealed(monst)))
  {
    return true;
  }
  return false;
...
// This is different from canSeeMonster() in that it counts only physical sight
// -- not clairvoyance or telepathy.
...
bool attackWouldBeFutile(const Creature* attacker, const Creature* defender)
{
  ...
  if ((attacker->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(attacker->status[STATUS_LEVITATING]) &&
  ...
  if (defender->info.flags & MONST_IMMUNE_TO_WEAPONS && !(attacker->info.abilityFlags & MA_POISONS))
  {
    return true;
  }
  return false;
...
bool monsterWillAttackTarget(const Creature* attacker, const Creature* defender)
{
  ...
  if (attacker->status[STATUS_ENTRANCED] && defender->creatureState != MONSTER_ALLY)
  {
    return true;
  }
  if (attacker->creatureState == MONSTER_ALLY && attacker != &player && defender->status[STATUS_ENTRANCED])
  {
    return false;
  }
  ...
  if (attacker->status[STATUS_DISCORDANT] || defender->status[STATUS_DISCORDANT] || attacker->status[STATUS_CONFUSED])
  {
    return true;
  }
  if (monstersAreEnemies(attacker, defender) && !monstersAreTeammates(attacker, defender))
  {
    return true;
  }
  return false;
...
{
  // if one follows the other, or the other follows the one, or they both follow
  // the same
...
bool monstersAreEnemies(const Creature* monst1, const Creature* monst2)
{
  ...
  if (((monst1->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(monst2->info.flags & MONST_IMMUNE_TO_WATER) &&
       !(monst2->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst2->xLoc, monst2->yLoc, T_IS_DEEP_WATER))

      || ((monst2->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(monst1->info.flags & MONST_IMMUNE_TO_WATER) &&
          !(monst1->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst1->xLoc, monst1->yLoc, T_IS_DEEP_WATER)))
  {
    return true;
  }
...
// Returns true if either string has a null terminator before they otherwise
// disagree.
...
{
  int pronounType, gender, i;
  char *insert, *scan;
  bool capitalize;
  // Note: Escape sequences MUST be longer than EACH of the possible
  // replacements. That way, the string only contracts, and we don't need a
  // buffer.
  const char pronouns[4][5][20] = { { "$HESHE", "you", "he", "she", "it" },
  ...
  for (insert = scan = text; *scan;)
  {
    if (scan[0] == '$')
    {
      ...
      if (pronounType == 4)
      {
        // Started with a '$' but didn't match an escape sequence; just copy the
    ...
    {  // Didn't match any of the escape sequences; copy the character
       // instead.
      if (*scan == '.')
      {
        capitalize = true;
                    ...
                    unsigned long requiredFlags)
{
  ...
  {
    if (!(hordeCatalog[i].flags & forbiddenFlags) && !(~(hordeCatalog[i].flags) & requiredFlags) &&
...
Creature* cloneMonster(Creature* monst, bool announce, bool placeClone)
{
  ...
  *newMonst = *monst;  // boink!
  ...
  {
    parentMonst = cloneMonster(monst->carriedMonster, false,
                               false);  // Also clone the carriedMonster
  ...
  newMonst->mapToMe = nullptr;
  newMonst->safetyMap = nullptr;
  newMonst->carriedItem = nullptr;
  ...
  if (placeClone)
  {
    //		getQualifyingLocNear(loc, monst->xLoc, monst->yLoc, true, 0,
    // forbiddenFlagsForMonster(&(monst->info)), (HAS_PLAYER | HAS_MONSTER),
    // false, false); 		newMonst->xLoc = loc[0]; 		newMonst->yLoc =
    // loc[1];
  ...
  if (monst == &player)
  {  // Player managed to clone himself.
  ...
  if (monst->creatureState == MONSTER_ALLY && (monst->info.abilityFlags & MA_CLONE_SELF_ON_DEFEND) &&
      !rogue.featRecord[FEAT_JELLYMANCER])
  {
    jellyCount = 0;
    for (nextMonst = monsters->nextCreature; nextMonst != nullptr; nextMonst = nextMonst->nextCreature)
    {
      if (nextMonst->creatureState == MONSTER_ALLY && (nextMonst->info.abilityFlags & MA_CLONE_SELF_ON_DEFEND))
      {
        jellyCount++;
      }
    }
...
{
  return ((monst->info.flags & MONST_SUBMERGES) && cellHasTMFlag(monst->xLoc, monst->yLoc, TM_ALLOWS_SUBMERGING) &&
          !cellHasTerrainFlag(monst->xLoc, monst->yLoc, T_OBSTRUCTS_PASSABILITY) &&
          !(monst->bookkeepingFlags & (MB_SEIZING | MB_SEIZED | MB_CAPTIVE)) &&
          ((monst->info.flags & (MONST_IMMUNE_TO_FIRE | MONST_INVULNERABLE)) || monst->status[STATUS_IMMUNE_TO_FIRE] ||
           !cellHasTerrainFlag(monst->xLoc, monst->yLoc, T_LAVA_INSTA_DEATH)));
}

// Returns true if at least one minion spawned.
bool spawnMinions(int hordeID, Creature* leader, bool summoned)
{
  int iSpecies, iMember, count;
    ...
    for (iMember = 0; iMember < count; iMember++)
    {
      ...
      do
      {
        getQualifyingPathLocNear(
            &(monst->xLoc), &(monst->yLoc), x, y, summoned, T_DIVIDES_LEVEL & forbiddenTerrainFlags,
            (HAS_PLAYER | HAS_UP_STAIRS | HAS_DOWN_STAIRS), forbiddenTerrainFlags, HAS_MONSTER, false);
      } while (theHorde->spawnsIn && !cellHasTerrainType(monst->xLoc, monst->yLoc, theHorde->spawnsIn) &&
               failsafe++ < 20);
      if (failsafe >= 20)
      {
        // abort
...
bool drawManacle(int x, int y, enum Directions dir)
{
  enum TileType manacles[8] = { MANACLE_T,  MANACLE_B,  MANACLE_L,  MANACLE_R,
                                MANACLE_TL, MANACLE_BL, MANACLE_TR, MANACLE_BR };
  int newX = x + nbDirs[dir][0];
  int newY = y + nbDirs[dir][1];
  if (coordinatesAreInMap(newX, newY) && pmap[newX][newY].layers[DUNGEON] == FLOOR &&
...
void drawManacles(int x, int y)
{
  enum Directions fallback[4][3] = {
    { UPLEFT, UP, LEFT }, { DOWNLEFT, DOWN, LEFT }, { UPRIGHT, UP, RIGHT }, { DOWNRIGHT, DOWN, RIGHT }
  };
  int i, j;
  for (i = 0; i < 4; i++)
  {
...
// If hordeID is 0, it's randomly assigned based on the depth, with a 10% chance
// of an out-of-depth spawn from 1-5 levels deeper. If x is negative, location
// is random. Returns a pointer to the leader.
Creature* spawnHorde(int hordeID, int x, int y, unsigned long forbiddenFlags, unsigned long requiredFlags)
{
  int loc[2];
  int i, failsafe, depth;
  HordeType* theHorde;
  ...
  if (hordeID <= 0)
  {
    ...
    do
    {
      ...
      if (x >= 0 && y >= 0)
      {
        ...
        {
          // don't spawn a horde in special terrain unless it's meant to spawn
          // there
          tryAgain = true;
        }
        if (hordeCatalog[hordeID].spawnsIn && !cellHasTerrainType(x, y, hordeCatalog[hordeID].spawnsIn))
        {
          // don't spawn a horde on normal terrain if it's meant for special
          // terrain
          tryAgain = true;
        }
      }
  ...
  if (x < 0 || y < 0)
  {
    ...
    do
    {
      while (!randomMatchingLocation(&(loc[0]), &(loc[1]), FLOOR, NOTHING,
                                     (hordeCatalog[hordeID].spawnsIn ? hordeCatalog[hordeID].spawnsIn : -1)) ||
      ...
      // This "while" condition should contain IN_FIELD_OF_VIEW, since that is
      // specifically calculated from the entry stairs when the level is
      // generated, and will prevent monsters from spawning within FOV of the
      // entry stairs.
    } while (i < 25 && (pmap[x][y].flags & (ANY_KIND_OF_VISIBLE | IN_FIELD_OF_VIEW)));
  }

  //	if (hordeCatalog[hordeID].spawnsIn == DEEP_WATER &&
  // pmap[x][y].layers[LIQUID] != DEEP_WATER) { 		message("Waterborne
  // monsters spawned on land!", true);
  //	}
  ...
  if (theHorde->machine > 0)
  {
    ...
    buildAMachine(theHorde->machine, x, y, 0, nullptr, nullptr, nullptr);
  }
  ...
  if (preexistingMonst)
  {
    killCreature(preexistingMonst,
                 true);  // If there's already a monster here, quietly bury the body.
  }
...
bool removeMonsterFromChain(Creature* monst, Creature* theChain)
{
  ...
  for (previousMonster = theChain; previousMonster->nextCreature; previousMonster = previousMonster->nextCreature)
  {
...
bool summonMinions(Creature* summoner)
{
  ...
  const int hordeID = pickHordeType(0, summonerType, 0, 0);
  int seenMinionCount = 0, x, y;
  bool atLeastOneMinion = false;
  Creature *monst, *host;
  char buf[DCOLS];
  ...
  {
    // Create a grid where "1" denotes a valid summoning location: within
    // DCOLS/2 pathing distance, not in harmful terrain, and outside of the
    // player's field of view.
    grid = allocGrid();
    fillGrid(grid, 0);
    calculateDistances(grid, summoner->xLoc, summoner->yLoc, (T_PATHING_BLOCKER | T_SACRED), nullptr, true, true);
    findReplaceGrid(grid, 1, DCOLS / 2, 1);
    findReplaceGrid(grid, 2, 30000, 0);
    getTerrainGrid(grid, 0, (T_PATHING_BLOCKER | T_HARMFUL_TERRAIN),
                   (IN_FIELD_OF_VIEW | CLAIRVOYANT_VISIBLE | HAS_PLAYER | HAS_MONSTER));
  }
  else
  {
    grid = nullptr;
  ...
  {
    if (monst != summoner && monstersAreTeammates(monst, summoner) && (monst->bookkeepingFlags & MB_JUST_SUMMONED))
    {
      if (hordeCatalog[hordeID].flags & HORDE_SUMMONED_AT_DISTANCE)
      {
        x = y = -1;
...
void populateMonsters()
{
  ...
  int i, numberOfMonsters = min(20, 6 + 3 * max(0, rogue.depthLevel - AMULET_LEVEL));  // almost always 6.
  ...
  for (i = 0; i < numberOfMonsters; i++)
  {
    spawnHorde(0, -1, -1, (HORDE_IS_SUMMONED | HORDE_MACHINE_ONLY),
               0);  // random horde type, random location
  }
}
...
bool getRandomMonsterSpawnLocation(int* x, int* y)
{
  ...
  grid = allocGrid();
  fillGrid(grid, 0);
  calculateDistances(grid, player.xLoc, player.yLoc, T_DIVIDES_LEVEL, nullptr, true, true);
  getTerrainGrid(grid, 0, (T_PATHING_BLOCKER | T_HARMFUL_TERRAIN),
                 (HAS_PLAYER | HAS_MONSTER | HAS_STAIRS | IN_FIELD_OF_VIEW));
  ...
  if (*x < 0 || *y < 0)
  {
    fillGrid(grid, 1);
    getTerrainGrid(grid, 0, (T_PATHING_BLOCKER | T_HARMFUL_TERRAIN),
                   (HAS_PLAYER | HAS_MONSTER | HAS_STAIRS | IN_FIELD_OF_VIEW | IS_IN_MACHINE));
    randomLocationInGrid(grid, x, y, 1);
  }
  //    DEBUG {
  //        dumpLevelToScreen();
  //        hiliteGrid(grid, &orange, 50);
  //        plotCharWithColor('X', mapToWindowX(x), mapToWindowY(y), &black,
  //        &white); temporaryMessage("Horde spawn location possibilities:",
  //        true);
  //    }
  freeGrid(grid);
  if (*x < 0 || *y < 0)
...
void spawnPeriodicHorde()
{
  ...
  {
    monst = spawnHorde(0, x, y,
                       (HORDE_IS_SUMMONED | HORDE_LEADER_CAPTIVE | HORDE_NO_PERIODIC_SPAWN | HORDE_MACHINE_ONLY), 0);
    if (monst)
    {
      monst->creatureState = MONSTER_WANDERING;
      for (monst2 = monsters->nextCreature; monst2 != nullptr; monst2 = monst2->nextCreature)
      {
...
void teleport(Creature* monst, int x, int y, bool respectTerrainAvoidancePreferences)
{
  ...
  if (!coordinatesAreInMap(x, y))
  {
    ...
    grid = allocGrid();
    fillGrid(grid, 0);
    calculateDistances(grid, monst->xLoc, monst->yLoc, forbiddenFlagsForMonster(&(monst->info)) & T_DIVIDES_LEVEL, nullptr,
                       true, false);
    ...
    if (respectTerrainAvoidancePreferences)
    {
      ...
      getTerrainGrid(grid, 0, avoidedFlagsForMonster(&(monst->info)),
                     (IS_IN_MACHINE | HAS_PLAYER | HAS_MONSTER | HAS_STAIRS));
    }
    else
    {
      getTerrainGrid(grid, 0, forbiddenFlagsForMonster(&(monst->info)),
                     (IS_IN_MACHINE | HAS_PLAYER | HAS_MONSTER | HAS_STAIRS));
    }
    for (i = 0; i < DCOLS; i++)
    {
    ...
    //            dumpLevelToScreen();
    //            hiliteGrid(grid, &orange, 50);
    //            plotCharWithColor('X', mapToWindowX(x), mapToWindowY(y),
    //            &white, &red); temporaryMessage("Teleport candidate
    //            locations:", true);
    //        }
    freeGrid(grid);
    if (x < 0 || y < 0)
...
bool isValidWanderDestination(Creature* monst, int wpIndex)
{
  return (wpIndex >= 0 && wpIndex < rogue.wpCount && !monst->waypointAlreadyVisited[wpIndex] &&
...
int closestWaypointIndex(Creature* monst)
{
  ...
  {
    if (isValidWanderDestination(monst, i) && rogue.wpDistance[i][monst->xLoc][monst->yLoc] < closestDistance)
    {
...
void chooseNewWanderDestination(Creature* monst)
{
  ...
  // Set two checkpoints at random to false (which equilibrates to 50% of
  // checkpoints being active).
...
// Returns the terrain flags of this tile after it's promoted according to the
// event corresponding to subseqDFTypes.
...
bool monsterAvoids(Creature* monst, int x, int y)
{
  ...
  getLocationFlags(x, y, &tFlags, nullptr, &cFlags, monst == &player);

  // everyone but the player avoids the stairs
  if ((x == rogue.downLoc[0] && y == rogue.downLoc[1]) || (x == rogue.upLoc[0] && y == rogue.upLoc[1]))
  {
    return monst != &player;
  }

  // dry land
  if (monst->info.flags & MONST_RESTRICTED_TO_LIQUID && !cellHasTMFlag(x, y, TM_ALLOWS_SUBMERGING))
  {
    return true;
  }
  ...
  {
    if (monst != &player && cellHasTMFlag(x, y, TM_IS_SECRET) &&
        !(discoveredTerrainFlagsAtLoc(x, y) & avoidedFlagsForMonster(&(monst->info))))
    {
      // This is so monsters can use secret doors but won't embed themselves in
      // secret levers.
      return false;
    }
  ...
  {
    defender = monsterAtLoc(x, y);
    if (defender && monsterWillAttackTarget(monst, defender))
    {
  ...
  defender = monsterAtLoc(x, y);
  if (defender && monstersAreEnemies(monst, defender) && attackWouldBeFutile(monst, defender))
  {
    return true;
  }
  ...
  {
    terrainImmunities |=
        (T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION | T_CAUSES_NAUSEA | T_CAUSES_POISON);
  }
  ...
  if (monst == &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && rogue.armor->enchant2 == A_RESPIRATION)
  {
  ...
  // brimstone
  if (!(monst->status[STATUS_IMMUNE_TO_FIRE]) && !(monst->info.flags & MONST_INVULNERABLE) &&
      (tFlags & T_SPONTANEOUSLY_IGNITES) && !(cFlags & (HAS_MONSTER | HAS_PLAYER)) &&
  ...
  if (monst != &player && monst->creatureState == MONSTER_WANDERING && (monst->info.flags & MONST_FIERY) &&
  ...
  if (monst != &player && monst->status[STATUS_BURNING] &&
      (burnedTerrainFlagsAtLoc(x, y) & (T_CAUSES_EXPLOSIVE_DAMAGE | T_CAUSES_DAMAGE | T_AUTO_DESCENT) &
       ~terrainImmunities))
  {
    return true;
  }
  ...
  // fire
  if ((tFlags & T_IS_FIRE & ~terrainImmunities) && !cellHasTerrainFlag(monst->xLoc, monst->yLoc, T_IS_FIRE) &&
  ...
  if ((tFlags & T_IS_DF_TRAP & ~terrainImmunities) && !(cFlags & PRESSURE_PLATE_DEPRESSED) &&
      (monst == &player || monst->creatureState == MONSTER_WANDERING ||
       (monst->creatureState == MONSTER_ALLY && !(cellHasTMFlag(x, y, TM_IS_SECRET)))) &&
      !(monst->status[STATUS_ENTRANCED]) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)))
  {
    return true;
  }
  ...
  if ((tFlags & T_LAVA_INSTA_DEATH & ~terrainImmunities) &&
      (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)) &&
      (monst != &player ||
       rogue.mapToShore[x][y] >= max(player.status[STATUS_IMMUNE_TO_FIRE], player.status[STATUS_LEVITATING])))
  {
    return true;
  }
  ...
  // Smart monsters don't attack in corridors if they belong to a group and they
  // can help it.
  if ((monst->info.abilityFlags & MA_AVOID_CORRIDORS) && monst->creatureState == MONSTER_TRACKING_SCENT &&
      (monst->bookkeepingFlags & (MB_FOLLOWER | MB_LEADER)) && passableArcCount(x, y) >= 2 &&
...
bool moveMonsterPassivelyTowards(Creature* monst, int targetLoc[2], bool willingToAttackPlayer)
{
  ...
  if (monst->creatureState != MONSTER_TRACKING_SCENT && dx && dy)
  {
    ...
    {
      if (!(monsterAvoids(monst, newX, y) || (!willingToAttackPlayer && (pmap[newX][y].flags & HAS_PLAYER)) ||
            !moveMonster(monst, dx, 0)))
      {
        return true;
      }
    }
    else if (abs(targetLoc[0] - x) < abs(targetLoc[1] - y) &&
             rand_range(0, abs(targetLoc[1] - y)) > abs(targetLoc[0] - x))
    {
      if (!(monsterAvoids(monst, x, newY) || (!willingToAttackPlayer && (pmap[x][newY].flags & HAS_PLAYER)) ||
            !moveMonster(monst, 0, dy)))
      {
        return true;
      }
    }
  ...
  if (monsterAvoids(monst, newX, newY) || (!willingToAttackPlayer && (pmap[newX][newY].flags & HAS_PLAYER)) ||
      !moveMonster(monst, dx, dy))
  {
    if (distanceBetween(x, y, targetLoc[0], targetLoc[1]) <= 1 && (dx == 0 || dy == 0))
    {                // cardinally adjacent
    ...
    if ((max(targetLoc[0], x) - min(targetLoc[0], x)) < (max(targetLoc[1], y) - min(targetLoc[1], y)))
    {
          ...
          !moveMonster(monst, 0, dy))
      {
        if (monsterAvoids(monst, newX, y) || (!willingToAttackPlayer && pmap[newX][y].flags & HAS_PLAYER) ||
            !moveMonster(monst, dx, 0))
        {
          if (monsterAvoids(monst, x - 1, newY) || (!willingToAttackPlayer && pmap[x - 1][newY].flags & HAS_PLAYER) ||
              !moveMonster(monst, -1, dy))
          {
            if (monsterAvoids(monst, x + 1, newY) || (!willingToAttackPlayer && pmap[x + 1][newY].flags & HAS_PLAYER) ||
                !moveMonster(monst, 1, dy))
            {
    ...
    else
    {
          ...
          !moveMonster(monst, dx, 0))
      {
        if (monsterAvoids(monst, x, newY) || (!willingToAttackPlayer && pmap[x][newY].flags & HAS_PLAYER) ||
            !moveMonster(monst, 0, dy))
        {
          if (monsterAvoids(monst, newX, y - 1) || (!willingToAttackPlayer && pmap[newX][y - 1].flags & HAS_PLAYER) ||
              !moveMonster(monst, dx, -1))
          {
            if (monsterAvoids(monst, newX, y + 1) || (!willingToAttackPlayer && pmap[newX][y + 1].flags & HAS_PLAYER) ||
                !moveMonster(monst, dx, 1))
            {
...
void wakeUp(Creature* monst)
{
  ...
  for (teammate = monsters->nextCreature; teammate != nullptr; teammate = teammate->nextCreature)
  {
    if (monst != teammate && monstersAreTeammates(monst, teammate) && teammate->creatureMode == MODE_NORMAL)
    {
      if (teammate->creatureState == MONSTER_SLEEPING || teammate->creatureState == MONSTER_WANDERING)
      {
...
int awarenessDistance(Creature* observer, Creature* target)
{
  long perceivedDistance;

  // start with base distance
  if ((observer->status[STATUS_LEVITATING] || (observer->info.flags & MONST_RESTRICTED_TO_LIQUID) ||
       (observer->info.flags & MONST_IMMOBILE) || (observer->bookkeepingFlags & MB_SUBMERGED) ||
  ...
  {
    // if monster flies or is immobile or waterbound or underwater or can cross
    // pits with webs,
  ...
  {
    perceivedDistance = (rogue.scentTurnNumber - scentMap[observer->xLoc][observer->yLoc]);  // this value is double the
                                                                                             // apparent distance
  }
...
{
  int perceivedDistance = awarenessDistance(observer, target);
  int awareness = rogue.aggroRange * 2;
  bool retval;
  ...
  else if (observer->info.flags & MONST_IMMOBILE)
  {
    ...
    // The only exception is mirror totems; they're always ready to shoot
    // because they have "always hunting" set.
  ...
  else if (target == &player && !(pmap[observer->xLoc][observer->yLoc].flags & IN_FIELD_OF_VIEW))
  {
...
int closestWaypointIndexTo(const int x, const int y)
{
  int i, closestDistance, closestIndex;
...
void wanderToward(Creature* monst, const int x, const int y)
{
  if (coordinatesAreInMap(x, y))
  {
    const int theWaypointIndex = closestWaypointIndexTo(x, y);
...
void updateMonsterState(Creature* monst)
{
  ...
  if ((monst->info.flags & MONST_ALWAYS_HUNTING) && monst->creatureState != MONSTER_ALLY)
  {
  ...
  if ((monst->info.flags & MONST_IMMOBILE) && monst->creatureState != MONSTER_ALLY)
  {
    if (awareOfPlayer)
    {
      monst->creatureState = MONSTER_TRACKING_SCENT;
  ...
  {
    if (monsterFleesFrom(monst, monst2) && distanceBetween(x, y, monst2->xLoc, monst2->yLoc) < closestFearedEnemy &&
        traversiblePathBetween(monst2, x, y) && openPathBetween(x, y, monst2->xLoc, monst2->yLoc))
    {
  ...
  {
    // if tracking scent, but the scent is weaker than the scent detection
    // threshold, begin wandering.
    monst->creatureState = MONSTER_WANDERING;
    wanderToward(monst, monst->lastSeenPlayerAt[0], monst->lastSeenPlayerAt[1]);
  }
  else if (monst->creatureState == MONSTER_TRACKING_SCENT && closestFearedEnemy < 3)
  {
    monst->creatureState = MONSTER_FLEEING;
  }
  else if (monst->creatureState != MONSTER_ALLY && (monst->info.flags & MONST_FLEES_NEAR_DEATH) &&
           monst->currentHP <= 3 * monst->info.maxHP / 4)
  {
    if (monst->creatureState == MONSTER_FLEEING || monst->currentHP <= monst->info.maxHP / 4)
    {
      monst->creatureState = MONSTER_FLEEING;
    }
  }
  else if (monst->creatureMode == MODE_NORMAL && monst->creatureState == MONSTER_FLEEING &&
           !(monst->status[STATUS_MAGICAL_FEAR]) && closestFearedEnemy >= 3)
  {
    monst->creatureState = MONSTER_TRACKING_SCENT;
  }
  else if (monst->creatureMode == MODE_PERM_FLEEING && monst->creatureState == MONSTER_FLEEING &&
           (monst->info.abilityFlags & MA_HIT_STEAL_FLEE) && !(monst->status[STATUS_MAGICAL_FEAR]) &&
           !(monst->carriedItem))
  {
    monst->creatureMode = MODE_NORMAL;
    alertMonster(monst);
           ...
           (monst->info.flags & MONST_FLEES_NEAR_DEATH) && !(monst->status[STATUS_MAGICAL_FEAR]) &&
  ...
  if (awareOfPlayer)
  {
    if (monst->creatureState == MONSTER_FLEEING || monst->creatureState == MONSTER_TRACKING_SCENT)
    {
      monst->lastSeenPlayerAt[0] = player.xLoc;
      monst->lastSeenPlayerAt[1] = player.yLoc;
    }
...
void decrementMonsterStatus(Creature* monst)
{
    ...
    switch (i)
    {
      ...
      case STATUS_BURNING:
        if (monst->status[i])
        {
          ...
          if (!(monst->status[STATUS_IMMUNE_TO_FIRE]) && !(monst->info.flags & MONST_INVULNERABLE) &&
              inflictDamage(nullptr, monst, damage, &orange, true))
          {
            if (canSeeMonster(monst))
            {
      ...
      case STATUS_POISONED:
        ...
        {
          monst->status[i]--;
          if (inflictDamage(nullptr, monst, monst->poisonAmount, &green, true))
          {
            if (canSeeMonster(monst))
            {
              monsterName(buf, monst, true);
      ...
      case STATUS_DISCORDANT:
        if (monst->status[i] && !--monst->status[i])
        {
          if (monst->creatureState == MONSTER_FLEEING && !monst->status[STATUS_MAGICAL_FEAR] &&
              monst->leader == &player)
          {
            monst->creatureState = MONSTER_ALLY;
            if (monst->carriedItem)
      ...
      case STATUS_INVISIBLE:
        if (monst->status[i] && !(monst->info.flags & MONST_INVISIBLE) && !--monst->status[i] &&
  ...
  if (monsterCanSubmergeNow(monst) && !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
    ...
    else if (monst->info.flags & (MONST_RESTRICTED_TO_LIQUID) && monst->creatureState != MONSTER_ALLY)
    {
      monst->creatureState = MONSTER_FLEEING;
    }
  }
...
{
  int coords[DCOLS][2], i, x, y, n;
  int originLoc[2] = { monst->xLoc, monst->yLoc };
  int targetLoc[2] = { x2, y2 };
...
bool specifiedPathBetween(int x1, int y1, int x2, int y2, unsigned long blockingTerrain,
                             unsigned long blockingFlags)
{
  int coords[DCOLS][2], i, x, y, n;
  int originLoc[2] = { x1, y1 };
  int targetLoc[2] = { x2, y2 };
  n = getLineCoordinates(coords, originLoc, targetLoc);
...
{
  int returnLoc[2], startLoc[2] = { x1, y1 }, targetLoc[2] = { x2, y2 };
...
Creature* monsterAtLoc(int x, int y)
{
  ...
  for (monst = monsters->nextCreature; monst != nullptr && (monst->xLoc != x || monst->yLoc != y);
...
Creature* dormantMonsterAtLoc(int x, int y)
{
  ...
  for (monst = dormantMonsters->nextCreature; monst != nullptr && (monst->xLoc != x || monst->yLoc != y);
       monst = monst->nextCreature)
    ;
  return monst;
}
...
void pathTowardCreature(Creature* monst, Creature* target)
{
  ...
  // blink to the target?
  if (distanceBetween(monst->xLoc, monst->yLoc, target->xLoc, target->yLoc) > 10 || monstersAreEnemies(monst, target))
  {
    if (monsterBlinkToPreferenceMap(monst, target->mapToMe, false))
    {  // if it blinked
...
bool creatureEligibleForSwarming(Creature* monst)
{
  if ((monst->info.flags & (MONST_IMMOBILE | MONST_GETS_TURN_ON_ACTIVATION | MONST_MAINTAINS_DISTANCE)) ||
      monst->status[STATUS_ENTRANCED] || monst->status[STATUS_CONFUSED] || monst->status[STATUS_STUCK] ||
      monst->status[STATUS_PARALYZED] || monst->status[STATUS_MAGICAL_FEAR] ||
      monst->status[STATUS_LIFESPAN_REMAINING] == 1 || (monst->bookkeepingFlags & (MB_SEIZED | MB_SEIZING)))
  {
    return false;
  }
  if (monst != &player && monst->creatureState != MONSTER_ALLY && monst->creatureState != MONSTER_TRACKING_SCENT)
  {
    return false;
  }
  return true;
...
// Swarming behavior.
// If you’re adjacent to an enemy and about to strike it, and you’re adjacent to
// a hunting-mode tribemate who is not adjacent to another enemy, and there is
// no empty space adjacent to the tribemate AND the enemy, and there is an empty
// space adjacent to you AND the enemy, then move into that last space. (In each
// case, "adjacent" excludes diagonal tiles obstructed by corner walls.)
enum Directions monsterSwarmDirection(Creature* monst, Creature* enemy)
{
  int newX, newY, i;
  enum Directions dir, targetDir;
  int dirList[8] = { 0, 1, 2, 3, 4, 5, 6, 7 };
  ...
  if (distanceBetween(monst->xLoc, monst->yLoc, enemy->xLoc, enemy->yLoc) != 1 ||
      (diagonalBlocked(monst->xLoc, monst->yLoc, enemy->xLoc, enemy->yLoc, false) ||
       (enemy->info.flags & MONST_ATTACKABLE_THRU_WALLS)) ||
      !monstersAreEnemies(monst, enemy))
  {
    return NO_DIRECTION;  // Too far from the enemy, diagonally blocked, or not
                          // enemies with it.
  }
  ...
  for (i = 0; i < 8 && targetDir == NO_DIRECTION; i++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && distanceBetween(enemy->xLoc, enemy->yLoc, newX, newY) == 1 &&
  ...
  // OK, now we have a place to move toward. Let's analyze the teammates around
  // us to make sure that one of them could take advantage of the space we open.
  CYCLE_MONSTERS_AND_PLAYERS(ally)
  {
    if (ally != monst && ally != enemy && monstersAreTeammates(monst, ally) && monstersAreEnemies(ally, enemy) &&
        creatureEligibleForSwarming(ally) && distanceBetween(monst->xLoc, monst->yLoc, ally->xLoc, ally->yLoc) == 1 &&
        !diagonalBlocked(monst->xLoc, monst->yLoc, ally->xLoc, ally->yLoc, false) &&
        !monsterAvoids(ally, monst->xLoc, monst->yLoc) &&
        (distanceBetween(enemy->xLoc, enemy->yLoc, ally->xLoc, ally->yLoc) > 1 ||
         diagonalBlocked(enemy->xLoc, enemy->yLoc, ally->xLoc, ally->yLoc, false)))
    {
      // Found a prospective ally.
      // Check that there isn't already an open space from which to attack the
      // enemy that is accessible to the ally.
      ...
      for (dir = 0; dir < DIRECTION_COUNT && !alternateDirectionExists; dir++)
      {
        ...
        if (coordinatesAreInMap(newX, newY) && !(pmap[newX][newY].flags & (HAS_PLAYER | HAS_MONSTER)) &&
            distanceBetween(enemy->xLoc, enemy->yLoc, newX, newY) == 1 &&
            !diagonalBlocked(enemy->xLoc, enemy->yLoc, newX, newY, false) &&
            !diagonalBlocked(ally->xLoc, ally->yLoc, newX, newY, false) && !monsterAvoids(ally, newX, newY))
        {
      ...
      if (!alternateDirectionExists)
      {
        ...
        {
          if (ally != otherEnemy && monst != otherEnemy && enemy != otherEnemy &&
              monstersAreEnemies(ally, otherEnemy) &&
              distanceBetween(ally->xLoc, ally->yLoc, otherEnemy->xLoc, otherEnemy->yLoc) == 1 &&
              (!diagonalBlocked(ally->xLoc, ally->yLoc, otherEnemy->xLoc, otherEnemy->yLoc, false) ||
               (otherEnemy->info.flags & MONST_ATTACKABLE_THRU_WALLS)))
          {
...
// Isomorphs a number in [0, 39] to coordinates along the square of radius 5
// surrounding (0,0). This is used as the sample space for bolt target
// coordinates, e.g. when reflecting or when monsters are deciding where to
// blink.
...
// Tries to make the monster blink to the most desirable square it can aim at,
// according to the preferenceMap argument. "blinkUphill" determines whether
// it's aiming for higher or lower numbers on the preference map -- true means
// higher. Returns true if the monster blinked; false if it didn't.
bool monsterBlinkToPreferenceMap(Creature* monst, int** preferenceMap, bool blinkUphill)
{
  int i, bestTarget[2], bestPreference, nowPreference, maxDistance, target[2], impact[2], origin[2];
  bool gotOne;
  char monstName[DCOLS];
  char buf[DCOLS];
...
bool monsterSummons(Creature* monst, bool alwaysUse)
{
  ...
  if (monst->info.abilityFlags & (MA_CAST_SUMMON))
  {
    ...
    for (target = monsters->nextCreature; target != nullptr; target = target->nextCreature)
    {
      if (monst->creatureState == MONSTER_ALLY)
      {
      ...
      {
        minionCount++;  // Enemy summoners count only direct followers, not
                        // teammates.
      }
    }
    if (monst->creatureState == MONSTER_ALLY)
    {  // Allied summoners also count monsters on the previous
       // and next depths.
      if (rogue.depthLevel > 1)
      {
        for (target = levels[rogue.depthLevel - 2].monsters; target != nullptr; target = target->nextCreature)
        {
      ...
      {
        for (target = levels[rogue.depthLevel].monsters; target != nullptr; target = target->nextCreature)
        {
          if (target->creatureState == MONSTER_ALLY && !(target->info.flags & MONST_WILL_NOT_USE_STAIRS))
          {
            minionCount++;
...
// Some monsters never make good targets irrespective of what bolt we're
// contemplating. Return false for those. Otherwise, return true.
bool generallyValidBoltTarget(Creature* caster, Creature* target)
{
  ...
  if (monsterIsHidden(target, caster) || (target->bookkeepingFlags & MB_SUBMERGED))
  {
    // No bolt will affect a submerged creature. Can't shoot at invisible
    // creatures unless it's in gas.
    return false;
  }
...
bool targetEligibleForCombatBuff(Creature* caster, Creature* target)
{
  ...
  if (caster->creatureState == MONSTER_ALLY)
  {
    if (canDirectlySeeMonster(caster))
    {
      ...
      {
        if (monstersAreEnemies(&player, enemy) && canSeeMonster(enemy) &&
...
// Make a decision as to whether the given caster should fire the given bolt at
// the given target. Assumes that the conditions in generallyValidBoltTarget
...
bool specificallyValidBoltTarget(Creature* caster, Creature* target, enum BoltType theBoltType)
{
  ...
  if ((boltCatalog[theBoltType].flags & BF_TARGET_ENEMIES) && (!monstersAreEnemies(caster, target)))
  {
    return false;
  }
  if ((boltCatalog[theBoltType].flags & BF_TARGET_ENEMIES) && (target->info.flags & MONST_INVULNERABLE))
  {
    return false;
  }
  if ((target->info.flags & MONST_REFLECT_4) && target->creatureState != MONSTER_ALLY &&
      !(boltCatalog[theBoltType].flags & (BF_NEVER_REFLECTS | BF_HALTS_BEFORE_OBSTRUCTION)))
  {
    // Don't fire a reflectable bolt at a reflective target unless it's your
    // ally.
    return false;
  }
  ...
  if ((boltCatalog[theBoltType].flags & BF_FIERY) && target->status[STATUS_IMMUNE_TO_FIRE])
  {
  ...
  {
    // Don't shoot fireballs if you're standing on a tile that could combust
    // into something that harms you.
    return false;
  }
  ...
  switch (boltCatalog[theBoltType].boltEffect)
  {
    ...
    case BE_NONE:
      ...
      // so our logic has to follow from the terrain parameters of the bolt's
      // target DF.
      if (boltCatalog[theBoltType].targetDF)
      {
        const unsigned long terrainFlags =
            tileCatalog[dungeonFeatureCatalog[boltCatalog[theBoltType].targetDF].tile].flags;
    ...
    case BE_DISCORD:
      ...
      {
        // Don't cast discord if the target is already discordant, or if it is
        // the player. (Players should never be intentionally targeted by
        // discord. It's just a fact of monster psychology.)
        return false;
      }
    case BE_NEGATION:
      if (monstersAreEnemies(caster, target))
      {
        ...
        {
          // Dispel magic creatures; strip weapon invulnerability from
          // revenants.
          return true;
        }
        ...
        if (monstersAreTeammates(caster, target) && target->status[STATUS_DISCORDANT] &&
      ...
      {
        if (target == &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) &&
            (rogue.armor->flags & ITEM_RUNIC_IDENTIFIED) && rogue.armor->enchant2 == A_REFLECTION &&
            netEnchant(rogue.armor) > 0)
        {
          // Allies shouldn't cast negation on the player if she's knowingly
          // wearing armor of reflection. Too much risk of negating themselves
          // in the process.
          return false;
        }
  ...
  CYCLE_MONSTERS_AND_PLAYERS(target)
  {
    if (generallyValidBoltTarget(monst, target))
    {
      for (i = 0; monst->info.bolts[i]; i++)
      {
        ...
        {
          if ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(30))
          {
...
bool isLocalScentMaximum(int x, int y)
{
  enum Directions dir;
  int newX, newY;

  const int baselineScent = scentMap[x][y];

  for (dir = 0; dir < DIRECTION_COUNT; dir++)
  {
    newX = x + nbDirs[dir][0];
    newY = y + nbDirs[dir][1];
    if (coordinatesAreInMap(newX, newY) && (scentMap[newX][newY] > baselineScent) &&
        !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) && !diagonalBlocked(x, y, newX, newY, false))
    {
      return false;
    }
  }
...
// Returns the direction the player's scent points to from a given cell. Returns
// -1 if the nose comes up blank.
enum Directions scentDirection(Creature* monst)
{
  int newX, newY, x, y, newestX, newestY;
  enum Directions bestDirection = NO_DIRECTION, dir, dir2;
  unsigned int bestNearbyScent = 0;
  ...
  for (;;)
  {
    for (dir = 0; dir < DIRECTION_COUNT; dir++)
    {
      ...
      if (coordinatesAreInMap(newX, newY) && (scentMap[newX][newY] > bestNearbyScent) &&
    ...
    if (canTryAgain)
    {
      ...
      // If so, we can diffuse the scent into the offending kink and solve the
      // problem. There's a possibility he's stuck for some other reason,
      // though, so we'll only try once per his move -- hence the failsafe.
      canTryAgain = false;
      for (dir = 0; dir < 4; dir++)
      {
        newX = x + nbDirs[dir][0];
...
bool resurrectAlly(const int x, const int y)
{
  ...
  if (monst)
  {
    ...
    monst->nextCreature = monsters->nextCreature;
    monsters->nextCreature = monst;
    getQualifyingPathLocNear(&monst->xLoc, &monst->yLoc, x, y, true, (T_PATHING_BLOCKER | T_HARMFUL_TERRAIN), 0, 0,
...
{
  const int x = monst->xLoc;
  const int y = monst->yLoc;
      ...
      !(defender->info.flags & MONST_IMMOBILE))
  {
    ...
    // i.e., keep distance from revenants and stone guardians but not mirror
    // totems.
    return true;
  }

  if ((monst->info.flags & MONST_MAINTAINS_DISTANCE) || (defender->info.abilityFlags & MA_KAMIKAZE))
  {
...
{
  const int x = ally->xLoc;
  const int y = ally->yLoc;
  ...
  if (ally->info.maxHP <= 1 || (ally->status[STATUS_LIFESPAN_REMAINING]) > 0)
  {  // Spectral blades and timed allies should never flee.
    return false;
  }

  if (distanceBetween(x, y, closestEnemy->xLoc, closestEnemy->yLoc) < 10 &&
      (100 * ally->currentHP / ally->info.maxHP <= 33) && ally->info.turnsBetweenRegen > 0 && !ally->carriedMonster &&
      ((ally->info.flags & MONST_FLEES_NEAR_DEATH) ||
       (100 * ally->currentHP / ally->info.maxHP * 2 < 100 * player.currentHP / player.info.maxHP)))
  {
    // Flee if you're within 10 spaces, your HP is under 1/3, you're not a
    // phoenix or lich or vampire in bat form, and you either flee near death or
    // your health fraction is less than half of the player's.
    return true;
  }

  // so do allies that keep their distance or while in the presence of
  // damage-immune or kamikaze enemies
  if (monsterFleesFrom(ally, closestEnemy))
  {
    // Flee if you're within 3 spaces and you either flee near death or the
    // closest enemy is a bloat, revenant or guardian.
    return true;
  }
...
void monsterMillAbout(Creature* monst, int movementChance)
{
  enum Directions dir;
  int targetLoc[2];
...
{
  Creature *target, *closestMonster = nullptr;
  int i, j, x, y, dir, shortestDistance, targetLoc[2], leashLength;
  int **enemyMap, **costMap;
  char buf[DCOLS], monstName[DCOLS];
  ...
  // If we're standing in harmful terrain and there is a way to escape it, spend
  // this turn escaping it.
  if (cellHasTerrainFlag(
          x, y, (T_HARMFUL_TERRAIN & ~(T_IS_FIRE | T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION))) ||
      (cellHasTerrainFlag(x, y, T_IS_FIRE) && !monst->status[STATUS_IMMUNE_TO_FIRE]) ||
      (cellHasTerrainFlag(x, y, T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION) &&
  ...
  // Look around for enemies; shortestDistance will be the distance to the
  // nearest.
  shortestDistance = max(DROWS, DCOLS);
  for (target = monsters->nextCreature; target != nullptr; target = target->nextCreature)
  {
    if (target != monst && (!(target->bookkeepingFlags & MB_SUBMERGED) || (monst->bookkeepingFlags & MB_SUBMERGED)) &&
        ...
        (!cellHasTerrainFlag(target->xLoc, target->yLoc, T_OBSTRUCTS_PASSABILITY) ||
         (target->info.flags & MONST_ATTACKABLE_THRU_WALLS)) &&
  ...
  if (allyFlees(monst, closestMonster))
  {
    if (monsterHasBoltEffect(monst, BE_BLINKING) &&
        ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(30)) && monsterBlinkToSafety(monst))
    {
      return;
    }
    if (monsterSummons(monst, (monst->info.flags & MONST_ALWAYS_USE_ABILITY)))
    ...
    if (dir == -1 || (allySafetyMap[targetLoc[0]][targetLoc[1]] >= allySafetyMap[x][y]) ||
  ...
  {
    leashLength = 11;  // If the ally is adjacent to a monster at the end of its
                       // leash, it shouldn't be prevented from attacking.
  }
  else
  {
    leashLength = 10;
  ...
  if (closestMonster &&
      (distanceBetween(x, y, player.xLoc, player.yLoc) < leashLength ||
  ...
  else if (monst->targetCorpseLoc[0] && !monst->status[STATUS_POISONED] &&
           (!monst->status[STATUS_BURNING] || monst->status[STATUS_IMMUNE_TO_FIRE]))
  {  // Going to start eating
     // a corpse.
    moveMonsterPassivelyTowards(monst, monst->targetCorpseLoc, false);
    if (monst->xLoc == monst->targetCorpseLoc[0] && monst->yLoc == monst->targetCorpseLoc[1] &&
        !(monst->bookkeepingFlags & MB_ABSORBING))
    {
      if (canSeeMonster(monst))
      {
        monsterName(monstName, monst, true);
        sprintf(buf, "%s begins %s the fallen %s.", monstName, monsterText[monst->info.monsterID].absorbing,
                monst->targetCorpseName);
  ...
  {
    if (!(monst->bookkeepingFlags & MB_GIVEN_UP_ON_SCENT) && distanceBetween(x, y, player.xLoc, player.yLoc) > 10 &&
      ...
      (monst->bookkeepingFlags & MB_ABSORBING))
  {
    if (!--monst->corpseAbsorptionCounter)
    {
      ...
      if (canSeeMonster(monst))
      {
        ...
        sprintf(buf, "%s finished %s the %s.", buf2, monsterText[monst->info.monsterID].absorbing,
                monst->targetCorpseName);
        messageWithColor(buf, &goodMessageColor, false);
        if (monst->absorptionBolt != BOLT_NONE)
        {
...
{
  int x, y, playerLoc[2], targetLoc[2], dir, shortestDistance;
  ...
  if (monst->info.DFChance && (monst->info.flags & MONST_GETS_TURN_ON_ACTIVATION) && rand_percent(monst->info.DFChance))
  {
    spawnDungeonFeature(monst->xLoc, monst->yLoc, &dungeonFeatureCatalog[monst->info.DFType], true, false);
  }

  applyInstantTileEffectsToCreature(monst);  // Paralysis, confusion etc. take effect before the monster can
                                             // move.

  // if the monster is paralyzed, entranced or chained, this is where its turn
  // ends.
  ...
  if ((monst->creatureState == MONSTER_TRACKING_SCENT ||
      ...
      && (!(monst->info.flags & MONST_RESTRICTED_TO_LIQUID) ||
          cellHasTMFlag(player.xLoc, player.yLoc, TM_ALLOWS_SUBMERGING)))
  {
    // magic users sometimes cast spells
    if (monstUseMagic(monst) || (monsterHasBoltEffect(monst, BE_BLINKING) &&
                                 ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(30)) &&
                                 monsterBlinkToPreferenceMap(monst, scentMap, true)))
    {  // if he actually cast a spell
    ...
    // if the monster is adjacent to an ally and not adjacent to the player,
    // attack the ally
    if (distanceBetween(x, y, player.xLoc, player.yLoc) > 1 || diagonalBlocked(x, y, player.xLoc, player.yLoc, false))
    {
      for (ally = monsters->nextCreature; ally != nullptr; ally = ally->nextCreature)
      {
        if (monsterWillAttackTarget(monst, ally) && distanceBetween(x, y, ally->xLoc, ally->yLoc) == 1 &&
    ...
    if ((monst->status[STATUS_LEVITATING] || (monst->info.flags & MONST_RESTRICTED_TO_LIQUID) ||
         (monst->bookkeepingFlags & MB_SUBMERGED) ||
    ...
    if ((monst->info.flags & MONST_ALWAYS_HUNTING) && (monst->bookkeepingFlags & MB_GIVEN_UP_ON_SCENT))
    {
  ...
  else if (monst->creatureState == MONSTER_FLEEING)
  {
    ...
    if (fleeingMonsterAwareOfPlayer(monst))
    {
      ...
      dir = nextStep(safetyMap, monst->xLoc, monst->yLoc, nullptr, true);
    }
    else
    {
      if (!monst->safetyMap)
      ...
      dir = nextStep(monst->safetyMap, monst->xLoc, monst->yLoc, nullptr, true);
    }
    if (dir != -1)
    {
      targetLoc[0] = x + nbDirs[dir][0];
        ...
        (!moveMonster(monst, nbDirs[dir][0], nbDirs[dir][1]) && !moveMonsterPassivelyTowards(monst, targetLoc, true)))
    {
      CYCLE_MONSTERS_AND_PLAYERS(ally)
      {
        if (!monst->status[STATUS_MAGICAL_FEAR]  // Fearful monsters will never
                                                 // attack.
            && monsterWillAttackTarget(monst, ally) && distanceBetween(x, y, ally->xLoc, ally->yLoc) <= 1)
        {
          moveMonster(monst, ally->xLoc - x,
                      ally->yLoc - y);  // attack the player if cornered
          return;
        }
      }
    }
    return;
           ...
           || ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) &&
               !cellHasTMFlag(player.xLoc, player.yLoc, TM_ALLOWS_SUBMERGING)))
  {
    // if we're standing in harmful terrain and there is a way to escape it,
    // spend this turn escaping it.
    ...
    // if a captive leader is captive and healthy enough to withstand an attack,
    // then approach or attack him.
    if ((monst->bookkeepingFlags & MB_FOLLOWER) && (monst->leader->bookkeepingFlags & MB_CAPTIVE) &&
    ...
    if (monst->bookkeepingFlags & MB_FOLLOWER)
    {
      ...
      {
        monsterMillAbout(monst,
                         30);  // Other followers mill about like your allies do.
      }
    }
    else
    {
      ...
      // If there's no path forward, call that waypoint finished and pick a new
      // one.
      if (!isValidWanderDestination(monst, monst->targetWaypointIndex) || dir == NO_DIRECTION)
      {
        chooseNewWanderDestination(monst);
        if (isValidWanderDestination(monst, monst->targetWaypointIndex))
        {
...
bool canPass(Creature* mover, Creature* blocker)
{
  ...
  if (blocker->status[STATUS_CONFUSED] || blocker->status[STATUS_STUCK] || blocker->status[STATUS_PARALYZED] ||
      blocker->status[STATUS_ENTRANCED] || mover->status[STATUS_ENTRANCED])
  {
    return false;
  }

  if ((blocker->bookkeepingFlags & (MB_CAPTIVE | MB_ABSORBING)) || (blocker->info.flags & MONST_IMMOBILE))
  {
    return false;
  }
...
void executeMonsterMovement(Creature* monst, int newX, int newY)
{
  ...
  if (playerCanSee(newX, newY) && cellHasTMFlag(newX, newY, TM_IS_SECRET) &&
      cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY))
  {
    discover(newX,
             newY);  // if you see a monster use a secret door, you discover it
  }
...
// Tries to move the given monster in the given vector; returns true if the move
// was legal (including attacking player, vomiting or struggling in vain) Be
// sure that dx, dy are both in the range [-1, 1] or the move will sometimes
// fail due to the diagonal check.
bool moveMonster(Creature* monst, int dx, int dy)
{
  int x = monst->xLoc, y = monst->yLoc;
  int newX, newY;
  int i;
  int confusedDirection, swarmDirection;
  Creature* defender = nullptr;
  Creature* hitList[16] = { nullptr };
  enum Directions dir;
  ...
  if (!coordinatesAreInMap(newX, newY))
  {
    // DEBUG printf("\nProblem! Monster trying to move more than one space at a
    // time.");
    return false;
  }
  ...
  {
    if (!(monst->info.flags & MONST_INVULNERABLE) && --monst->status[STATUS_STUCK])
    {
      monst->ticksUntilTurn = monst->movementSpeed;
      return true;
    }
  ...
  else
  {
    ...
    {
      for (defender = monsters->nextCreature; defender != nullptr; defender = defender->nextCreature)
      {
        if ((defender->bookkeepingFlags & MB_SEIZING) && monstersAreEnemies(monst, defender) &&
  ...
  if (((defender && (defender->info.flags & MONST_ATTACKABLE_THRU_WALLS)) ||
       (isPassableOrSecretDoor(newX, newY) && !diagonalBlocked(x, y, newX, newY, false) &&
      ...
      (!defender || canPass(monst, defender) || monsterWillAttackTarget(monst, defender)))
  {
    ...
    if (defender)
    {
      if (canPass(monst, defender))
      {
        ...
        {  // don't want a flying monster to swap a
           // non-flying monster into lava!
      ...
      else
      {
        ...
        {
          // Bog monsters and krakens won't surface on the turn that they seize
          // their target.
          monst->bookkeepingFlags &= ~MB_SUBMERGED;
        }
        refreshDungeonCell(x, y);

        buildHitList(hitList, monst, defender, (monst->info.abilityFlags & MA_ATTACKS_PENETRATE) ? true : false,
    ...
    else
    {
      for (dir = 0; dir < DIRECTION_COUNT; dir++)
      {
        if (dx == nbDirs[dir][0] && dy == nbDirs[dir][1])
        {
          break;
        }
      }
      brogueAssert(dir != NO_DIRECTION);
      if (handleWhipAttacks(monst, dir, nullptr))
      {
        monst->ticksUntilTurn = monst->attackSpeed;
        return true;
      }
...
{
  int sCols[DCOLS], sRows[DROWS], i, j, maxPermissibleDifference, dist;
  ...
  for (maxPermissibleDifference = 1; maxPermissibleDifference < max(DCOLS, DROWS); maxPermissibleDifference++)
  {
    for (i = 0; i < DCOLS; i++)
    {
      for (j = 0; j < DROWS; j++)
      {
        ...
        if (dist <= maxPermissibleDifference && dist > 0 &&
...
bool getQualifyingLocNear(int loc[2], int x, int y, bool hallwaysAllowed, char blockingMap[DCOLS][DROWS],
                             ...
                             bool deterministic)
{
  int i, j, k, candidateLocs, randIndex;
  ...
  for (k = 0; k < max(DROWS, DCOLS) && !candidateLocs; k++)
  {
    for (i = x - k; i <= x + k; i++)
    {
      for (j = y - k; j <= y + k; j++)
      {
        if (coordinatesAreInMap(i, j) && (i == x - k || i == x + k || j == y - k || j == y + k) &&
            (!blockingMap || !blockingMap[i][j]) && !cellHasTerrainFlag(i, j, forbiddenTerrainFlags) &&
            !(pmap[i][j].flags & forbiddenMapFlags) && (!forbidLiquid || pmap[i][j].layers[LIQUID] == NOTHING) &&
...
bool getQualifyingGridLocNear(int loc[2], int x, int y, bool grid[DCOLS][DROWS], bool deterministic)
{
  ...
  for (k = 0; k < max(DROWS, DCOLS) && !candidateLocs; k++)
  {
    for (i = x - k; i <= x + k; i++)
    {
      for (j = y - k; j <= y + k; j++)
      {
        if (coordinatesAreInMap(i, j) && (i == x - k || i == x + k || j == y - k || j == y + k) && grid[i][j])
        {
          candidateLocs++;
        }
      }
...
{
  int x, y;
  getQualifyingPathLocNear(&x, &y, monst->xLoc, monst->yLoc, true, T_DIVIDES_LEVEL | T_OBSTRUCTS_ITEMS, 0, 0,
                           (HAS_PLAYER | HAS_UP_STAIRS | HAS_DOWN_STAIRS | HAS_ITEM), false);
  // getQualifyingLocNear(loc, monst->xLoc, monst->yLoc, true, 0,
  // (T_OBSTRUCTS_ITEMS), (HAS_ITEM), false, false);
...
void checkForContinuedLeadership(Creature* monst)
{
  ...
  {
    for (follower = monsters->nextCreature; follower != nullptr; follower = follower->nextCreature)
    {
...
{
  Creature *follower, *newLeader = nullptr;
  ...
  for (follower = dormantMonsters->nextCreature; follower != nullptr; follower = follower->nextCreature)
  {
    if (follower->leader == monst && monst != follower)
    {
      follower->leader = nullptr;
...
void toggleMonsterDormancy(Creature* monst)
{
  ...
  for (prevMonst = dormantMonsters; prevMonst != nullptr; prevMonst = prevMonst->nextCreature)
  {
    if (prevMonst->nextCreature == monst)
    {
      ...
      if (pmap[monst->xLoc][monst->yLoc].flags & (HAS_MONSTER | HAS_PLAYER))
      {  // Occupied!
        getQualifyingPathLocNear(&(monst->xLoc), &(monst->yLoc), monst->xLoc, monst->yLoc, true,
                                 ...
                                 (HAS_PLAYER | HAS_MONSTER | HAS_UP_STAIRS | HAS_DOWN_STAIRS), false);
        //				getQualifyingLocNear(loc, monst->xLoc,
        // monst->yLoc, true, 0, T_PATHING_BLOCKER, (HAS_PLAYER | HAS_MONSTER),
        // false, false); 				monst->xLoc = loc[0];
        // monst->yLoc = loc[1];
      }
  ...
  for (prevMonst = monsters; prevMonst != nullptr; prevMonst = prevMonst->nextCreature)
  {
    if (prevMonst->nextCreature == monst)
    {
...
bool staffOrWandEffectOnMonsterDescription(char* newText, Item* theItem, Creature* monst)
{
  ...
  if ((theItem->category & (STAFF | WAND)) && tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified)
  {
    monsterName(monstName, monst, true);
    ...
    switch (boltEffectForItem(theItem))
    {
      case BE_DAMAGE:
        ...
        else if (theItem->flags & (ITEM_MAX_CHARGES_KNOWN | ITEM_IDENTIFIED))
        {
          ...
          {
            sprintf(newText, "\n     Your %s (%c) will %s the %s in one hit.", theItemName, theItem->inventoryLetter,
                    (monst->info.flags & MONST_INANIMATE) ? "destroy" : "kill", monstName);
          }
          else
          {
            sprintf(newText,
                    "\n     Your %s (%c) will hit %s for between %i%% and %i%% "
                    "of $HISHER current health.",
                    theItemName, theItem->inventoryLetter, monstName,
      ...
      case BE_POISON:
        ...
        {
          sprintf(newText,
                  "\n     Your %s (%c) will poison %s for %i%% of $HISHER "
                  "current health.",
                  theItemName, theItem->inventoryLetter, monstName,
      ...
      case BE_DOMINATION:
        if (monst->creatureState != MONSTER_ALLY)
        {
          ...
          {
            sprintf(newText,
                    "\n     A wand of domination will have no effect on objects "
                    "like %s.",
                    monstName);
          }
          else if (monst->info.flags & MONST_INVULNERABLE)
          {
            sprintf(newText, "\n     A wand of domination will have no effect on %s.", monstName);
          }
          else if (wandDominate(monst) <= 0)
          {
            sprintf(newText,
                    "\n     A wand of domination will fail at %s's current "
                    "health level.",
                    monstName);
          }
          ...
          {
            sprintf(newText,
                    "\n     A wand of domination will always succeed at %s's "
          ...
          {
            sprintf(newText,
                    "\n     A wand of domination will have a %i%% chance of "
                    "success at %s's current health level.",
                    wandDominate(monst), monstName);
          }
          successfulDescription = true;
        }
        break;
...
void monsterDetails(char buf[], Creature* monst)
{
  ...
  int i, j, combatMath, combatMath2, playerKnownAverageDamage, playerKnownMaxDamage, commaCount, realArmorValue;
      ...
      cellHasTerrainFlag(monst->xLoc, monst->yLoc, T_OBSTRUCTS_PASSABILITY))
  {
    ...
    sprintf(newText, "%s is trapped %s %s.\n     ", capMonstName,
            (tileCatalog[pmap[monst->xLoc][monst->yLoc]
                             .layers[layerWithFlag(monst->xLoc, monst->yLoc, T_OBSTRUCTS_PASSABILITY)]]
                 .mechFlags &
             TM_STAND_IN_TILE) ?
                "in" :
                "on",
  ...
  else if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && (rogue.armor->flags & ITEM_RUNIC_IDENTIFIED) &&
           rogue.armor->enchant2 == A_IMMUNITY && monsterIsInClass(monst, rogue.armor->vorpalEnemy))
  {
    itemName(rogue.armor, theItemName, false, false, nullptr);
  ...
  else
  {
    ...
    if (monst->info.abilityFlags & MA_POISONS)
    {
      ...
      if (i == 0)
      {
        ...
        sprintf(newText,
                "%s has a %i%% chance to poison you and typically poisons for "
                "%i turns.\n     ",
                capMonstName, combatMath2,
                (int)((monst->info.damage.lowerBound + monst->info.damage.upperBound) *
                          monsterDamageAdjustmentAmount(monst) / 2 +
                      FLOAT_FUDGE));
      }
      else
      {
        sprintf(newText,
                "%s has a %i%% chance to poison you, typically poisons for %i "
                "turns, and at worst, could fatally poison you in %i hit%s.\n     ",
                capMonstName, combatMath2,
                (int)((monst->info.damage.lowerBound + monst->info.damage.upperBound) *
                          monsterDamageAdjustmentAmount(monst) / 2 +
                      FLOAT_FUDGE),
    ...
    {
      combatMath = (player.currentHP + monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst) - 1) /
      ...
      sprintf(newText,
              "%s has a %i%% chance to hit you, typically hits for %i%% of your "
              "current health, and at worst, could defeat you in %i hit%s.\n     ",
              capMonstName, combatMath2,
              (int)(100 * (monst->info.damage.lowerBound + monst->info.damage.upperBound) *
                        monsterDamageAdjustmentAmount(monst) / 2 / player.currentHP +
                    FLOAT_FUDGE),
              combatMath, (combatMath > 1 ? "s" : ""));
    }
  }
  upperCase(newText);
  strcat(buf, newText);
  ...
  else
  {
    ...
    else if (rogue.weapon && (rogue.weapon->flags & ITEM_RUNIC) && (rogue.weapon->flags & ITEM_RUNIC_IDENTIFIED) &&
             rogue.weapon->enchant2 == W_SLAYING && monsterIsInClass(monst, rogue.weapon->vorpalEnemy))
    {
      i = strlen(buf);
      i = encodeMessageColor(buf, i, &goodMessageColor);
      itemName(rogue.weapon, theItemName, false, false, nullptr);
    ...
    else
    {
      ...
      sprintf(newText,
              ...
              combatMath2, monstName, 100 * playerKnownAverageDamage / monst->currentHP, combatMath,
              (combatMath > 1 ? "s" : ""));
    }
  }
  upperCase(newText);
  ...
  if (monst->carriedItem)
  {
    i = strlen(buf);
    i = encodeMessageColor(buf, i, &itemMessageColor);
    itemName(monst->carriedItem, theItemName, true, true, nullptr);
  ...
  for (i = 0; i < 32; i++)
  {
    if ((monst->info.abilityFlags & (Fl(i))) && monsterAbilityFlagDescriptions[i][0])
    {
      if (anyFlags)
      {
        strcat(newText, "& ");
  ...
  for (i = 0; i < 32; i++)
  {
    if ((monst->info.flags & (Fl(i))) && monsterBehaviorFlagDescriptions[i][0])
    {
      if (anyFlags)
      {
        strcat(newText, "& ");
  ...
  for (i = 0; i < 32; i++)
  {
    if ((monst->bookkeepingFlags & (Fl(i))) && monsterBookkeepingFlagDescriptions[i][0])
    {
      if (anyFlags)
      {
        strcat(newText, "& ");
