 ...
 *
 * Player combatants take their base defense value of their actual armor. Their accuracy is a combination of weapon,
 * armor and strength.
 *
...
{
  int accuracy = monsterAccuracyAdjusted(attacker);
  int defense = monsterDefenseAdjusted(defender);
  int hitProbability;
  ...
  if ((defender->bookkeepingFlags & MB_SEIZED) && (attacker->bookkeepingFlags & MB_SEIZING))
  {
    return 100;
  }
  ...
  if (attacker == &player && rogue.weapon)
  {
    ...
    accuracy =
        (double)player.info.accuracy * pow(WEAPON_ENCHANT_ACCURACY_FACTOR, netEnchant(rogue.weapon) + FLOAT_FUDGE);
  }
...
void addMonsterToContiguousMonsterGrid(int x, int y, Creature* monst, char grid[DCOLS][DROWS])
{
  int newX, newY;
  enum Directions dir;
  Creature* tempMonst;
...
void splitMonster(Creature* monst, int x, int y)
{
  int i, j, b, dir, newX, newY, eligibleLocationCount, randIndex;
  char buf[DCOLS * 3];
      ...
      if (monsterGrid[i][j])
      {
        for (dir = 0; dir < 4; dir++)
        {
          ...
          if (coordinatesAreInMap(newX, newY) && !eligibleGrid[newX][newY] && !monsterGrid[newX][newY] &&
              !(pmap[newX][newY].flags & (HAS_PLAYER | HAS_MONSTER)) && !monsterAvoids(monst, newX, newY))
          {
    ...
    for (i = 0; i < DCOLS; i++)
    {
      for (j = 0; j < DROWS; j++)
      {
        if (eligibleGrid[i][j] && !--randIndex)
        {
          ...
          {
            clone->info.flags &=
                (monsterCatalog[clone->info.monsterID].flags | mutationCatalog[monst->mutationIndex].monsterFlags);
          ...
          if (!(clone->info.flags & MONST_FLIES) && clone->status[STATUS_LEVITATING] == 1000)
          {
...
int alliedCloneCount(Creature* monst)
{
  int count;
  Creature* temp;

  count = 0;
  for (temp = monsters->nextCreature; temp != nullptr; temp = temp->nextCreature)
  {
    if (temp != monst && temp->info.monsterID == monst->info.monsterID && monstersAreTeammates(temp, monst))
    {
      count++;
    }
  }
  ...
  {
    for (temp = levels[rogue.depthLevel - 2].monsters; temp != nullptr; temp = temp->nextCreature)
    {
      if (temp != monst && temp->info.monsterID == monst->info.monsterID && monstersAreTeammates(temp, monst))
      {
        count++;
  ...
  {
    for (temp = levels[rogue.depthLevel].monsters; temp != nullptr; temp = temp->nextCreature)
    {
      if (temp != monst && temp->info.monsterID == monst->info.monsterID && monstersAreTeammates(temp, monst))
      {
        count++;
...
void moralAttack(Creature* attacker, Creature* defender)
{
  ...
  if (defender->currentHP > 0 && !(defender->bookkeepingFlags & MB_IS_DYING))
  {
...
void specialHit(Creature* attacker, Creature* defender, int damage)
{
  int itemCandidates, randItemIndex, stolenQuantity;
  Item *theItem = nullptr, *itemFromTopOfStack;
        ...
        && attackHit(attacker, defender))
    {
      ...
      if (itemCandidates)
      {
        ...
        if (theItem)
        {
          ...
          {  // Monkeys will steal half of a stack of weapons, and one of any other stack.
...
{
  const float effectChances[NUMBER_WEAPON_RUNIC_KINDS] = { 0.16,  // W_SPEED
                                                           0.06,  // W_QUIETUS
                                                           0.07,  // W_PARALYSIS
                                                           0.15,  // W_MULTIPLICITY
                                                           0.14,  // W_SLOWING
...
{
  int oldLoc[2], newLoc[2], forceDamage;
  char buf[DCOLS * 3], buf2[COLS], monstName[DCOLS];
  Creature* otherMonster = nullptr;
  ...
  {
    if (pmap[defender->xLoc + newLoc[0] - oldLoc[0]][defender->yLoc + newLoc[1] - oldLoc[1]].flags &
    ...
    {
      otherMonster = nullptr;
      strcpy(buf2, tileCatalog[pmap[defender->xLoc + newLoc[0] - oldLoc[0]][defender->yLoc + newLoc[1] - oldLoc[1]]
                                   .layers[highestPriorityLayer(defender->xLoc + newLoc[0] - oldLoc[0],
                                                                defender->yLoc + newLoc[1] - oldLoc[1], true)]]
                       .description);
    }
    ...
    if (!(defender->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)) &&
        inflictDamage(nullptr, defender, forceDamage, &white, false))
    {
      if (canDirectlySeeMonster(defender))
      {
    ...
    else
    {
      if (canDirectlySeeMonster(defender))
      {
        sprintf(buf, "%s slams against %s", monstName, buf2);
        buf[DCOLS] = '\0';
        combatMessage(buf, messageColorFromVictim(defender));
        autoID = true;
      }
    ...
    if (otherMonster && !(defender->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)))
    {
      if (inflictDamage(nullptr, otherMonster, forceDamage, &white, false))
      {
...
void magicWeaponHit(Creature* defender, Item* theItem, bool backstabbed)
{
  ...
  Color* effectColors[NUMBER_WEAPON_RUNIC_KINDS] = { &white, &black, &yellow,  &pink,   &green, &confusionGasColor,
                                                     nullptr,   nullptr,   &darkRed, &rainbow };
  ...
  if (chance > 0 && rand_percent(chance))
  {
    ...
    switch (enchantType)
    {
      ...
      case W_QUIETUS:
        ...
        sprintf(buf, "%s suddenly %s", monstName, (defender->info.flags & MONST_INANIMATE) ? "shatters" : "dies");
        buf[DCOLS] = '\0';
        combatMessage(buf, messageColorFromVictim(defender));
        autoID = true;
        break;
...
void attackVerb(char returnString[DCOLS], Creature* attacker, int hitPercentile)
{
  int verbCount, increment;
...
void applyArmorRunicEffect(char returnString[DCOLS], Creature* attacker, int* damage, bool melee)
{
  ...
  int newDamage, dir, newX, newY, count, i;
  float enchant;
  ...
  itemName(rogue.armor, armorName, false, false, nullptr);
    ...
    case A_MULTIPLICITY:
      if (melee && !(attacker->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) && rand_percent(33))
      {
        for (i = 0; i < armorImageCount(enchant); i++)
        {
          ...
          {
            killCreature(monst->carriedMonster,
                         true);  // Otherwise you can get infinite phoenices from a discordant phoenix.
...
void decrementWeaponAutoIDTimer()
{
  ...
  if (rogue.weapon && !(rogue.weapon->flags & ITEM_IDENTIFIED) && !--rogue.weapon->charges)
  {
    ...
    itemName(rogue.weapon, buf2, true, true, nullptr);
...
{
  int damage, specialDamage, poisonDamage;
  ...
  {
    sneakAttack =
        (defender != &player && attacker == &player && (defender->creatureState == MONSTER_WANDERING) ? true : false);
  ...
  if (sneakAttack || defenderWasAsleep || defenderWasParalyzed || lungeAttack || attackHit(attacker, defender))
  {
    ...
    damage = (defender->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE) ?
                  0 :
                  randClump(attacker->info.damage) * monsterDamageAdjustmentAmount(attacker));
    ...
    {
      if (attacker == &player && rogue.weapon && (rogue.weapon->flags & ITEM_SNEAK_ATTACK_BONUS))
      {
    ...
    if (inflictDamage(attacker, defender, damage, &red, false))
    {  // if the attack killed the defender
      ...
      {
        sprintf(buf, "%s %s %s%s", attackerName, ((defender->info.flags & MONST_INANIMATE) ? "destroyed" : "defeated"),
                defenderName, explicationClause);
      }
      if (sightUnseen)
      {
        if (defender->info.flags & MONST_INANIMATE)
...
bool canAbsorb(Creature* ally, bool ourBolts[NUMBER_BOLT_KINDS], Creature* prey, int** grid)
{
  int i;

  if (ally->creatureState == MONSTER_ALLY && ally->newPowerCount > 0 && (ally->targetCorpseLoc[0] <= 0) &&
      !((ally->info.flags | prey->info.flags) & (MONST_INANIMATE | MONST_IMMOBILE)) &&
      !monsterAvoids(ally, prey->xLoc, prey->yLoc) && grid[ally->xLoc][ally->yLoc] <= 10)
  {
    ...
    else
    {
      ...
      {
        if (!(boltCatalog[prey->info.bolts[i]].flags & BF_NOT_LEARNABLE) && !ourBolts[prey->info.bolts[i]])
        {
          return true;
        }
      }
...
bool anyoneWantABite(Creature* decedent)
{
  ...
  grid = allocGrid();
  fillGrid(grid, 0);
  calculateDistances(grid, decedent->xLoc, decedent->yLoc, T_PATHING_BLOCKER, nullptr, true, true);
  for (ally = monsters->nextCreature; ally != nullptr; ally = ally->nextCreature)
  {
    if (canAbsorb(ally, ourBolts, decedent, grid))
    {  // This populates ourBolts if it returns true.
      candidates++;
    }
  }
  if (candidates > 0)
  {
    ...
    if (ally)
    {
      ...
      if (candidates > 0)
      {
        ...
        for (i = 0; i < 32; i++)
        {
          if ((Fl(i) & ~(ally->info.abilityFlags) & decedent->info.abilityFlags & LEARNABLE_ABILITIES) && !--randIndex)
          {
        ...
        {
          if ((Fl(i) & ~(ally->info.flags) & decedent->info.flags & LEARNABLE_BEHAVIORS) && !--randIndex)
          {
            ally->absorptionFlags = Fl(i);
...
{
  inflictDamage(attacker, defender, defender->currentHP, nullptr, true);
}
                      ...
                      bool ignoresProtectionShield)
{
  ...
  if (defender == &player && rogue.easyMode && damage > 0)
  {
  ...
  {
    transferenceAmount =
        min(damage, defender->currentHP);  // Maximum transferred damage can't exceed the victim's remaining health.
...
void addPoison(Creature* monst, int durationIncrement, int concentrationIncrement)
{
...
// Removes the decedent from the screen and from the monster chain; inserts it into the graveyard chain; does NOT free
// the memory. Or, if the decedent is a player ally at the moment of death, insert it into the purgatory chain for
// possible future resurrection. Use "administrativeDeath" if the monster is being deleted for administrative purposes,
// as opposed to dying as a result of physical actions. AdministrativeDeath means the monster simply disappears, with no
// messages, dropped item, DFs or other effect.
void killCreature(Creature* decedent, bool administrativeDeath)
{
  ...
  else
  {
    ...
    if (decedent->leader == &player && !(decedent->info.flags & MONST_INANIMATE) &&
        (decedent->bookkeepingFlags & MB_HAS_SOUL) && !administrativeDeath)
    {
    ...
    if (!administrativeDeath && !(decedent->bookkeepingFlags & MB_IS_DORMANT))
    {
      ...
      if (decedent->carriedMonster)
      {
        ...
        decedent->carriedMonster = nullptr;
      }
...
void buildHitList(Creature** hitList, const Creature* attacker, Creature* defender, const bool penetrate,
                  const bool sweep)
{
  int i, x, y, newX, newY, newestX, newestY;
...
int monsterPower(const Creature* theMonst)
{
  int damageDealt[2] = { 0, 0 };
  int statuses[NUMBER_OF_STATUS_EFFECTS];
  int ticksTillTurn[2] = { 0, 0 }, speed;
  int i, k;
  ...
  damagePerHit[0] = (theMonst->info.damage.lowerBound + theMonst->info.damage.upperBound) *
                    monsterDamageAdjustmentAmount(theMonst) / 2;
  damagePerHit[1] = 10;
  ...
  {  // Loop until the dummy kills the monster in the simulation.
    for (k = 0; k <= 1; k++)
    {  // k is whose turn it is
