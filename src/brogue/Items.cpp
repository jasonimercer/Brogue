...
Item* initializeItem()
{
  ...
  theItem->category = ItemCategory::NONE;
  theItem->kind = 0;
  theItem->flags = 0;
  theItem->displayChar = '&';
...
unsigned long pickItemCategory(ItemCategory theCategory)
{
  int i, sum, randIndex;
  int probabilities[13] = { 50, 42, 52, 3, 3, 10, 8, 2, 3, 2, 0, 0, 0 };
  ItemCategory correspondingCategories[13] = { GOLD, SCROLL, POTION, STAFF,  WAND, WEAPON, ARMOR,
                                                 FOOD, RING,   CHARM,  AMULET, GEM,  KEY };
...
Item* makeItemInto(Item* theItem, ItemCategory itemCategory, int itemKind)
{
  ...
  switch (itemCategory)
  {
    ...
    case WEAPON:
      ...
      if (rand_percent(40))
      {
        ...
        else if (rand_range(3, 10) * ((theItem->flags & ITEM_ATTACKS_HIT_SLOWLY) ? 2 : 1) /
    ...
    case ARMOR:
      ...
      if (rand_percent(40))
      {
        ...
        else if (rand_range(0, 95) > theItem->armor)
    ...
    case STAFF:
      ...
      theItem->enchant2 =
          (itemKind == STAFF_BLINKING || itemKind == STAFF_OBSTRUCTION ? 1000 : 500);  // start with no recharging mojo
      break;
    case WAND:
      if (itemKind < 0)
      {
  ...
  if (theItem && !(theItem->flags & ITEM_IDENTIFIED) &&
...
{
  int i, totalFrequencies = 0, randomFrequency;
...
Item* placeItem(Item* theItem, int x, int y)
{
  ...
  removeItemFromChain(theItem, floorItems);  // just in case; double-placing an item will result in game-crashing loops
                                             // in the item list
  ...
  if (cellHasTerrainFlag(x, y, T_IS_DF_TRAP) && !cellHasTerrainFlag(x, y, T_MOVES_ITEMS) &&
      !(pmap[x][y].flags & PRESSURE_PLATE_DEPRESSED))
  {
    ...
    if (playerCanSee(x, y))
    {
      ...
      itemName(theItem, theItemName, false, false, nullptr);
...
void fillItemSpawnHeatMap(unsigned int heatMap[DCOLS][DROWS], unsigned int heatLevel, int x, int y)
{
  enum Directions dir;
  int newX, newY;
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) &&
        (!cellHasTerrainFlag(newX, newY,
                             T_OBSTRUCTS_PASSABILITY | T_IS_DEEP_WATER | T_LAVA_INSTA_DEATH | T_AUTO_DESCENT) ||
...
void coolHeatMapAt(unsigned int heatMap[DCOLS][DROWS], int x, int y, unsigned long* totalHeat)
{
  int k, l;
  unsigned int currentHeat;
...
bool getItemSpawnLoc(unsigned int heatMap[DCOLS][DROWS], int* x, int* y, unsigned long* totalHeat)
{
...
void populateItems(int upstairsX, int upstairsY)
{
  ...
  Item* theItem;
  unsigned int itemSpawnHeatMap[DCOLS][DROWS];
  int i, j, numberOfItems, numberOfGoldPiles, goldBonusProbability, x = 0, y = 0;
  unsigned long totalHeat;
  int theCategory, theKind, randomDepthOffset = 0;
  ...
  if (rogue.depthLevel > AMULET_LEVEL)
  {
    ...
    {
      const int lumenstoneDistribution[8] = { 3, 3, 3, 2, 2, 2, 2, 2 };
  ...
  else
  {
    ...
    for (goldBonusProbability = 60; rand_percent(goldBonusProbability) && numberOfGoldPiles <= 10;
  ...
  for (j = 0; j < DROWS; j++)
  {
    for (i = 0; i < DCOLS; i++)
    {
      if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_ITEMS | T_PATHING_BLOCKER) ||
          (pmap[i][j].flags & (IS_CHOKEPOINT | IN_LOOP | IS_IN_MACHINE)) || passableArcCount(i, j) > 1)
      {  // Not in hallways, quest rooms, loops or chokepoints, please.
...
#ifdef AUDIT_RNG
      sprintf(RNGmessage, "%u%s%s\t%s", itemSpawnHeatMap[i][j],
  ...
  for (i = 0; i < numberOfItems; i++)
  {
    ...
    if ((theItem->category & SCROLL) && theItem->kind == SCROLL_ENCHANTING)
    {
      ...
      if (D_MESSAGE_ITEM_GENERATION)
        printf("\n(?)  Depth %i: generated an enchant scroll at %i frequency", rogue.depthLevel,
               rogue.enchantScrollFrequency);
    }
    ...
    {
      if (D_MESSAGE_ITEM_GENERATION)
        printf("\n(!s) Depth %i: generated a strength potion at %i frequency", rogue.depthLevel,
               rogue.strengthPotionFrequency);
  ...
  scrollTable[SCROLL_ENCHANTING].frequency =
      0;  // No enchant scrolls or strength/life potions can spawn except via initial
...
// Name of this function is a bit misleading -- basically returns true iff the item will stack without consuming an
// extra slot i.e. if it's a throwing weapon with a sibling already in your pack. False for potions and scrolls.
bool itemWillStackWithPack(Item* theItem)
{
  ...
  {
    for (tempItem = packItems->nextItem; tempItem != nullptr && tempItem->quiverNumber != theItem->quiverNumber;
...
void pickUpItemAt(int x, int y)
{
  ...
  if ((theItem->flags & ITEM_KIND_AUTO_ID) && tableForItemCategory(theItem->category, nullptr) &&
      !(tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified))
  {
    identifyItemKind(theItem);
  }

  if ((theItem->category & WAND) && wandTable[theItem->kind].identified &&
  ...
  if (numberOfItemsInPack() < MAX_PACK_ITEMS || (theItem->category & GOLD) || itemWillStackWithPack(theItem))
  {
    ...
    itemName(theItem, buf2, true, true, nullptr);  // include suffix, article
    ...
    if ((theItem->category & AMULET) && !(rogue.yendorWarden))
    {
  ...
  else
  {
    ...
    itemName(theItem, buf2, false, true, nullptr);  // include article
...
Item* addItemToPack(Item* theItem)
{
  ...
  {
    for (tempItem = packItems->nextItem; tempItem != nullptr; tempItem = tempItem->nextItem)
    {
  ...
  for (previousItem = packItems;
       previousItem->nextItem != nullptr && previousItem->nextItem->category <= theItem->category;
...
void checkForDisenchantment(Item* theItem)
{
  ...
  if ((theItem->flags & ITEM_RUNIC) && theItem->enchant2 < NUMBER_GOOD_ARMOR_ENCHANT_KINDS && theItem->enchant1 <= 0)
  {
    theItem->enchant2 = 0;
    ...
    {
      identify(theItem);
      itemName(theItem, buf2, false, false, nullptr);
...
{
  if ((theItem->category & CAN_BE_SWAPPED) && theItem->quiverNumber == 0)
  {
    return true;
  }
  else
...
void swapItemToEnchantLevel(Item* theItem, int newEnchant, bool enchantmentKnown)
{
  int x, y, charmPercent;
  char buf1[COLS * 3], buf2[COLS * 3];

  if ((theItem->category & STAFF) && newEnchant < 2 || (theItem->category & CHARM) && newEnchant < 1 ||
      (theItem->category & WAND) && newEnchant < 0)
  {
    itemName(theItem, buf1, false, true, nullptr);
    sprintf(buf2, "%s shatter%s from the strain!", buf1, theItem->quantity == 1 ? "s" : "");
    x = theItem->xLoc;
    y = theItem->yLoc;
  ...
  {
    if ((theItem->category & STAFF) && theItem->charges > newEnchant)
    {
...
{
  if ((theItem->category & STAFF) && (theItem->flags & ITEM_MAX_CHARGES_KNOWN))
  {
    return true;
  }
  else
...
int effectiveEnchantLevel(const Item* theItem)
{
  if (theItem->category & WAND)
  {
...
bool swapItemEnchants(const int machineNumber)
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      ...
      if (tempItem && pmap[i][j].machineNumber == machineNumber && itemIsSwappable(tempItem))
      {
        if (lockedItem)
        {
...
void updateFloorItems()
{
  ...
  for (theItem = floorItems->nextItem; theItem != nullptr; theItem = nextItem)
  {
    ...
    if (cellHasTerrainFlag(x, y, T_AUTO_DESCENT))
    {
      ...
      {
        itemName(theItem, buf, false, false, nullptr);
    ...
    if (pmap[x][y].machineNumber && pmap[x][y].machineNumber == pmap[player.xLoc][player.yLoc].machineNumber &&
        (theItem->flags & ITEM_KIND_AUTO_ID))
    {
      identifyItemKind(theItem);
    }
    if (cellHasTMFlag(x, y, TM_SWAP_ENCHANTS_ACTIVATION) && pmap[x][y].machineNumber)
    {
      while (nextItem != nullptr && pmap[x][y].machineNumber == pmap[nextItem->xLoc][nextItem->yLoc].machineNumber &&
      ...
      if (!circuitBreakersPreventActivation(pmap[x][y].machineNumber) && swapItemEnchants(pmap[x][y].machineNumber))
      {
...
bool inscribeItem(Item* theItem)
{
  ...
  theItem->inscription[0] = '\0';
  itemName(theItem, nameOfItem, true, true, nullptr);
  strcpy(theItem->inscription, oldInscription);

  sprintf(buf, "inscribe: %s \"", nameOfItem);
  if (getInputTextString(itemText, buf, min(29, DCOLS - strLenWithoutEscapes(buf) - 1), "", "\"", TEXT_INPUT_NORMAL,
                         false))
  {
...
void call(Item* theItem)
{
  ...
  if (theItem == nullptr)
  {
    ...
    for (tempItem = packItems->nextItem; tempItem != nullptr; tempItem = tempItem->nextItem)
    {
      if ((tempItem->category & (POTION | SCROLL)) &&
          tableForItemCategory(tempItem->category, nullptr)[tempItem->kind].identified)
      {
    ...
    theItem = promptForItemOfType(
        (WEAPON | ARMOR | SCROLL | RING | POTION | STAFF | WAND | CHARM), ITEM_CAN_BE_IDENTIFIED, 0,
        KEYBOARD_LABELS ? "Call what? (a-z, shift for more info; or <esc> to cancel)" : "Call what?", true);
  ...
  {
    if (tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified)
    {
      if (inscribeItem(theItem))
      {
        command[c++] = '\0';
  ...
  if (tableForItemCategory(theItem->category, nullptr) &&
      !(tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified))
  {
    if (getInputTextString(itemText, "call them: \"", 29, "", "\"", TEXT_INPUT_NORMAL, false))
    {
      ...
      if (itemText[0])
      {
        strcpy(tableForItemCategory(theItem->category, nullptr)[theItem->kind].callTitle, itemText);
        tableForItemCategory(theItem->category, nullptr)[theItem->kind].called = true;
      }
      else
      {
        tableForItemCategory(theItem->category, nullptr)[theItem->kind].callTitle[0] = '\0';
        tableForItemCategory(theItem->category, nullptr)[theItem->kind].called = false;
      }
      confirmMessages();
      itemName(theItem, buf, false, true, nullptr);
      messageWithColor(buf, &itemMessageColor, false);
    }
  }
  else
...
//	a "sandalwood" staff, a "ruby" ring) will be in dark purple, and the Amulet of Yendor and lumenstones will be in
//yellow.
...
{
  char buf[DCOLS * 5], pluralization[10], article[10] = "", grayEscapeSequence[5], purpleEscapeSequence[5],
                                          yellowEscapeSequence[5], baseEscapeSequence[5];
  Color tempColor;
  ...
  switch (theItem->category)
  {
      ...
      if (includeDetails)
      {
        ...
        if (theItem->flags & ITEM_RUNIC)
        {
          if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience)
          {
            if (theItem->enchant2 == W_SLAYING)
            {
              sprintf(root, "%s of %s slaying%s", root, MonsterClassCatalog[theItem->vorpalEnemy].name,
                      grayEscapeSequence);
            }
            else
            {
              sprintf(root, "%s of %s%s", root, weaponRunicNames[theItem->enchant2], grayEscapeSequence);
            }
          }
    ...
    case ARMOR:
      ...
      if (includeDetails)
      {
        ...
        if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
        {
          ...
          {
            sprintf(buf, "%s%i %s%s [%i]<%i>", (theItem->enchant1 < 0 ? "" : "+"), theItem->enchant1, root,
                    grayEscapeSequence, theItem->armor / 10 + theItem->enchant1, theItem->strengthRequired);
          }
          strcpy(root, buf);
        }
        else
        ...
        if ((theItem->flags & ITEM_RUNIC) && (theItem->flags & (ITEM_IDENTIFIED | ITEM_RUNIC_HINTED)) &&
            !(theItem->flags & ITEM_RUNIC_IDENTIFIED) && !rogue.playbackOmniscience)
        {
          strcat(root, " (unknown runic)");
        }
      }
    ...
    case SCROLL:
      ...
      {
        sprintf(root, "scroll%s called %s%s%s", pluralization, purpleEscapeSequence,
                scrollTable[theItem->kind].callTitle, baseEscapeSequence);
      }
      else
      {
        sprintf(root, "scroll%s entitled %s\"%s\"%s", pluralization, purpleEscapeSequence,
                scrollTable[theItem->kind].flavor, baseEscapeSequence);
      }
      break;
    case POTION:
      ...
      {
        sprintf(root, "potion%s called %s%s%s", pluralization, purpleEscapeSequence,
                potionTable[theItem->kind].callTitle, baseEscapeSequence);
      }
      else
      {
        sprintf(root, "%s%s%s potion%s", purpleEscapeSequence, potionTable[theItem->kind].flavor, baseEscapeSequence,
                pluralization);
      }
      break;
    case WAND:
      ...
      {
        sprintf(root, "wand%s called %s%s%s", pluralization, purpleEscapeSequence, wandTable[theItem->kind].callTitle,
                baseEscapeSequence);
      }
      else
      {
        sprintf(root, "%s%s%s wand%s", purpleEscapeSequence, wandTable[theItem->kind].flavor, baseEscapeSequence,
                pluralization);
      }
      if (includeDetails)
      {
        ...
        {
          sprintf(root, "%s%s [%i]", root, grayEscapeSequence, theItem->charges);
        }
        else if (theItem->enchant2 > 2)
        {
          sprintf(root, "%s%s (used %i times)", root, grayEscapeSequence, theItem->enchant2);
        }
        else if (theItem->enchant2)
        {
          sprintf(root, "%s%s (used %s)", root, grayEscapeSequence, (theItem->enchant2 == 2 ? "twice" : "once"));
        }
      }
      break;
    case STAFF:
      ...
      {
        sprintf(root, "staff%s called %s%s%s", pluralization, purpleEscapeSequence, staffTable[theItem->kind].callTitle,
                baseEscapeSequence);
      }
      else
      {
        sprintf(root, "%s%s%s staff%s", purpleEscapeSequence, staffTable[theItem->kind].flavor, baseEscapeSequence,
                pluralization);
      }
      if (includeDetails)
      {
    ...
    case RING:
      ...
      {
        sprintf(root, "ring%s called %s%s%s", pluralization, purpleEscapeSequence, ringTable[theItem->kind].callTitle,
                baseEscapeSequence);
      }
      else
      {
        sprintf(root, "%s%s%s ring%s", purpleEscapeSequence, ringTable[theItem->kind].flavor, baseEscapeSequence,
                pluralization);
      }
    ...
    case CHARM:
      ...
      if (includeDetails)
      {
        ...
        if (theItem->charges)
        {
          sprintf(buf, "%s %s(%i%%)", root, grayEscapeSequence,
                  (charmRechargeDelay(theItem->kind, theItem->enchant1) - theItem->charges) * 100 /
    ...
    case KEY:
      ...
      {
        sprintf(root, "%s%s%s from depth %i", keyTable[theItem->kind].name, pluralization, grayEscapeSequence,
                theItem->originDepth);
      }
      else
      {
...
bool isVowelish(char* theChar)
{
  ...
  if (stringsMatch(str, "UNI")  // Words that start with "uni" aren't treated like vowels; e.g., "a" unicorn.
      || stringsMatch(str, "EU"))
  {  // Words that start with "eu" aren't treated like vowels; e.g., "a" eucalpytus staff.
...
int charmEffectDuration(int charmKind, int enchant)
{
  ...
  const int increment[NUMBER_CHARM_KINDS] = {
    0,   // Health
    0,   // Protection
    20,  // Haste
    25,  // Fire immunity
...
int charmRechargeDelay(int charmKind, int enchant)
{
  ...
  return charmEffectDuration(charmKind, enchant) +
         duration[charmKind] * (pow((double)(100 - (increment[charmKind])) / 100, enchant) + FLOAT_FUDGE);
}
...
int effectiveRingEnchant(Item* theItem)
{
...
int apparentRingBonus(const enum RingKind kind)
{
...
void itemDetails(char* buf, Item* theItem)
{
  ...
  float enchant;
  int nextLevelState = 0, new;
  float accuracyChange, damageChange, current, currentDamage, newDamage;
  const char weaponRunicEffectDescriptions[NUMBER_WEAPON_RUNIC_KINDS][DCOLS] = { "time will stop while you take an "
                                                                                 "extra turn",
  ...
  // Name
  itemName(theItem, theName, true, true, nullptr);
  buf[0] = '\0';
  ...
  itemName(theItem, theName, false, false, nullptr);

  // introductory text
  if (tableForItemCategory(theItem->category, nullptr) &&
      (tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified || rogue.playbackOmniscience))
  {
    strcat(buf, tableForItemCategory(theItem->category, nullptr)[theItem->kind].description);

    if (theItem->category == POTION && theItem->kind == POTION_LIFE)
    {
      sprintf(buf2, "\n\nIt will increase your maximum health by %s%i%%%s.", goodColorEscape,
              (player.info.maxHP + 10) * 100 / player.info.maxHP - 100, whiteColorEscape);
      strcat(buf, buf2);
    }
  }
  else
    ...
    switch (theItem->category)
    {
      case POTION:
        sprintf(buf2, "%s flask%s contain%s a swirling %s liquid. Who knows what %s will do when drunk or thrown?",
                (singular ? "This" : "These"), (singular ? "" : "s"), (singular ? "s" : ""),
                tableForItemCategory(theItem->category, nullptr)[theItem->kind].flavor, (singular ? "it" : "they"));
        break;
      case SCROLL:
        sprintf(buf2,
                "%s parchment%s %s covered with indecipherable writing, and bear%s a title of \"%s.\" Who knows what "
                "%s will do when read aloud?",
                (singular ? "This" : "These"), (singular ? "" : "s"), (singular ? "is" : "are"), (singular ? "s" : ""),
                tableForItemCategory(theItem->category, nullptr)[theItem->kind].flavor, (singular ? "it" : "they"));
        break;
      case STAFF:
        sprintf(buf2, "This gnarled %s staff is warm to the touch. Who knows what it will do when used?",
                tableForItemCategory(theItem->category, nullptr)[theItem->kind].flavor);
        break;
      case WAND:
      ...
      case RING:
        sprintf(buf2,
                "This metal band is adorned with a%s %s gem that glitters in the darkness. Who knows what effect it "
                "has when worn? ",
                isVowelish(tableForItemCategory(theItem->category, nullptr)[theItem->kind].flavor) ? "n" : "",
                tableForItemCategory(theItem->category, nullptr)[theItem->kind].flavor);
        break;
      case CHARM:  // Should never be displayed.
        ...
        break;
      case AMULET:
        strcpy(buf2, "Legends are told about this mysterious golden amulet, and hundreds of adventurers have perished "
                     "in its pursuit. Unfathomable riches await anyone with the skill and ambition to carry it into "
                     "the light of day.");
        break;
      case GEM:
        sprintf(buf2,
                "Mysterious lights swirl and fluoresce beneath the stone%s surface. Lumenstones are said to contain "
                "mysterious properties of untold power, but for you, they mean one thing: riches.",
                (singular ? "'s" : "s'"));
        break;
      case KEY:
  ...
  {
    sprintf(buf2, " (You found %s on depth %i.) ", singular ? "it" : "them", theItem->originDepth);
    strcat(buf, buf2);
  }
  ...
  switch (theItem->category)
  {
    case FOOD:
      sprintf(buf2, "\n\nYou are %shungry enough to fully enjoy a %s.",
              ((STOMACH_SIZE - player.status[STATUS_NUTRITION]) >= foodTable[theItem->kind].strengthRequired ? "" :
                                                                                                               "not "
                                                                                                               "yet "),
    ...
    case ARMOR:
      ...
      if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
      {
        if (theItem->enchant1)
        {
          if (theItem->enchant1 > 0)
          {
            sprintf(buf2, "\n\nThe %s bear%s an intrinsic enchantment of %s+%i%s", theName, (singular ? "s" : ""),
                    goodColorEscape, theItem->enchant1, whiteColorEscape);
          }
          else
          {
            sprintf(buf2, "\n\nThe %s bear%s an intrinsic penalty of %s%i%s", theName, (singular ? "s" : ""),
                    badColorEscape, theItem->enchant1, whiteColorEscape);
          }
        }
        else
        {
        ...
        {
          sprintf(
              buf2, ", %s %s %s %s%s%.2f%s because of your %s strength. ", (theItem->enchant1 ? "and" : "but"),
              ...
              (strengthModifier(theItem) > 0 ? goodColorEscape : badColorEscape), strengthModifier(theItem),
              whiteColorEscape, (strengthModifier(theItem) > 0 ? "excess" : "inadequate"));
          strcat(buf, buf2);
        }
        else
        {
      ...
      else
      {
        ...
        {
          sprintf(buf2, "\n\nYou can feel an %saura of benevolent magic%s radiating from the %s. ", goodColorEscape,
                  whiteColorEscape, theName);
          strcat(buf, buf2);
        }
        if (strengthModifier(theItem))
        {
          sprintf(buf2, "\n\nThe %s %s%s a %s%s%.2f%s because of your %s strength. ", theName,
                  ((theItem->enchant1 > 0) && (theItem->flags & ITEM_MAGIC_DETECTED) ? "also " : ""),
                  (singular ? "carries" : "carry"), (strengthModifier(theItem) > 0 ? "bonus of +" : "penalty of "),
                  (strengthModifier(theItem) > 0 ? goodColorEscape : badColorEscape), strengthModifier(theItem),
                  whiteColorEscape, (strengthModifier(theItem) > 0 ? "excess" : "inadequate"));
          strcat(buf, buf2);
        }
        ...
        if (theItem->category & WEAPON)
        {
          sprintf(buf2, "It will reveal its secrets if you defeat %i%s %s with it. ", theItem->charges,
                  (theItem->charges == WEAPON_KILLS_TO_AUTO_ID ? "" : " more"),
                  (theItem->charges == 1 ? "enemy" : "enemies"));
        }
        else
        {
          sprintf(buf2, "It will reveal its secrets if worn for %i%s turn%s. ", theItem->charges,
                  (theItem->charges == ARMOR_DELAY_TO_AUTO_ID ? "" : " more"), (theItem->charges == 1 ? "" : "s"));
        }
        strcat(buf, buf2);
      }
      ...
      if (!(theItem->flags & ITEM_EQUIPPED))
      {
        if (theItem->category & WEAPON)
        {
          ...
          sprintf(buf2,
                  "Wielding the %s%s will %s your current accuracy by %s%i%%%s, and will %s your current damage by "
                  "%s%i%%%s. ",
                  theName,
                  ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) ? "" :
                                                                                      ", assuming it has no hidden "
                                                                                      "properties,",
                  (((short)accuracyChange) < 0) ? "decrease" : "increase",
                  (((short)accuracyChange) < 0) ? badColorEscape : (accuracyChange > 0 ? goodColorEscape : ""),
                  abs((short)accuracyChange), whiteColorEscape, (((short)damageChange) < 0) ? "decrease" : "increase",
                  (((short)damageChange) < 0) ? badColorEscape : (damageChange > 0 ? goodColorEscape : ""),
                  abs((short)damageChange), whiteColorEscape);
        }
        else
        {
          new = theItem->armor;
          ...
          new = max(0, new);
          new /= 10;
          sprintf(buf2, "Wearing the %s%s will result in an armor rating of %s%i%s. ", theName,
                  ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) ? "" :
                                                                                      ", assuming it has no hidden "
                                                                                      "properties,",
                  (new > displayedArmorValue() ? goodColorEscape :
                                                 (new < displayedArmorValue() ? badColorEscape : whiteColorEscape)),
                  (int)(new + FLOAT_FUDGE), whiteColorEscape);
        }
        strcat(buf, buf2);
      }
      ...
      if ((theItem->category & ARMOR) && !(theItem->flags & ITEM_EQUIPPED) &&
          (current != armorAggroAdjustment(theItem)))
      {
        ...
        sprintf(buf2, "Equipping the %s will %s%s your stealth range by %i%s. ", theName,
                new > 0 ? badColorEscape : goodColorEscape, new > 0 ? "increase" : "decrease", abs(new),
                whiteColorEscape);
        strcat(buf, buf2);
      }
      ...
      if (theItem->category & WEAPON)
      {
        ...
        if (theItem->flags & ITEM_RUNIC)
        {
          if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience)
          {
            ...
            if (theItem->enchant2 == W_SLAYING)
            {
              ...
              sprintf(buf2, "It will never fail to slay a%s %s in a single stroke. ", (isVowelish(buf3) ? "n" : ""),
                      buf3);
              strcat(buf, buf2);
            }
            else if (theItem->enchant2 == W_MULTIPLICITY)
            {
              ...
              {
                sprintf(buf2,
                        "%i%% of the time that it hits an enemy, %i spectral %s%s will spring into being with accuracy "
                        "and attack power equal to your own, and will dissipate %i turns later. (If the %s is "
                        "enchanted, %i image%s will appear %i%% of the time, and will last %i turns.)",
                        runicWeaponChance(theItem, false, 0), weaponImageCount(enchant), theName,
                        (weaponImageCount(enchant) > 1 ? "s" : ""), weaponImageDuration(enchant), theName,
              ...
              {
                sprintf(buf2,
                        "Sometimes, when it hits an enemy, spectral %ss will spring into being with accuracy and "
                        "attack power equal to your own, and will dissipate shortly thereafter.",
                        theName);
              }
              strcat(buf, buf2);
            }
            else
            {
              if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
              {
                ...
                {
                  strcpy(buf2, "Its runic effect will almost never activate because of your inadequate strength, but "
                               "sometimes, when");
                }
                else
                {
              ...
              if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
              {
                switch (theItem->enchant2)
                {
                  ...
                  case W_PARALYSIS:
                    ...
                    strcat(buf, buf2);
                    nextLevelState =
                        (int)(weaponParalysisDuration((float)(enchant + enchantIncrement(theItem))) + FLOAT_FUDGE);
                    break;
                  case W_SLOWING:
                  ...
                  case W_FORCE:
                    sprintf(buf2,
                            " up to %i spaces backward. If the enemy hits an obstruction, it (and any monster it hits) "
                            "will take damage in proportion to the distance it flew. ",
                ...
                if (((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) &&
                    runicWeaponChance(theItem, false, 0) <
                        runicWeaponChance(theItem, true, (float)(enchant + enchantIncrement(theItem))))
                {
                  sprintf(buf2, "(If the %s is enchanted, the chance will increase to %i%%", theName,
                          runicWeaponChance(theItem, true, (float)(enchant + enchantIncrement(theItem))));
                  strcat(buf, buf2);
                  if (nextLevelState)
                  {
        ...
        if (theItem->flags & ITEM_EQUIPPED)
        {
          sprintf(buf2, "\n\nYou hold the %s at the ready%s. ", theName,
          ...
          strcat(buf, buf2);
                 ...
                 (theItem->flags & ITEM_CURSED))
        {
          sprintf(buf2, "\n\n%sYou can feel a malevolent magic lurking within the %s.%s ", badColorEscape, theName,
                  whiteColorEscape);
          strcat(buf, buf2);
        }
      }
      else if (theItem->category & ARMOR)
      {
        ...
        if (theItem->flags & ITEM_RUNIC)
        {
          if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience)
          {
            ...
            switch (theItem->enchant2)
            {
              case A_MULTIPLICITY:
                sprintf(buf2,
                        "When worn, 33%% of the time that an enemy's attack connects, %i allied spectral duplicate%s "
                        "of your attacker will appear for 3 turns. ",
                ...
                {
                  sprintf(buf3, "(If the %s is enchanted, the number of duplicates will increase to %i.) ", theName,
                          (armorImageCount((float)enchant + enchantIncrement(theItem))));
                  strcat(buf2, buf3);
                }
                break;
              case A_MUTUALITY:
                strcpy(buf2, "When worn, the damage that you incur from physical attacks will be split evenly among "
                             "yourself and all other adjacent enemies. ");
                break;
              case A_ABSORPTION:
                if (theItem->flags & ITEM_IDENTIFIED)
                {
                  sprintf(
                      buf2,
                      "It will reduce the damage of inbound attacks by a random amount between 0 and %i, which is %i%% "
                      "of your current maximum health. (If the %s is enchanted, this maximum amount will %s %i.) ",
                      (int)armorAbsorptionMax(enchant), (int)(100 * armorAbsorptionMax(enchant) / player.info.maxHP),
                      theName,
                      (armorAbsorptionMax(enchant) == armorAbsorptionMax((float)(enchant + enchantIncrement(theItem))) ?
                           "remain at" :
                           "increase to"),
                ...
                {
                  strcpy(buf2, "It will reduce the damage of inbound attacks by a random amount determined by its "
                               "enchantment level. ");
                }
                break;
              case A_REPRISAL:
                ...
                {
                  sprintf(buf2,
                          "Any enemy that attacks you will itself be wounded by %i%% of the damage that it inflicts. "
                          "(If the %s is enchanted, this percentage will increase to %i%%.) ",
                          armorReprisalPercent(enchant), theName,
                          armorReprisalPercent((float)(enchant + enchantIncrement(theItem))));
                }
                else
                {
                  strcpy(buf2, "Any enemy that attacks you will itself be wounded by a percentage (determined by "
                               "enchantment level) of the damage that it inflicts. ");
                }
                break;
              case A_IMMUNITY:
                describeMonsterClass(buf3, theItem->vorpalEnemy, false);
              ...
              case A_REFLECTION:
                if (theItem->flags & ITEM_IDENTIFIED)
                {
                  if (theItem->enchant1 > 0)
                  {
                    int reflectChance = reflectionChance(enchant);
                    int reflectChance2 = reflectionChance(enchant + enchantIncrement(theItem));
                    sprintf(buf2,
                            "When worn, you will deflect %i%% of incoming spells -- including directly back at their "
                            "source %i%% of the time. (If the armor is enchanted, these will increase to %i%% and "
                            "%i%%.) ",
                            reflectChance, reflectChance * reflectChance / 100, reflectChance2,
                  ...
                  else if (theItem->enchant1 < 0)
                  {
                    ...
                    int reflectChance2 = reflectionChance(enchant + enchantIncrement(theItem));
                    sprintf(buf2,
                            "When worn, %i%% of your own spells will deflect from their target -- including directly "
                            "back at you %i%% of the time. (If the armor is enchanted, these will decrease to %i%% and "
                            "%i%%.) ",
                            reflectChance, reflectChance * reflectChance / 100, reflectChance2,
                            reflectChance2 * reflectChance2 / 100);
                  }
                ...
                {
                  strcpy(buf2, "When worn, you will deflect some percentage of incoming spells, determined by "
                               "enchantment level. ");
                }
                break;
              case A_RESPIRATION:
                strcpy(buf2, "When worn, it will maintain a pocket of fresh air around you, rendering you immune to "
                             "the effects of steam and all toxic gases. ");
                break;
              case A_DAMPENING:
                strcpy(buf2, "When worn, it will safely absorb the concussive impact of any explosions (though you may "
                             "still be burned). ");
                break;
              case A_BURDEN:
        ...
        if (theItem->flags & ITEM_EQUIPPED)
        {
          sprintf(buf2, "\n\nYou are wearing the %s%s. ", theName,
    ...
    case STAFF:
      ...
      if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
      {
        ...
        sprintf(buf2,
                ...
                "approximately %i turns. ",
                theName, theItem->charges, theItem->enchant1, new == 0 ? "" : ", with your current rings,",
                staffChargeDuration(theItem) / ringWisdomMultiplier(new));
        strcat(buf, buf2);
      }
      else if (theItem->flags & ITEM_MAX_CHARGES_KNOWN)
      {
        sprintf(buf2, "\n\nThe %s has a maximum of %i charges, and recovers its charges gradually over time. ", theName,
                theItem->enchant1);
        strcat(buf, buf2);
      }
          ...
          rogue.playbackOmniscience)
      {
        switch (theItem->kind)
        {
          case STAFF_LIGHTNING:
            sprintf(buf2,
                    "This staff deals damage to every creature in its line of fire; nothing is immune. (If the staff "
                    "is enchanted, its average damage will increase by %i%%.)",
                    (int)(100 * (staffDamageLow(enchant + 1) + staffDamageHigh(enchant + 1)) /
                              (staffDamageLow(enchant) + staffDamageHigh(enchant)) -
                          100));
            break;
          case STAFF_FIRE:
            sprintf(buf2,
                    "This staff deals damage to any creature that it hits, unless the creature is immune to fire. (If "
                    "the staff is enchanted, its average damage will increase by %i%%.) It also sets creatures and "
          ...
          case STAFF_POISON:
            sprintf(buf2,
                    ...
                    staffPoison(enchant), staffPoison(enchant + 1));
            break;
          case STAFF_TUNNELING:
            sprintf(buf2,
                    "The bolt from this staff will dissolve %i layers of obstruction. (If the staff is enchanted, this "
                    "will increase to %i layers.)",
                    theItem->enchant1, theItem->enchant1 + 1);
            break;
          case STAFF_BLINKING:
            sprintf(buf2,
                    "This staff enables you to teleport up to %i spaces. (If the staff is enchanted, this will "
                    "increase to %i spaces.)",
                    staffBlinkDistance(theItem->enchant1), staffBlinkDistance(theItem->enchant1 + 1));
            break;
          case STAFF_ENTRANCEMENT:
            sprintf(buf2,
                    "This staff will compel its target to mirror your movements for %i turns. (If the staff is "
                    "enchanted, this will increase to %i turns.)",
                    staffEntrancementDuration(theItem->enchant1), staffEntrancementDuration(theItem->enchant1 + 1));
            break;
          case STAFF_HEALING:
            if (enchant < 10)
            {
              sprintf(buf2,
                      "This staff will heal its target by %i%% of its maximum health. (If the staff is enchanted, this "
                      "will increase to %i%%.)",
                      theItem->enchant1 * 10, (theItem->enchant1 + 1) * 10);
            }
            else
            {
          ...
          case STAFF_HASTE:
            sprintf(buf2,
                    "This staff will cause its target to move twice as fast for %i turns. (If the staff is enchanted, "
                    "this will increase to %i turns.)",
                    staffHasteDuration(theItem->enchant1), staffHasteDuration(theItem->enchant1 + 1));
            break;
          ...
          case STAFF_DISCORD:
            sprintf(buf2,
                    "This staff will cause discord for %i turns. (If the staff is enchanted, this will increase to %i "
                    "turns.)",
                    staffDiscordDuration(theItem->enchant1), staffDiscordDuration(theItem->enchant1 + 1));
            break;
          case STAFF_CONJURATION:
            sprintf(buf2,
                    "%i phantom blades will be called into service. (If the staff is enchanted, this will increase to "
                    "%i blades.)",
                    staffBladeCount(theItem->enchant1), staffBladeCount(theItem->enchant1 + 1));
            break;
          case STAFF_PROTECTION:
            sprintf(buf2,
                    "This staff will shield a creature for up to 20 turns against up to %i damage. (If the staff is "
                    "enchanted, this will increase to %i damage.)",
                    staffProtection(theItem->enchant1) / 10, staffProtection(theItem->enchant1 + 1) / 10);
            break;
          default:
    ...
    case WAND:
      ...
      if ((theItem->flags & (ITEM_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN)) || rogue.playbackOmniscience)
      {
        if (theItem->charges)
        {
          sprintf(buf2, "%i charge%s remain%s. Enchanting this wand will add %i charge%s.", theItem->charges,
                  (theItem->charges == 1 ? "" : "s"), (theItem->charges == 1 ? "s" : ""),
      ...
      else
      {
        if (theItem->enchant2)
        {
          sprintf(buf2, "You have used this wand %i time%s, but do not know how many charges, if any, remain.",
                  theItem->enchant2, (theItem->enchant2 == 1 ? "" : "s"));
        }
        else
        {
        ...
        if (wandTable[theItem->kind].identified)
        {
          strcat(buf, buf2);
          sprintf(buf2,
                  " Wands of this type can be found with %i to %i charges. Enchanting this wand will add %i charge%s.",
                  wandTable[theItem->kind].range.lowerBound, wandTable[theItem->kind].range.upperBound,
                  wandTable[theItem->kind].range.lowerBound,
                  (wandTable[theItem->kind].range.lowerBound == 1 ? "" : "s"));
        }
    ...
    case RING:
        ...
        if (theItem->enchant1)
        {
          switch (theItem->kind)
          {
            case RING_CLAIRVOYANCE:
              ...
              {
                sprintf(buf2,
                        "\n\nThis ring provides magical sight with a radius of %i. (If the ring is enchanted, this "
                        "will increase to %i.)",
                        theItem->enchant1 + 1, theItem->enchant1 + 2);
              }
              else
              {
                sprintf(buf2,
                        "\n\nThis ring magically blinds you to a radius of %i. (If the ring is enchanted, this will "
                        "decrease to %i.)",
                        (theItem->enchant1 * -1) + 1, (theItem->enchant1 * -1));
              }
              strcat(buf, buf2);
              break;
            case RING_REGENERATION:
              sprintf(buf2,
                      "\n\nWith this ring equipped, you will regenerate all of your health in %li turns (instead of "
                      "%li). (If the ring is enchanted, this will decrease to %li turns.)",
                      (long)(turnsForFullRegen(theItem->enchant1) / 1000), (long)TURNS_FOR_FULL_REGEN,
                      (long)(turnsForFullRegen(theItem->enchant1 + 1) / 1000));
              strcat(buf, buf2);
              break;
            case RING_TRANSFERENCE:
              sprintf(buf2,
                      "\n\nDealing direct damage to a creature (whether in melee or otherwise) will %s you by %i%% of "
                      "the damage dealt. (If the ring is enchanted, this will %s to %i%%.)",
                      (theItem->enchant1 >= 0 ? "heal" : "harm"), abs(theItem->enchant1) * 5,
                      (theItem->enchant1 >= 0 ? "increase" : "decrease"), abs(theItem->enchant1 + 1) * 5);
              strcat(buf, buf2);
              break;
            case RING_WISDOM:
              sprintf(buf2,
            ...
            case RING_REAPING:
              sprintf(buf2,
                      "\n\nEach blow that you land in melee will %s your staffs and charms by 0-%i turns per point of "
                      "damage dealt. (If the ring is enchanted, this will %s to 0-%i turns per point of damage.)",
                      (theItem->enchant1 >= 0 ? "recharge" : "drain"), abs(theItem->enchant1),
                      (theItem->enchant1 >= 0 ? "increase" : "decrease"), abs(theItem->enchant1 + 1));
              strcat(buf, buf2);
              break;
            default:
              break;
      ...
      {
        sprintf(buf2, "\n\nIt will reveal its secrets if worn for %i%s turn%s", theItem->charges,
                (theItem->charges == RING_DELAY_TO_AUTO_ID ? "" : " more"), (theItem->charges == 1 ? "" : "s"));
        strcat(buf, buf2);

        if ((theItem->charges < RING_DELAY_TO_AUTO_ID || (theItem->flags & (ITEM_MAGIC_DETECTED | ITEM_IDENTIFIED))) &&
            theItem->enchant1 > 0)
        {  // Mention the unknown-positive-ring footnote only if it's good magic and you know it.

          sprintf(buf2, ", and until you understand its secrets, it will function as a +%i ring.",
      ...
      if (theItem->flags & ITEM_EQUIPPED)
      {
        sprintf(buf2, "\n\nThe %s is on your finger%s. ", theName,
                ((theItem->flags & ITEM_CURSED) ? ", and because it is cursed, you are powerless to remove it" : ""));
        strcat(buf, buf2);
      }
      else if (((theItem->flags & (ITEM_IDENTIFIED | ITEM_MAGIC_DETECTED)) || rogue.playbackOmniscience) &&
      ...
      switch (theItem->kind)
      {
        case CHARM_HEALTH:
          sprintf(buf2,
                  ...
                  charmHealing(theItem->enchant1), charmRechargeDelay(theItem->kind, theItem->enchant1),
                  charmHealing(theItem->enchant1 + 1), charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
          break;
        case CHARM_PROTECTION:
          sprintf(buf2,
                  "\n\nWhen used, the charm will shield you for up to 20 turns against up to %i damage and recharge in "
                  "%i turns. (If the charm is enchanted, it will block up to %i damage and recharge in %i turns.)",
                  charmProtection(theItem->enchant1) / 10, charmRechargeDelay(theItem->kind, theItem->enchant1),
        ...
        case CHARM_FIRE_IMMUNITY:
          sprintf(buf2,
                  "\n\nWhen used, the charm will grant you immunity to fire for %i turns and recharge in %i turns. (If "
                  "the charm is enchanted, the immunity will last %i turns and it will recharge in %i turns.)",
                  charmEffectDuration(theItem->kind, theItem->enchant1),
                  charmRechargeDelay(theItem->kind, theItem->enchant1),
                  charmEffectDuration(theItem->kind, theItem->enchant1 + 1),
                  charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
        ...
        case CHARM_SHATTERING:
          sprintf(buf2,
                  "\n\nWhen used, the charm will dissolve the nearby walls and recharge in %i turns. (If the charm is "
                  "enchanted, it will recharge in %i turns.)",
                  charmRechargeDelay(theItem->kind, theItem->enchant1),
                  charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
          break;
        case CHARM_GUARDIAN:
          sprintf(buf2,
                  "\n\nWhen used, a guardian will materialize for %i turns, and the charm will recharge in %i turns. "
                  "(If the charm is enchanted, the guardian will last for %i turns and the charm will recharge in %i "
                  "turns.)",
                  charmGuardianLifespan(theItem->enchant1), charmRechargeDelay(theItem->kind, theItem->enchant1),
        ...
        case CHARM_NEGATION:
          sprintf(buf2,
                  "\n\nWhen used, the charm will negate all magical effects on the creatures in your field of view and "
                  "the items on the ground up to %i spaces away, and recharge in %i turns. (If the charm is enchanted, "
                  "it will reach up to %i spaces and recharge in %i turns.)",
                  charmNegationRadius(theItem->enchant1), charmRechargeDelay(theItem->kind, theItem->enchant1),
                  charmNegationRadius(theItem->enchant1 + 1), charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
          break;
        default:
          break;
      }
...
{
  if (!(theItem->flags & ITEM_MAGIC_DETECTED) || (theItem->category & PRENAMED_CATEGORY))
  {
    return false;
  }
  else
...
{
  Item* theItem;
  int i, j, m, maxLength = 0, itemNumber, itemCount, equippedItemCount;
  ...
  int highlightItemLine, itemSpaceRemaining;
  ...
  char whiteColorEscapeSequence[20], grayColorEscapeSequence[20], yellowColorEscapeSequence[20],
      darkYellowColorEscapeSequence[20], goodColorEscapeSequence[20], badColorEscapeSequence[20];
  char* magicEscapePtr;
  ...
  for (i = 0; i < itemNumber; i++)
  {
    ...
    if ((theItem->category & categoryMask) && !(~(theItem->flags) & requiredFlags) &&
    ...
    if ((theItem->flags & ITEM_MAGIC_DETECTED) && !(theItem->category & AMULET))
    {  // Won't include food, keys, lumenstones or amulet.
      ...
      sprintf(buttons[i].text, " %c%c %s* %s* %s%s%s%s", KEYBOARD_LABELS ? theItem->inventoryLetter : ' ',
              (theItem->flags & ITEM_PROTECTED ? '}' : closeParen), magicEscapePtr,
              (buttons[i].flags & B_HOVER_ENABLED) ? yellowColorEscapeSequence : darkYellowColorEscapeSequence,
              (buttons[i].flags & B_HOVER_ENABLED) ? whiteColorEscapeSequence : grayColorEscapeSequence, buf,
    ...
    else
    {
      sprintf(buttons[i].text, " %c%c %s%s* %s%s%s%s",  // The '*' is the item character, e.g. ':' for food.
              KEYBOARD_LABELS ? theItem->inventoryLetter : ' ', (theItem->flags & ITEM_PROTECTED ? '}' : closeParen),
  ...
  if (waitForAcknowledge)
  {
    ...
    if (itemSpaceRemaining)
    {
      sprintf(buttons[itemNumber + extraLineCount].text, "%s%s    You have room for %i more item%s.",
              grayColorEscapeSequence, (magicDetected ? "  " : ""), itemSpaceRemaining,
              (itemSpaceRemaining == 1 ? "" : "s"));
    }
    else
    {
      sprintf(buttons[itemNumber + extraLineCount].text, "%s%s    Your pack is full.", grayColorEscapeSequence,
              (magicDetected ? "  " : ""));
    }
    ...
    sprintf(buttons[itemNumber + extraLineCount].text,
            ...
            grayColorEscapeSequence, (magicDetected ? "  " : ""));
    maxLength = max(maxLength, (strLenWithoutEscapes(buttons[itemNumber + extraLineCount].text)));
    extraLineCount++;
  }
  if (equippedItemCount)
  {
    ...
    sprintf(buttons[itemNumber + extraLineCount].text, "      %s%s---", (magicDetected ? "  " : ""),
            grayColorEscapeSequence);
  ...
  do
  {
    ...
    // Do the button loop.
    highlightItemLine = -1;
    overlayDisplayBuffer(rbuf, nullptr);  // Remove the inventory display while the buttons are active,
    ...
    highlightItemLine =
        buttonInputLoop(buttons,
                        ...
                        COLS - maxLength, mapToWindowY(0), maxLength, itemNumber + extraLineCount, &theEvent);
    ...
    if (highlightItemLine > -1 && (waitForAcknowledge || theEvent.shiftKey || theEvent.controlKey))
    {
      do
      {
        ...
        drawButton(&(buttons[highlightItemLine]), BUTTON_PRESSED, nullptr);
        ...
        if (theEvent.shiftKey || theEvent.controlKey || waitForAcknowledge)
        {
          ...
          actionKey = printCarriedItemDetails(theItem, max(2, mapToWindowX(DCOLS - maxLength - 42)), mapToWindowY(2),
                                              40, includeButtons, nullptr);

          overlayDisplayBuffer(rbuf, nullptr);  // remove the item info window

          if (actionKey == -1)
          {
            repeatDisplay = true;
            overlayDisplayBuffer(dbuf, nullptr);  // redisplay the inventory
          }
          else
          {
            restoreRNG;
            repeatDisplay = false;
            overlayDisplayBuffer(rbuf, nullptr);  // restore the original screen
          }
...
int displayedArmorValue()
{
  ...
  {
    return (
        short)(((armorTable[rogue.armor->kind].range.upperBound + armorTable[rogue.armor->kind].range.lowerBound) / 2) /
                   10 +
               strengthModifier(rogue.armor) + FLOAT_FUDGE);
  }
}
...
void strengthCheck(Item* theItem)
{
  ...
  if (theItem)
  {
    if (theItem->category & WEAPON && theItem->strengthRequired > rogue.strength - player.weaknessAmount)
    {
      ...
      strcpy(buf1, "");
      itemName(theItem, buf1, false, false, nullptr);
    ...
    if (theItem->category & ARMOR && theItem->strengthRequired > rogue.strength - player.weaknessAmount)
    {
      ...
      strcpy(buf1, "");
      itemName(theItem, buf1, false, false, nullptr);
      sprintf(buf2, "You stagger under the weight of the %s; %i more strength would be ideal.", buf1,
              strengthDeficiency);
      message(buf2, false);
    }
  }
...
{
  Item* previouslyEquippedItem = nullptr;
...
void equip(Item* theItem)
{
  ...
  {
    theItem = promptForItemOfType(
        (WEAPON | ARMOR | RING), 0, ITEM_EQUIPPED,
  ...
  if (theItem->category & (WEAPON | ARMOR | RING))
  {
    if (theItem->category & RING)
    {
      ...
      else if (rogue.ringLeft && rogue.ringRight)
      {
        ...
        {
          if (theItem2)
          {  // No message if canceled or did an inventory action instead.
        ...
        else
        {
          ...
          {
            itemName(theItem2, buf1, false, false, nullptr);
    ...
    if (!canEquip(theItem))
    {
      ...
      if (theItem->category & WEAPON)
      {
        itemName(rogue.weapon, buf1, false, false, nullptr);
      }
      else if (theItem->category & ARMOR)
      {
        itemName(rogue.armor, buf1, false, false, nullptr);
      }
      else
      {
        sprintf(buf1, "one");
    ...
    itemName(theItem, buf2, true, true, nullptr);
...
bool keyMatchesLocation(Item* theItem, int x, int y)
{
  int i;
...
void aggravateMonsters(int distance, int x, int y, const Color* flashColor)
{
  ...
  grid = allocGrid();
  fillGrid(grid, 0);
  calculateDistances(grid, x, y, T_PATHING_BLOCKER, nullptr, true, false);
...
int getLineCoordinates(int listOfCoordinates[][2], const int originLoc[2], const int targetLoc[2])
{
  ...
  int largerTargetComponent, currentVector[2], previousVector[2], quadrantTransform[2], i;
  int currentLoc[2], previousLoc[2];
  int cellNumber = 0;
  ...
  do
  {
    ...
    // DEBUG printf("\ncell %i: (%i, %i)", cellNumber, listOfCoordinates[cellNumber][0],
    // listOfCoordinates[cellNumber][1]);
    cellNumber++;
                  ...
                  const bool returnLastEmptySpace)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (monst && !monsterIsHidden(monst, monsterAtLoc(originLoc[0], originLoc[1])) &&
...
bool impermissibleKinkBetween(int x1, int y1, int x2, int y2)
{
  brogueAssert(coordinatesAreInMap(x1, y1));
  brogueAssert(coordinatesAreInMap(x2, y2));
  if (cellHasTerrainFlag(x1, y1, T_OBSTRUCTS_PASSABILITY) || cellHasTerrainFlag(x2, y2, T_OBSTRUCTS_PASSABILITY))
  {
  ...
  if (!cellHasTerrainFlag(x2, y1, T_OBSTRUCTS_PASSABILITY) || !cellHasTerrainFlag(x1, y2, T_OBSTRUCTS_PASSABILITY))
  {
...
bool tunnelize(int x, int y)
{
  ...
  if (didSomething)
  {
    ...
    if (pmap[x][y].flags & HAS_MONSTER)
    {
      ...
      {
        inflictLethalDamage(nullptr, monst);
      }
    }
  }
  if (!cellHasTerrainFlag(x, y, T_OBSTRUCTS_DIAGONAL_MOVEMENT) && didSomething)
  {
    ...
    for (dir = 0; dir < DIRECTION_COUNT; dir++)
    {
      ...
      {
        if ((pmap[x][y2].flags & IMPREGNABLE) || (!(pmap[x2][y].flags & IMPREGNABLE) && rand_percent(50)))
        {
          tunnelize(x2, y);
        }
        else
...
{
  int retval =
      monst->info.accuracy * (pow(WEAPON_ENCHANT_ACCURACY_FACTOR, -2.5 * (float)monst->weaknessAmount) + FLOAT_FUDGE);
  return max(retval, 0);
}
...
{
  int previousDamageTaken, healthFraction, newMonsterIndex;
  ...
  } while (monsterCatalog[newMonsterIndex].flags & (MONST_INANIMATE | MONST_NO_POLYMORPH)  // Can't turn something into
...
void heal(Creature* monst, int percent, bool panacea)
{
  ...
  if (canDirectlySeeMonster(monst) && monst != &player && !panacea)
  {
    monsterName(monstName, monst, true);
    sprintf(buf, "%s looks healthier", monstName);
    combatMessage(buf, nullptr);
  }
}

void makePlayerTelepathic(int duration)
{
  Creature* monst;
  ...
  if (monsters->nextCreature == nullptr)
  {
...
void rechargeItems(unsigned long categories)
{
  ...
  for (tempItem = packItems->nextItem; tempItem != nullptr; tempItem = tempItem->nextItem)
  {
    if (tempItem->category & categories & STAFF)
    {
      ...
      tempItem->enchant2 =
          (tempItem->kind == STAFF_BLINKING || tempItem->kind == STAFF_OBSTRUCTION ? 10000 : 5000) / tempItem->enchant1;
    }
...
void negationBlast(const char* emitterName, const int distance)
{
  ...
  for (monst = monsters->nextCreature; monst != nullptr;)
  {
    nextMonst = monst->nextCreature;
    if ((pmap[monst->xLoc][monst->yLoc].flags & IN_FIELD_OF_VIEW) &&
        (player.xLoc - monst->xLoc) * (player.xLoc - monst->xLoc) +
                (player.yLoc - monst->yLoc) * (player.yLoc - monst->yLoc) <=
            distance * distance)
    {
      if (canSeeMonster(monst))
      {
  ...
  for (theItem = floorItems; theItem != nullptr; theItem = theItem->nextItem)
  {
    if ((pmap[theItem->xLoc][theItem->yLoc].flags & IN_FIELD_OF_VIEW) &&
        (player.xLoc - theItem->xLoc) * (player.xLoc - theItem->xLoc) +
                (player.yLoc - theItem->yLoc) * (player.yLoc - theItem->yLoc) <=
            distance * distance)
    {
      ...
      switch (theItem->category)
      {
        ...
        case RING:
          theItem->enchant1 = 0;
          theItem->flags |=
              ITEM_IDENTIFIED;  // Reveal that it is (now) +0, but not necessarily which kind of ring it is.
...
void discordBlast(const char* emitterName, const int distance)
{
  Creature *monst, *nextMonst;
  char buf[DCOLS];
...
bool imbueInvisibility(Creature* monst, int duration)
{
  bool autoID = false;
...
bool projectileReflects(Creature* attacker, Creature* defender)
{
  ...
  if (defender == &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && rogue.armor->enchant2 == A_IMMUNITY &&
      monsterIsInClass(attacker, rogue.armor->vorpalEnemy) && monstersAreEnemies(attacker, defender))
  {
    return true;
  }
...
{
  int k, target[2], origin[2], newPath[DCOLS][2], newPathLength, failsafe, finalLength;
...
void beckonMonster(Creature* monst, int x, int y)
{
  int from[2], to[2];
...
enum BoltEffects boltEffectForItem(Item* theItem)
{
  ...
  {
    return boltCatalog[tableForItemCategory(theItem->category, nullptr)[theItem->kind].strengthRequired].boltEffect;
  }
  else
  {
    return BE_NONE;
...
enum BoltType boltForItem(Item* theItem)
{
  if (theItem->category & (STAFF | WAND))
  {
    return tableForItemCategory(theItem->category, nullptr)[theItem->kind].strengthRequired;
  }
  else
  {
    return 0;
...
bool updateBolt(Bolt* theBolt, Creature* caster, int x, int y, bool boltInView, bool alreadyReflected,
                   bool* autoID, bool* lightingChanged)
{
    ...
    switch (theBolt->boltEffect)
    {
      ...
      case BE_DAMAGE:
            ...
            (monst->info.flags & MONST_INVULNERABLE))
        {
          if (canSeeMonster(monst))
          {
            sprintf(buf, "%s ignore%s %s %s", monstName, (monst == &player ? "" : "s"),
                    canSeeMonster(caster) ? "the" : "a", theBolt->name);
            combatMessage(buf, 0);
          }
        }
        else if (inflictDamage(caster, monst, staffDamage(theBolt->magnitude), theBolt->backColor, false))
        {
          ...
          {
            sprintf(buf, "%s %s %s %s", canSeeMonster(caster) ? "the" : "a", theBolt->name,
                    ((monst->info.flags & MONST_INANIMATE) ? "destroys" : "kills"), monstName);
            combatMessage(buf, messageColorFromVictim(monst));
          }
          else
          {
            sprintf(buf, "you hear %s %s", monstName,
                    ((monst->info.flags & MONST_INANIMATE) ? "get destroyed" : "die"));
            combatMessage(buf, 0);
          }
        }
        else
        {
          // monster lives
          if (monst->creatureMode != MODE_PERM_FLEEING && monst->creatureState != MONSTER_ALLY &&
      ...
      case BE_TELEPORT:
        ...
        break;
      case BE_BECKONING:
        if (!(monst->info.flags & MONST_IMMOBILE) && caster &&
          ...
          if (canSeeMonster(monst))
          {
            ...
            if (monst != &player)
            {
              sprintf(buf, "%s %s %s sick", monstName, (monst == &player ? "feel" : "looks"),
                      (monst->status[STATUS_POISONED] * monst->poisonAmount >= monst->currentHP &&
      ...
      case BE_ENTRANCEMENT:
        ...
        {
          monst->status[STATUS_ENTRANCED] = monst->maxStatus[STATUS_ENTRANCED] =
              staffEntrancementDuration(theBolt->magnitude);
          wakeUp(monst);
          if (canSeeMonster(monst))
          {
      ...
      case BE_DISCORD:
        if (!(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)))
        {
          monst->status[STATUS_DISCORDANT] = monst->maxStatus[STATUS_DISCORDANT] =
              max(staffDiscordDuration(theBolt->magnitude), monst->status[STATUS_DISCORDANT]);
          if (canSeeMonster(monst))
          {
  ...
  if ((theBolt->flags & BF_FIERY) && exposeTileToFire(x, y, true))
  {
    if (lightingChanged)
    {
      *lightingChanged = true;
  ...
  if ((theBolt->flags & BF_ELECTRIC) && exposeTileToElectricity(x, y))
  {
    if (lightingChanged)
    {
      *lightingChanged = true;
...
// If the effect is visible enough for the player to identify the shooting item,
// *autoID will be set to true. (AutoID can be null.)
void detonateBolt(Bolt* theBolt, Creature* caster, int x, int y, bool* autoID)
{
  DungeonFeature feat;
  int i, x2, y2;
  Creature* monst;
    ...
    case BE_CONJURATION:
      for (i = 0; i < (staffBladeCount(theBolt->magnitude)); i++)
      {
        ...
        getQualifyingPathLocNear(&(monst->xLoc), &(monst->yLoc), x, y, true,
                                 T_DIVIDES_LEVEL & avoidedFlagsForMonster(&(monst->info)) & ~T_SPONTANEOUSLY_IGNITES,
                                 HAS_PLAYER, avoidedFlagsForMonster(&(monst->info)) & ~T_SPONTANEOUSLY_IGNITES,
                                 (HAS_PLAYER | HAS_MONSTER | HAS_UP_STAIRS | HAS_DOWN_STAIRS), false);
...
{
  int listOfCoordinates[MAX_BOLT_LENGTH][2];
  int i, j, k, x, y, x2, y2, numCells, blinkDistance, boltLength, initialBoltLength, lights[DCOLS][DROWS][3];
  Creature *monst = nullptr, *shootingMonst;
  char buf[COLS], monstName[COLS];
  bool autoID = false;
  ...
  {
    if (cellHasTerrainFlag(listOfCoordinates[0][0], listOfCoordinates[0][1],
  ...
  if (boltColor)
  {
    for (i = 0; i < initialBoltLength; i++)
    {
      ...
      boltLights[i].lightRadius.lowerBound = boltLights[i].lightRadius.upperBound =
          50 * (3 + theBolt->magnitude * 1.33) * (initialBoltLength - i) / initialBoltLength;
    }
  }
  ...
  for (i = 0; i < numCells; i++)
  {
    ...
    if (monst && !(theBolt->flags & BF_NEVER_REFLECTS) && projectileReflects(shootingMonst, monst) &&
        i < MAX_BOLT_LENGTH - max(DCOLS, DROWS))
    {
      if (projectileReflects(shootingMonst, monst))
      {  // if it scores another reflection roll, reflect at caster
      ...
      {
        monsterName(monstName, monst, true);
        sprintf(buf, "%s deflect%s the %s", monstName, (monst == &player ? "" : "s"), theBolt->name);
        combatMessage(buf, 0);
      }
      if (monst == &player && rogue.armor && rogue.armor->enchant2 == A_REFLECTION &&
    ...
    for (k = min(i, boltLength + 2); k >= 0; k--)
    {
      ...
      if (playerCanSeeOrSense(x2, y2))
      {
        if (!fastForward)
        {
          ...
          if (displayChar && theBolt->foreColor && theBolt->theChar)
          {
          ...
          else if (k == 1 && theBolt->foreColor && theBolt->theChar)
          {
    ...
    if (cellHasTerrainFlag(x, y, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION)) &&
        theBolt->boltEffect == BE_TUNNELING && tunnelize(x, y))
    {
      updateVision(true);
      backUpLighting(lights);
      autoID = true;
    ...
    // Can happen with a cursed deflection ring or a reflective terrain target, or when shooting a tunneling bolt into
    // an impregnable wall.
    if (i + 1 < numCells && !(theBolt->flags & BF_NEVER_REFLECTS))
    {
      ...
      y2 = listOfCoordinates[i + 1][1];
      if (cellHasTerrainFlag(x2, y2, (T_OBSTRUCTS_VISION | T_OBSTRUCTS_PASSABILITY)) &&
          (projectileReflects(shootingMonst, nullptr) || cellHasTMFlag(x2, y2, TM_REFLECTS_BOLTS) ||
           (theBolt->boltEffect == BE_TUNNELING && (pmap[x2][y2].flags & IMPREGNABLE))) &&
          i < MAX_BOLT_LENGTH - max(DCOLS, DROWS))
      {
        sprintf(buf, "the bolt reflects off of %s", tileText(x2, y2));
        if (projectileReflects(shootingMonst, nullptr))
        {
  ...
  if (boltLength > 0)
  {
    if (boltColor)
    {
      ...
      for (j = i; j < i + boltLength + 2; j++)
      {  // j can imply a bolt tip position that is off the map
        ...
        for (k = min(j, boltLength + 2); k >= j - i; k--)
        {
          if (playerCanSee(listOfCoordinates[j - k][0], listOfCoordinates[j - k][1]))
          {
            if (boltColor)
            {
              hiliteCell(listOfCoordinates[j - k][0], listOfCoordinates[j - k][1], boltColor,
                         max(0, 100 - k * 100 / (boltLength)), false);
            }
            boltInView = true;
          }
        }
...
// Relies on the sidebar entity list. If one is already selected, select the next qualifying. Otherwise, target the
// first qualifying.
bool nextTargetAfter(int* returnX, int* returnY, int targetX, int targetY, bool targetEnemies,
                        bool targetAllies, bool targetItems, bool targetTerrain, bool requireOpenPath,
                        bool reverseDirection)
{
  int i, n, targetCount, newX, newY;
  int selectedIndex = 0;
  Creature* monst;
  Item* theItem;
  int deduplicatedTargetList[ROWS][2];
  ...
  for (i = 0; i < ROWS; i++)
  {
    ...
    {
      if (targetCount == 0 || deduplicatedTargetList[targetCount - 1][0] != rogue.sidebarLocationList[i][0] ||
          deduplicatedTargetList[targetCount - 1][1] != rogue.sidebarLocationList[i][1])
      {
        ...
        if (rogue.sidebarLocationList[i][0] == targetX && rogue.sidebarLocationList[i][1] == targetY)
        {
  ...
  for (i = reverseDirection ? targetCount - 1 : 0; reverseDirection ? i >= 0 : i < targetCount;
       reverseDirection ? i-- : i++)
  {
    n = (selectedIndex + i) % targetCount;
    newX = deduplicatedTargetList[n][0];
    newY = deduplicatedTargetList[n][1];
    if ((newX != player.xLoc || newY != player.yLoc) && (newX != targetX || newY != targetY) &&
...
int hiliteTrajectory(int coordinateList[DCOLS][2], int numCells, bool eraseHiliting,
                       bool passThroughMonsters, const Color* hiliteColor)
{
  int x, y, i;
  Creature* monst;
  ...
  for (i = 0; i < numCells; i++)
  {
    ...
    if (cellHasTerrainFlag(x, y, (T_OBSTRUCTS_VISION | T_OBSTRUCTS_PASSABILITY)) || pmap[x][y].flags & (HAS_PLAYER))
    {
      i++;
      break;
    }
    ...
    {
      monst = monsterAtLoc(x, y);
      if (!(monst->bookkeepingFlags & MB_SUBMERGED) && !monsterIsHidden(monst, &player))
      {
        i++;
        break;
      }
                   ...
                   ButtonState* state, bool colorsDance, bool keysMoveCursor, bool targetCanLeaveMap)
{
  ...
  int* cursor = rogue.cursorLoc;  // shorthand
  ...
  *targetConfirmed = *canceled = *tabKey = false;
  ...
  do
  {
    ...
    if (state)
    {  // Also running a button loop.

      // Update the display.
      overlayDisplayBuffer(state->dbuf, nullptr);

      // Get input.
      nextBrogueEvent(&theEvent, false, colorsDance, true);

      // Process the input.
      buttonInput = processButtonInput(state, nullptr, &theEvent);
      ...
      // Revert the display.
      overlayDisplayBuffer(state->rbuf, nullptr);
    }
    else
    ...
    {
      if (theEvent.param1 >= 0 && theEvent.param1 < mapToWindowX(0) && theEvent.param2 >= 0 &&
          theEvent.param2 < ROWS - 1 && rogue.sidebarLocationList[theEvent.param2][0] > -1)
      {
               ...
               targetCanLeaveMap && theEvent.eventType != MOUSE_UP)
      {
        ...
        if (theEvent.eventType == MOUSE_UP && !theEvent.shiftKey &&
            (theEvent.controlKey ||
             (cursor[0] == windowToMapX(theEvent.param1) && cursor[1] == windowToMapY(theEvent.param2))))
        {
          *targetConfirmed = true;
        }
    ...
    {
      // The sidebar is highlighted but the cursor is not on a visible item, monster or terrain. Un-highlight the
      // sidebar.
      refreshSideBar(-1, -1, false);
      sidebarHighlighted = false;
    }
...
void pullMouseClickDuringPlayback(int loc[2])
{
  RogueEvent theEvent;
...
bool chooseTarget(int returnLoc[2], int maxDistance, bool stopAtTarget, bool autoTarget,
                     bool targetAllies, bool passThroughCreatures, const Color* trajectoryColor)
{
  int originLoc[2], targetLoc[2], oldTargetLoc[2], coordinates[DCOLS][2], numCells, i, distance, newX, newY;
  Creature* monst;
  ...
  if (autoTarget)
  {
    if (rogue.lastTarget && canSeeMonster(rogue.lastTarget) &&
        (targetAllies == (rogue.lastTarget->creatureState == MONSTER_ALLY)) &&
        rogue.lastTarget->depth == rogue.depthLevel && !(rogue.lastTarget->bookkeepingFlags & MB_IS_DYING) &&
  ...
  do
  {
    ...
    monst = monsterAtLoc(targetLoc[0], targetLoc[1]);
    if (monst != nullptr && monst != &player && canSeeMonster(monst))
    {
      focusedOnSomething = true;
    }
    else if (playerCanSeeOrSense(targetLoc[0], targetLoc[1]) && (pmap[targetLoc[0]][targetLoc[1]].flags & HAS_ITEM) ||
             cellHasTMFlag(targetLoc[0], targetLoc[1], TM_LIST_IN_SIDEBAR))
    {
      focusedOnSomething = true;
    }
    ...
    moveCursor(&targetConfirmed, &canceled, &tabKey, targetLoc, &event, nullptr, false, true, false);
    if (event.eventType == RIGHT_MOUSE_UP)
    {  // Right mouse cancels.
      canceled = true;
    }
...
{
  ItemTable* theTable;
  int tableCount, i, lastItem;

  theTable = tableForItemCategory(theItem->category, nullptr);
  if (theTable)
  {
    theItem->flags &= ~ITEM_KIND_AUTO_ID;
...
void autoIdentify(Item* theItem)
{
      ...
      !tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified)
  {
    ...
    theItem->quantity = 1;
    itemName(theItem, newName, false, true, nullptr);
    theItem->quantity = quantityBackup;
    sprintf(buf, "(It must %s %s.)", ((theItem->category & (POTION | SCROLL)) ? "have been" : "be"), newName);
    messageWithColor(buf, &itemMessageColor, false);
  }

  if ((theItem->category & (WEAPON | ARMOR)) && (theItem->flags & ITEM_RUNIC) &&
      !(theItem->flags & ITEM_RUNIC_IDENTIFIED))
  {
    itemName(theItem, oldName, false, false, nullptr);
    theItem->flags |= (ITEM_RUNIC_IDENTIFIED | ITEM_RUNIC_HINTED);
    itemName(theItem, newName, true, true, nullptr);
...
bool hitMonsterWithProjectileWeapon(Creature* thrower, Creature* monst, Item* theItem)
{
  ...
  if (monst != &player && monst->creatureMode != MODE_PERM_FLEEING &&
      (monst->creatureState != MONSTER_FLEEING || monst->status[STATUS_MAGICAL_FEAR]) &&
      !(monst->bookkeepingFlags & MB_CAPTIVE) && monst->creatureState != MONSTER_ALLY)
  {
    monst->creatureState = MONSTER_TRACKING_SCENT;
  ...
  if (thrownWeaponHit)
  {
    damage = (monst->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE) ? 0 : randClump(theItem->damage)) *
             pow(WEAPON_ENCHANT_DAMAGE_FACTOR, netEnchant(theItem));
    ...
    if (inflictDamage(thrower, monst, damage, &red, false))
    {  // monster killed
      sprintf(buf, "the %s %s %s.", theItemName, (monst->info.flags & MONST_INANIMATE) ? "destroyed" : "killed",
              targetName);
...
void throwItem(Item* theItem, Creature* thrower, int targetLoc[2], int maxDistance)
{
  int listOfCoordinates[MAX_BOLT_LENGTH][2], originLoc[2];
  int i, x, y, numCells;
  Creature* monst = nullptr;
  ...
  if (thrower != &player && (pmap[originLoc[0]][originLoc[1]].flags & IN_FIELD_OF_VIEW))
  {
    ...
    itemName(theItem, buf3, false, true, nullptr);
  ...
  for (i = 0; i < numCells && i < maxDistance; i++)
  {
    ...
    if (cellHasTerrainFlag(x, y, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION)))
    {
      ...
      {
        // Incendiary darts thrown at flammable obstructions (foliage, wooden barricades, doors) will hit the
        // obstruction instead of bursting a cell earlier.
      }
      else if (cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY) && cellHasTMFlag(x, y, TM_PROMOTES_ON_PLAYER_ENTRY) &&
               tileCatalog[pmap[x][y].layers[layerWithTMFlag(x, y, TM_PROMOTES_ON_PLAYER_ENTRY)]].flags &
      ...
      else
      {
        ...
        {  // it was aimed point-blank into an obstruction
          x = thrower->xLoc;
          y = thrower->yLoc;
        }
      }
  ...
  {
    if (theItem->kind == POTION_CONFUSION || theItem->kind == POTION_POISON || theItem->kind == POTION_PARALYSIS ||
        theItem->kind == POTION_INCINERATION || theItem->kind == POTION_DARKNESS || theItem->kind == POTION_LICHEN ||
    ...
    else
    {
      ...
      sprintf(buf, "the flask shatters and %s liquid splashes harmlessly %s %s.", potionTable[theItem->kind].flavor,
              buf2, tileText(x, y));
      message(buf, false);
...
void throwCommand(Item* theItem)
{
  ...
  unsigned char command[10];
  int maxDistance, zapTarget[2], originLoc[2], quantity;
  bool autoTarget;
  ...
  if (((theItem->flags & ITEM_EQUIPPED) || theItem->timesEnchanted > 0) && theItem->quantity <= 1)
  {
  ...
  sprintf(buf, "Throw %s %s where? (<hjklyubn>, mouse, or <tab>)", (theItem->quantity > 1 ? "a" : "your"), theName);
  ...
  if (chooseTarget(zapTarget, maxDistance, true, autoTarget, false, false, &red))
  {
    ...
    *thrownItem = *theItem;  // clone the item
    ...
    itemName(thrownItem, theName, false, false, nullptr);
    originLoc[0] = player.xLoc;
    originLoc[1] = player.yLoc;
  ...
  if (newLabel >= 'a' && newLabel <= 'z')
  {
    if (newLabel != theItem->inventoryLetter)
    {
      ...
      if (oldItem)
      {
        ...
        itemName(oldItem, theName, true, true, nullptr);
    ...
    {
      itemName(theItem, theName, true, true, nullptr);
      sprintf(buf, "%s %s already labeled (%c).", theName, theItem->quantity == 1 ? "is" : "are",
...
bool playerCancelsBlinking(const int originLoc[2], const int targetLoc[2], const int maxDistance)
{
  int coordinates[DCOLS][2], impactLoc[2];
  int numCells, i, x, y;
  ...
  if (player.status[STATUS_IMMUNE_TO_FIRE] || player.status[STATUS_LEVITATING])
  {
    return false;
  }

  getImpactLoc(impactLoc, originLoc, targetLoc, maxDistance > 0 ? maxDistance : DCOLS, true);
  getLocationFlags(impactLoc[0], impactLoc[1], &tFlags, nullptr, nullptr, true);
  if (maxDistance > 0)
  {
    if ((pmap[impactLoc[0]][impactLoc[1]].flags & DISCOVERED) && (tFlags & T_LAVA_INSTA_DEATH) &&
        !(tFlags & T_ENTANGLES))
    {
  ...
  else
  {
    ...
    for (i = 0; i < numCells; i++)
    {
      ...
      {
        getLocationFlags(x, y, &tFlags, nullptr, nullptr, true);
        if ((tFlags & T_LAVA_INSTA_DEATH) && !(tFlags & T_ENTANGLES))
        {
          possibleDeath = true;
        }
  ...
  if (possibleDeath && !confirm("Blink across lava with unknown range?", false))
  {
    return true;
  }
  return false;
...
bool useStaffOrWand(Item* theItem, bool* commandsRecorded)
{
  char buf[COLS], buf2[COLS];
  unsigned char command[10];
  int zapTarget[2], originLoc[2], maxDistance, c;
  ...
  printString(buf, mapToWindowX(0), 1, &itemMessageColor, &black, nullptr);

  theBolt = boltCatalog[tableForItemCategory(theItem->category, nullptr)[theItem->kind].strengthRequired];
  if (theItem->category == STAFF)
  {
  ...
  else if (theBolt.backColor == nullptr)
  {
  ...
  originLoc[0] = player.xLoc;
  originLoc[1] = player.yLoc;
  confirmedTarget = chooseTarget(zapTarget, maxDistance, false, autoTarget, targetAllies, passThroughCreatures,
                                 &trajectoryHiliteColor);
  if (confirmedTarget && boltKnown && theBolt.boltEffect == BE_BLINKING &&
  ...
  if (confirmedTarget)
  {
    ...
    {
      autoID = zap(originLoc, zapTarget, &theBolt,
      ...
      if (autoID)
      {
        if (!tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified)
        {
          ...
          identifyItemKind(theItem);
          itemName(theItem, buf2, false, true, nullptr);
          strcat(buf, buf2);
          strcat(buf, ".)");
          messageWithColor(buf, &itemMessageColor, false);
        }
...
{
  int x = player.xLoc, y = player.yLoc;
  Creature* monst;
  ...
  monst->creatureState = MONSTER_ALLY;
  monst->ticksUntilTurn = monst->info.attackSpeed + 1;  // So they don't move before the player's next turn.
  monst->status[STATUS_LIFESPAN_REMAINING] = monst->maxStatus[STATUS_LIFESPAN_REMAINING] =
...
void useCharm(Item* theItem)
{
  ...
  switch (theItem->kind)
  {
    ...
    case CHARM_HASTE:
      ...
      break;
    case CHARM_FIRE_IMMUNITY:
      player.status[STATUS_IMMUNE_TO_FIRE] = player.maxStatus[STATUS_IMMUNE_TO_FIRE] =
          charmEffectDuration(theItem->kind, theItem->enchant1);
    ...
    case CHARM_TELEPATHY:
      ...
      break;
    case CHARM_LEVITATION:
      player.status[STATUS_LEVITATING] = player.maxStatus[STATUS_LEVITATING] =
          charmEffectDuration(theItem->kind, theItem->enchant1);
      player.bookkeepingFlags &= ~MB_SEIZED;  // break free of holding monsters
      message("you float into the air!", false);
      break;
    case CHARM_SHATTERING:
      messageWithColor("your charm emits a wave of turquoise light that pierces the nearby walls!", &itemMessageColor,
                       false);
    ...
    case CHARM_RECHARGING:
      ...
      break;
    case CHARM_NEGATION:
      negationBlast("your charm", charmNegationRadius(theItem->enchant1) + 1);  // Add 1 because otherwise radius 1
                                                                                // would affect only the player.
      break;
    default:
      break;
  }
...
void apply(Item* theItem, bool recordCommands)
{
  ...
  {
    theItem = promptForItemOfType(
        (SCROLL | FOOD | POTION | STAFF | WAND | CHARM), 0, 0,
        KEYBOARD_LABELS ? "Apply what? (a-z, shift for more info; or <esc> to cancel)" : "Apply what?", true);
  }
       ...
       tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified))
  {
    if (tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified)
    {
      sprintf(buf, "Really %s a %s of %s?", theItem->category == SCROLL ? "read" : "drink",
              theItem->category == SCROLL ? "scroll" : "potion",
              tableForItemCategory(theItem->category, nullptr)[theItem->kind].name);
    }
    else
    {
      sprintf(buf, "Really %s a cursed %s?", theItem->category == SCROLL ? "read" : "drink",
              theItem->category == SCROLL ? "scroll" : "potion");
    }
    if (!confirm(buf, false))
    {
  ...
  {
    case FOOD:
      if (STOMACH_SIZE - player.status[STATUS_NUTRITION] < foodTable[theItem->kind].strengthRequired)
      {  // Not hungry enough.
      ...
      player.status[STATUS_NUTRITION] =
          min(foodTable[theItem->kind].strengthRequired + player.status[STATUS_NUTRITION], STOMACH_SIZE);
    ...
    case SCROLL:
      ...
      if (!scrollTable[theItem->kind].identified && theItem->kind != SCROLL_ENCHANTING &&
  ...
  if (!commandsRecorded)
  {  // to make sure we didn't already record the keystrokes above with staff/wand targeting
    command[c] = '\0';
    recordKeystrokeSequence(command);
    commandsRecorded = true;
  }
...
enum MonsterTypes chooseVorpalEnemy() {
  int i, index, possCount = 0, deepestLevel = 0, deepestHorde, chosenHorde, failsafe = 25;
...
int lotteryDraw(int* frequencies, int itemCount)
{
  int i, maxFreq, randIndex;
  maxFreq = 0;
...
{
  int i, frequencies[MONSTER_CLASS_COUNT];
  for (i = 0; i < MONSTER_CLASS_COUNT; i++)
  {
    if (MonsterClassCatalog[i].maxDepth <= 0 || rogue.depthLevel <= MonsterClassCatalog[i].maxDepth)
    {
...
void updateIdentifiableItem(Item* theItem)
{
  ...
  else if ((theItem->category & (RING | STAFF | WAND)) && (theItem->flags & ITEM_IDENTIFIED) &&
           tableForItemCategory(theItem->category, nullptr)[theItem->kind].identified)
  {
    theItem->flags &= ~ITEM_CAN_BE_IDENTIFIED;
  }
  else if ((theItem->category & (WEAPON | ARMOR)) && (theItem->flags & ITEM_IDENTIFIED) &&
...
void magicMapCell(int x, int y)
{
  pmap[x][y].flags |= MAGIC_MAPPED;
  pmap[x][y].rememberedTerrainFlags =
      tileCatalog[pmap[x][y].layers[DUNGEON]].flags | tileCatalog[pmap[x][y].layers[LIQUID]].flags;
  pmap[x][y].rememberedTMFlags =
      tileCatalog[pmap[x][y].layers[DUNGEON]].mechFlags | tileCatalog[pmap[x][y].layers[LIQUID]].mechFlags;
  if (pmap[x][y].layers[LIQUID] &&
      tileCatalog[pmap[x][y].layers[LIQUID]].drawPriority < tileCatalog[pmap[x][y].layers[DUNGEON]].drawPriority)
  {
...
{
  int i, j, x, y, numberOfMonsters = 0;
  Item* tempItem;
  Creature* monst;
  ...
  switch (theItem->kind)
  {
    case SCROLL_IDENTIFY:
      ...
      } while (theItem == nullptr || !(theItem->flags & ITEM_CAN_BE_IDENTIFIED));
      recordKeystroke(theItem->inventoryLetter, false, false);
      confirmMessages();
      identify(theItem);
      itemName(theItem, buf, true, true, nullptr);
    ...
    case SCROLL_ENCHANTING:
      ...
      {
        theItem =
            promptForItemOfType((WEAPON | ARMOR | RING | STAFF | WAND | CHARM), 0, 0,
                                KEYBOARD_LABELS ? "Enchant what? (a-z; shift for more info)" : "Enchant what?", false);
        confirmMessages();
        if (theItem == nullptr || !(theItem->category & (WEAPON | ARMOR | RING | STAFF | WAND | CHARM)))
        {
      ...
      } while (theItem == nullptr || !(theItem->category & (WEAPON | ARMOR | RING | STAFF | WAND | CHARM)));
      recordKeystroke(theItem->inventoryLetter, false, false);
      confirmMessages();
      switch (theItem->category)
      {
        ...
        case CHARM:
          theItem->enchant1++;
          theItem->charges = min(
              0, theItem->charges);  // Enchanting instantly recharges charms.
    ...
    case SCROLL_PROTECT_ARMOR:
      if (rogue.armor)
      {
        ...
        itemName(tempItem, buf2, false, false, nullptr);
    ...
    case SCROLL_SANCTUARY:
      ...
      messageWithColor("sprays of color arc to the ground, forming glyphs where they alight.", &itemMessageColor,
                       false);
      break;
    ...
    case SCROLL_SUMMON_MONSTER:
      for (j = 0; j < 25 && numberOfMonsters < 3; j++)
      {
        for (i = 0; i < 8; i++)
        {
          ...
          {
            monst = spawnHorde(
                0, x, y, (HORDE_LEADER_CAPTIVE | HORDE_NO_PERIODIC_SPAWN | HORDE_IS_SUMMONED | HORDE_MACHINE_ONLY), 0);
            if (monst)
            {
              // refreshDungeonCell(x, y);
              // monst->creatureState = MONSTER_TRACKING_SCENT;
    ...
    case SCROLL_SHATTERING:
      messageWithColor("the scroll emits a wave of turquoise light that pierces the nearby walls!", &itemMessageColor,
                       false);
      crystalize(9);
      break;
    case SCROLL_DISCORD:
...
void drinkPotion(Item* theItem)
{
  ...
  switch (theItem->kind)
  {
    ...
    case POTION_DETECT_MAGIC:
      hadEffect = false;
      hadEffect2 = false;
      for (tempItem = floorItems->nextItem; tempItem != nullptr; tempItem = tempItem->nextItem)
      {
...
int magicCharDiscoverySuffix(int category, int kind)
{
  ...
  switch (category)
  {
    ...
    case WAND:
    case STAFF:
      if (boltCatalog[tableForItemCategory(category, nullptr)[kind].strengthRequired].flags & (BF_TARGET_ALLIES))
      {
        result = -1;
      }
      else
...
uchar itemMagicChar(Item* theItem)
{
  switch (theItem->category)
  {
    ...
    case STAFF:
      if (boltCatalog[tableForItemCategory(theItem->category, nullptr)[theItem->kind].strengthRequired].flags &
          (BF_TARGET_ALLIES))
      {
        return BAD_MAGIC_CHAR;
      }
...
void unequip(Item* theItem)
{
  ...
  {
    theItem = promptForItemOfType(
        ALL_ITEMS, ITEM_EQUIPPED, 0,
        KEYBOARD_LABELS ? "Remove (unequip) what? (a-z or <esc> to cancel)" : "Remove (unequip) what?", true);
  }
  if (theItem == nullptr)
  {
    return;
  ...
  {
    itemName(theItem, buf2, false, false, nullptr);
    sprintf(buf, "your %s %s not equipped.", buf2, theItem->quantity == 1 ? "was" : "were");
    confirmMessages();
    messageWithColor(buf, &itemMessageColor, false);
    return;
  }
  else if (theItem->flags & ITEM_CURSED)
  {  // this is where the item gets unequipped
    itemName(theItem, buf2, false, false, nullptr);
    sprintf(buf, "you can't; your %s appear%s to be cursed.", buf2, theItem->quantity == 1 ? "s" : "");
    confirmMessages();
    messageWithColor(buf, &itemMessageColor, false);
    return;
  }
...
void drop(Item* theItem)
{
  ...
  {
    theItem = promptForItemOfType(
        ALL_ITEMS, 0, 0, KEYBOARD_LABELS ? "Drop what? (a-z, shift for more info; or <esc> to cancel)" : "Drop what?",
        true);
  }
  if (theItem == nullptr)
  {
...
Item* itemAtLoc(int x, int y)
{
  ...
  if (!(pmap[x][y].flags & HAS_ITEM))
  {
    return nullptr;  // easy optimization
  }
  for (theItem = floorItems->nextItem; theItem != nullptr && (theItem->xLoc != x || theItem->yLoc != y);
...
Item* dropItem(Item* theItem)
{
  ...
  if (theItem->quantity > 1 && !(theItem->category & WEAPON))
  {  // peel off the top item and drop it
...
void equipItem(Item* theItem, bool force)
{
  ...
  else if (theItem->category & RING)
  {
    ...
    else if (theItem->kind == RING_LIGHT || theItem->kind == RING_STEALTH)
    {
      identifyItemKind(theItem);
    }
  }
...
{
  if (theItem == nullptr || !(theItem->flags & ITEM_EQUIPPED))
  {
    return;
  }
...
void updateRingBonuses()
{
  ...
  rogue.clairvoyance = rogue.stealthBonus = rogue.transference = rogue.awarenessBonus = rogue.regenerationBonus =
      rogue.wisdomBonus = rogue.reaping = 0;
...
void updatePlayerRegenerationDelay()
{
  ...
  // DEBUG printf("\nTurnsForFull: %i; regenPerTurn: %i; (thousandths of) turnsBetweenRegen: %i", turnsForFull,
  // player.regenPerTurn, player.info.turnsBetweenRegen);
}
...
bool removeItemFromChain(Item* theItem, Item* theChain)
{
  ...
  for (previousItem = theChain; previousItem->nextItem; previousItem = previousItem->nextItem)
  {
...
{
  const int weaponRunicIntercepts[] = {
    700,    // W_SPEED,
    1000,   // W_QUIETUS,
    900,    // W_PARALYSIS,
    800,    // W_MULTIPLICITY,
  ...
  const int armorRunicIntercepts[] = {
    900,    // A_MULTIPLICITY,
    700,    // A_MUTUALITY,
    900,    // A_ABSORPTION,
    900,    // A_REPRISAL,
  ...
  switch (theItem->category)
  {
    ...
    case POTION:
      ...
      break;
    case STAFF:
      return staffTable[theItem->kind].marketValue * theItem->quantity * (float)(1 + 0.15 * (theItem->enchant1 - 1));
      break;
    case WAND:
      return wandTable[theItem->kind].marketValue * theItem->quantity;
      break;
    case RING:
      return ringTable[theItem->kind].marketValue * theItem->quantity * (float)(1 + 0.15 * (theItem->enchant1 - 1));
      break;
    case CHARM:
      return charmTable[theItem->kind].marketValue * theItem->quantity * (float)(1 + 0.15 * (theItem->enchant1 - 1));
      break;
    case AMULET:
      return 35000;
      break;
