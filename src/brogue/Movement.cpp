...
void playerRuns(int direction)
{
  ...
  while (!rogue.disturbed)
  {
    ...
    if (!coordinatesAreInMap(newX, newY) || monsterAvoids(&player, newX, newY))
    {
      rogue.disturbed = true;
    }
    ...
    else if (direction < 4)
    {
      for (dir = 0; dir < 4; dir++)
      {
        ...
        if (cardinalPassability[dir] != monsterAvoids(&player, newX, newY) &&
            !(nbDirs[dir][0] + nbDirs[direction][0] == 0 && nbDirs[dir][1] + nbDirs[direction][1] == 0))
        {
...
void describedItemBasedOnParameters(int theCategory, int theKind, int theQuantity, char* buf)
{
  ...
  itemName(tempItem, buf, false, true, nullptr);
  free(tempItem);
  return;
}
...
void describedItemName(Item* theItem, char* buf)
{
  ...
  {
    itemName(theItem, buf, (theItem->category & (WEAPON | ARMOR) ? false : true), true, nullptr);
  }
  else
  {
...
void describeLocation(char* buf, int x, int y)
{
  Creature* monst;
  ...
  monst = nullptr;
  standsInTerrain =
      ((tileCatalog[pmap[x][y].layers[highestPriorityLayer(x, y, false)]].mechFlags & TM_STAND_IN_TILE) ? true : false);
  ...
  magicItem = nullptr;
  if (theItem && !playerCanSeeOrSense(x, y) && (theItem->flags & ITEM_MAGIC_DETECTED) && itemMagicChar(theItem))
  {
    magicItem = theItem;
  }
  else if (monst && !canSeeMonster(monst) && monst->carriedItem && (monst->carriedItem->flags & ITEM_MAGIC_DETECTED) &&
  ...
  {
    strcpy(adjective, (((!player.status[STATUS_HALLUCINATING] || rogue.playbackOmniscience) &&
                        monst->info.displayChar >= 'a' && monst->info.displayChar <= 'z') ||
                               (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience && rand_range(0, 1)) ?
                           "small" :
                           "large"));
  ...
  if (!playerCanSeeOrSense(x, y))
  {
      ...
      if (pmap[x][y].rememberedItemCategory)
      {
        ...
        else
        {
          describedItemBasedOnParameters(pmap[x][y].rememberedItemCategory, pmap[x][y].rememberedItemKind,
                                         pmap[x][y].rememberedItemQuantity, object);
        }
      }
      else
      {
    ...
    else if (pmap[x][y].flags & MAGIC_MAPPED)
    {  // magic mapped
  ...
  if (monst)
  {
    ...
    if (pmap[x][y].layers[GAS] && monst->status[STATUS_INVISIBLE])
    {  // phantoms in gas
      sprintf(buf, "you can perceive the faint outline of %s in %s.", subject,
              tileCatalog[pmap[x][y].layers[GAS]].description);
      restoreRNG;
      return;
    }

    subjectMoving =
        (monst->turnsSpentStationary == 0 && !(monst->info.flags & (MONST_GETS_TURN_ON_ACTIVATION | MONST_IMMOBILE)) &&
         monst->creatureState != MONSTER_SLEEPING && !(monst->bookkeepingFlags & (MB_SEIZED | MB_CAPTIVE)));
    if ((monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) && cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY))
    {
    ...
    else
    {
      ...
      {
        strcpy(preposition, subjectMoving ? (standsInTerrain ? "through" : "across") : (standsInTerrain ? "in" : "on"));
      }
  ...
  {  // no monster
    ...
    if (theItem)
    {
      ...
      strcpy(preposition, standsInTerrain ? (subjectMoving ? "through" : "in") : (subjectMoving ? "across" : "on"));
    }
    else
    {  // no item
...
void useKeyAt(Item* theItem, int x, int y)
{
  ...
  if (disposable)
  {
    ...
    {
      itemName(theItem, buf2, true, false, nullptr);
      sprintf(buf, "you use your %s %s %s.", buf2, preposition, terrainName);
...
{
  int i, newX, newY, validDirectionCount = 0, randIndex;
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) &&
        ...
        (!respectAvoidancePreferences || (!monsterAvoids(monst, newX, newY)) ||
...
void vomit(Creature* monst)
{
  ...
  if (canDirectlySeeMonster(monst) && !rogue.automationActive)
  {
...
void moveEntrancedMonsters(enum Directions dir)
{
  ...
  for (monst = monsters->nextCreature; monst != nullptr; monst = nextMonst)
  {
    nextMonst = monst->nextCreature;
    if (monst->status[STATUS_ENTRANCED] && !monst->status[STATUS_STUCK] && !monst->status[STATUS_PARALYZED] &&
...
bool freeCaptivesEmbeddedAt(int x, int y)
{
  ...
  if (pmap[x][y].flags & HAS_MONSTER)
  {
    ...
    monst = monsterAtLoc(x, y);
    if ((monst->bookkeepingFlags & MB_CAPTIVE) && !(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) &&
...
bool abortAttackAgainstAcidicTarget(Creature* hitList[8])
{
  ...
  if (rogue.weapon && !(rogue.weapon->flags & ITEM_PROTECTED) && !player.status[STATUS_HALLUCINATING] &&
      !player.status[STATUS_CONFUSED])
  {
    for (i = 0; i < 8; i++)
    {
      if (hitList[i] && (hitList[i]->info.flags & MONST_DEFEND_DEGRADE_WEAPON) && canSeeMonster(hitList[i]) &&
          (!(rogue.weapon->flags & ITEM_RUNIC) || !(rogue.weapon->flags & ITEM_RUNIC_IDENTIFIED) ||
           rogue.weapon->enchant2 != W_SLAYING || !monsterIsInClass(hitList[i], rogue.weapon->vorpalEnemy)))
      {
        monsterName(monstName, hitList[i], true);
        itemName(rogue.weapon, weaponName, false, false, nullptr);
...
bool handleWhipAttacks(Creature* attacker, enum Directions dir, bool* aborted)
{
  ...
  int strikeLoc[2], originLoc[2], targetLoc[2];
  ...
  if (defender && (attacker != &player || canSeeMonster(defender)) && !monsterIsHidden(defender, attacker) &&
...
void buildFlailHitList(const int x, const int y, const int newX, const int newY, Creature* hitList[16])
{
  Creature* monst;
  int mx, my;
  int i = 0;
  ...
  for (monst = monsters->nextCreature; monst != nullptr; monst = monst->nextCreature)
  {
    mx = monst->xLoc;
    my = monst->yLoc;
    if (distanceBetween(x, y, mx, my) == 1 && distanceBetween(newX, newY, mx, my) == 1 && canSeeMonster(monst) &&
        monstersAreEnemies(&player, monst) && monst->creatureState != MONSTER_ALLY &&
...
bool diagonalBlocked(const int x1, const int y1, const int x2, const int y2,
                        const bool limitToPlayerKnowledge)
{
  ...
  getLocationFlags(x1, y2, &tFlags, nullptr, nullptr, limitToPlayerKnowledge);
  if (tFlags & T_OBSTRUCTS_DIAGONAL_MOVEMENT)
  {
    return true;
  }
  getLocationFlags(x2, y1, &tFlags, nullptr, nullptr, limitToPlayerKnowledge);
  if (tFlags & T_OBSTRUCTS_DIAGONAL_MOVEMENT)
  {
    return true;
  }
...
{
  int initialDirection = direction, i, layer;
  int x = player.xLoc, y = player.yLoc;
  int newX, newY, newestX, newestY;
  bool playerMoved = false, alreadyRecorded = false, whipAttackAborted = false, anyAttackHit = false;
  Creature *defender = nullptr, *tempMonst = nullptr, *hitList[16] = { nullptr };
  char monstName[COLS];
  char buf[COLS * 3];
  const uchar directionKeys[8] = { UP_KEY,     DOWN_KEY,     LEFT_KEY,    RIGHT_KEY,
  ...
  if (player.status[STATUS_CONFUSED])
  {
    ...
    if (player.status[STATUS_LEVITATING] <= 1 && player.status[STATUS_IMMUNE_TO_FIRE] <= 1)
    {
      for (i = 0; i < 8; i++)
      {
        newestX = x + nbDirs[i][0];
        newestY = y + nbDirs[i][1];
        if (coordinatesAreInMap(newestX, newestY) && (pmap[newestX][newestY].flags & (DISCOVERED | MAGIC_MAPPED)) &&
            ...
            !((pmap[newestX][newestY].flags & HAS_MONSTER) && canSeeMonster(monsterAtLoc(newestX, newestY)) &&
  ...
  if (!defender || (!canSeeMonster(defender) && !monsterRevealed(defender)) || !monstersAreEnemies(&player, defender))
  {
    if (cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) &&
        cellHasTMFlag(newX, newY, TM_PROMOTES_ON_PLAYER_ENTRY))
    {
  ...
  if (((!cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) ||
        (cellHasTMFlag(newX, newY, TM_PROMOTES_WITH_KEY) && keyInPackFor(newX, newY))) &&
       !diagonalBlocked(x, y, newX, newY, false) &&
       (!cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY) ||
        (cellHasTMFlag(x, y, TM_PROMOTES_WITH_KEY) && keyInPackFor(x, y)))) ||
      (defender && defender->info.flags & MONST_ATTACKABLE_THRU_WALLS))
  {
    ...
    if (defender)
    {
      ...
      if (defender->creatureState != MONSTER_ALLY)
      {
        ...
        buildHitList(hitList, &player, defender, rogue.weapon && (rogue.weapon->flags & ITEM_ATTACKS_PENETRATE),
                     rogue.weapon && (rogue.weapon->flags & ITEM_ATTACKS_ALL_ADJACENT));

        if (abortAttackAgainstAcidicTarget(hitList))
        {  // Acid mound attack confirmation.
        ...
        for (i = 0; i < 16; i++)
        {
          if (hitList[i] && monsterWillAttackTarget(&player, hitList[i]) &&
    ...
    else if (whipAttackAborted)
    {  // Canceled an attack against an acid mound.
      brogueAssert(!alreadyRecorded);
      rogue.disturbed = true;
      return false;
    }
    ...
    {
      for (tempMonst = monsters->nextCreature; tempMonst != nullptr; tempMonst = tempMonst->nextCreature)
      {
        if ((tempMonst->bookkeepingFlags & MB_SEIZING) && monstersAreEnemies(&player, tempMonst) &&
    ...
    if (pmap[newX][newY].flags & (DISCOVERED | MAGIC_MAPPED) && player.status[STATUS_LEVITATING] <= 1 &&
        !player.status[STATUS_CONFUSED] && cellHasTerrainFlag(newX, newY, T_LAVA_INSTA_DEATH) &&
        player.status[STATUS_IMMUNE_TO_FIRE] <= 1 && !cellHasTerrainFlag(newX, newY, T_ENTANGLES) &&
    ...
    else if (pmap[newX][newY].flags & (DISCOVERED | MAGIC_MAPPED) && player.status[STATUS_LEVITATING] <= 1 &&
             !player.status[STATUS_CONFUSED] && cellHasTerrainFlag(newX, newY, T_AUTO_DESCENT) &&
             !cellHasTerrainFlag(newX, newY, T_ENTANGLES) && !cellHasTMFlag(newX, newY, TM_IS_SECRET) &&
             !confirm("Dive into the depths?", false))
    {
      return false;
    }
    else if (playerCanSee(newX, newY) && !player.status[STATUS_CONFUSED] && !player.status[STATUS_BURNING] &&
             player.status[STATUS_IMMUNE_TO_FIRE] <= 1 && cellHasTerrainFlag(newX, newY, T_IS_FIRE) &&
             !cellHasTMFlag(newX, newY, TM_EXTINGUISHES_FIRE) && !confirm("Venture into flame?", false))
    {
      return false;
    }
    else if (pmap[newX][newY].flags & (ANY_KIND_OF_VISIBLE | MAGIC_MAPPED) && player.status[STATUS_LEVITATING] <= 1 &&
             !player.status[STATUS_CONFUSED] && cellHasTerrainFlag(newX, newY, T_IS_DF_TRAP) &&
             !(pmap[newX][newY].flags & PRESSURE_PLATE_DEPRESSED) && !cellHasTMFlag(newX, newY, TM_IS_SECRET) &&
    ...
    if (rogue.weapon && (rogue.weapon->flags & ITEM_LUNGE_ATTACKS))
    {
      ...
      if (coordinatesAreInMap(newestX, newestY) && (pmap[newestX][newestY].flags & HAS_MONSTER))
      {
        ...
        if (tempMonst && canSeeMonster(tempMonst) && monstersAreEnemies(&player, tempMonst) &&
            tempMonst->creatureState != MONSTER_ALLY && !(tempMonst->bookkeepingFlags & MB_IS_DYING) &&
            (!cellHasTerrainFlag(tempMonst->xLoc, tempMonst->yLoc, T_OBSTRUCTS_PASSABILITY) ||
             (tempMonst->info.flags & MONST_ATTACKABLE_THRU_WALLS)))
        {
          hitList[0] = tempMonst;
          if (abortAttackAgainstAcidicTarget(hitList))
          {  // Acid mound attack confirmation.
    ...
    else
    {
      ...
      if (defender && defender->creatureState == MONSTER_ALLY)
      {  // Swap places with ally.
        ...
        {
          getQualifyingPathLocNear(&(defender->xLoc), &(defender->yLoc), player.xLoc, player.yLoc, true,
                                   forbiddenFlagsForMonster(&(defender->info)), 0, 0,
                                   (HAS_PLAYER | HAS_MONSTER | HAS_STAIRS), false);
        }
        // getQualifyingLocNear(loc, player.xLoc, player.yLoc, true, NULL, forbiddenFlagsForMonster(&(defender->info)) &
        // ~(T_IS_DF_TRAP | T_IS_DEEP_WATER | T_SPONTANEOUSLY_IGNITES), HAS_MONSTER, false, false);
        // defender->xLoc = loc[0];
        // defender->yLoc = loc[1];
        pmap[defender->xLoc][defender->yLoc].flags |= HAS_MONSTER;
      }
...
bool updateDistanceCell(int **distanceMap, int x, int y) {
  int dir, newX, newY;
...
void dijkstraScan(int **distanceMap, char passMap[DCOLS][DROWS], bool allowDiagonals) {
  int i, j, maxDir;
  enum directions dir;
...
/*void enqueue(int x, int y, int val, distanceQueue *dQ) {
  int *qX2, *qY2, *qVal2;
...
void dequeue(int *x, int *y, int *val, distanceQueue *dQ) {
  int i, minIndex;
...
void dijkstraScan(int **distanceMap, char passMap[DCOLS][DROWS], bool allowDiagonals) {
  ...
  dQ.qX = (int *) malloc(dQ.qMaxLen * sizeof(short));
  dQ.qY = (int *) malloc(dQ.qMaxLen * sizeof(short));
  dQ.qVal = (int *) malloc(dQ.qMaxLen * sizeof(short));
  dQ.qLen = 0;
  dQ.qMinVal = 30000;
  dQ.qMinCount = 0;
...
creature *traveler) { int i, j; bool somethingChanged;
...
int nextStep(int** distanceMap, int x, int y, Creature* monst, bool preferDiagonals)
{
  int newX, newY, bestScore;
  ...
  for (dir = (preferDiagonals ? 7 : 0); (preferDiagonals ? dir >= 0 : dir < DIRECTION_COUNT);
       (preferDiagonals ? dir-- : dir++))
  {
    ...
    if (coordinatesAreInMap(newX, newY))
    {
      ...
      if (monst && monsterAvoids(monst, newX, newY))
      {
        blocked = true;
      }
      else if (monst && blocker && !canPass(monst, blocker) && !monstersAreTeammates(monst, blocker) &&
               !monstersAreEnemies(monst, blocker))
      {
        blocked = true;
      }
      if ((distanceMap[x][y] - distanceMap[newX][newY]) > bestScore &&
          !diagonalBlocked(x, y, newX, newY, monst == &player) && knownToPlayerAsPassableOrSecretDoor(newX, newY) &&
          !blocked)
      {
        bestDir = dir;
...
void displayRoute(int** distanceMap, bool removeRoute)
{
  int currentX = player.xLoc, currentY = player.yLoc, dir, newX, newY;
  bool advanced;
  ...
  do
  {
    ...
    for (dir = 7; dir >= 0; dir--)
    {
      ...
      if (coordinatesAreInMap(newX, newY) && distanceMap[newX][newY] >= 0 &&
...
void travelRoute(int path[1000][2], int steps)
{
  ...
  for (i = 0; i < steps && !rogue.disturbed; i++)
  {
    ...
    {
      if (player.xLoc + nbDirs[dir][0] == path[i][0] && player.yLoc + nbDirs[dir][1] == path[i][1])
      {
        if (!playerMoves(dir))
        {
          rogue.disturbed = true;
...
void travelMap(int** distanceMap)
{
  int currentX = player.xLoc, currentY = player.yLoc, dir, newX, newY;
  bool advanced;
...
void travel(int x, int y, bool autoConfirm)
{
  int **distanceMap, i;
  RogueEvent theEvent;
  unsigned int staircaseConfirmKey;
  ...
  if (distanceMap[player.xLoc][player.yLoc] < 30000)
  {
    ...
    else
    {
      ...
      if ((theEvent.eventType == MOUSE_UP && windowToMapX(theEvent.param1) == x &&
           windowToMapY(theEvent.param2) == y) ||
          (theEvent.eventType == KEYSTROKE &&
           (theEvent.param1 == 'Y' || theEvent.param1 == 'y' || theEvent.param1 == RETURN_KEY ||
            theEvent.param1 == ENTER_KEY || (theEvent.param1 == staircaseConfirmKey && theEvent.param1 != 0))))
      {
        travelMap(distanceMap);
        // refreshSideBar(-1, -1, false);
        commitDraws();
...
{
  if (limitToPlayerKnowledge && (pmap[x][y].flags & (DISCOVERED | MAGIC_MAPPED)) && !playerCanSee(x, y))
  {
    if (tFlags)
    {
...
void populateCreatureCostMap(int** costMap, Creature* monst)
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      ...
      getLocationFlags(i, j, &tFlags, nullptr, &cFlags, monst == &player);
      ...
      if ((tFlags & T_LAVA_INSTA_DEATH) &&
          ...
          max(monst->status[STATUS_LEVITATING], monst->status[STATUS_IMMUNE_TO_FIRE]) <
              (rogue.mapToShore[i][j] + distanceBetween(i, j, monst->xLoc, monst->yLoc) * monst->movementSpeed / 100))
      {
      ...
      if (((tFlags & T_AUTO_DESCENT) || (tFlags & T_IS_DEEP_WATER) && !(monst->info.flags & MONST_IMMUNE_TO_WATER)) &&
          !(monst->info.flags & MONST_FLIES) && (monst->status[STATUS_LEVITATING]) &&
          monst->status[STATUS_LEVITATING] <
              (rogue.mapToShore[i][j] + distanceBetween(i, j, monst->xLoc, monst->yLoc) * monst->movementSpeed / 100))
      {
        // Only a temporary effect will permit the monster to levitate over the chasm/water, and the remaining duration
        // either isn't enough to get it to the spot, or it won't suffice to let it return to shore if it does get
        // there. Treat these locations as obstacles.
        costMap[i][j] = PDS_FORBIDDEN;
        continue;
      }
      ...
      if (cFlags & HAS_MONSTER)
      {
        ...
        if (currentTenant && (currentTenant->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)) &&
      ...
      if ((cFlags & KNOWN_TO_BE_TRAP_FREE) || (monst != &player && monst->creatureState != MONSTER_ALLY))
      {
        costMap[i][j] = 10;
      }
      else
      ...
      {
        if ((tFlags & T_CAUSES_NAUSEA) || cellHasTMFlag(i, j, TM_PROMOTES_ON_ITEM_PICKUP) ||
...
enum Directions adjacentFightingDir()
{
  ...
  for (dir = 0; dir < DIRECTION_COUNT; dir++)
  {
    ...
    if (monst && canSeeMonster(monst) &&
        (!diagonalBlocked(player.xLoc, player.yLoc, newX, newY, false) ||
         (monst->info.flags & MONST_ATTACKABLE_THRU_WALLS)) &&
        monstersAreEnemies(&player, monst) && !(monst->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)))
    {
      return dir;
    }
  }
    ...
    for (j = 0; j < DROWS; j++)
    {
      ...
      if (!(pmap[i][j].flags & DISCOVERED))
      {
        if ((pmap[i][j].flags & MAGIC_MAPPED) &&
            (tileCatalog[pmap[i][j].layers[DUNGEON]].flags | tileCatalog[pmap[i][j].layers[LIQUID]].flags) &
                T_PATHING_BLOCKER)
        {
      ...
      else if (theItem && !monsterAvoids(&player, i, j))
      {
...
bool explore(int frameDelay)
{
  ...
  if (dir != NO_DIRECTION && startFighting(dir, (player.status[STATUS_HALLUCINATING] ? true : false)))
  {
    return true;
  }
  ...
  {
    message(KEYBOARD_LABELS ? "Exploring... press any key to stop." : "Exploring... touch anywhere to stop.", false);
    ...
    printString(KEYBOARD_LABELS ? "Exploring... press any key to stop." : "Exploring... touch anywhere to stop.",
                mapToWindowX(0), mapToWindowY(-1), &white, &black, nullptr);
  }
  rogue.disturbed = false;
  rogue.automationActive = true;
  ...
  do
  {
    ...
    // take a step
    dir = nextStep(distanceMap, player.xLoc, player.yLoc, nullptr, false);
...
int directionOfKeypress(unsigned int ch)
{
  switch (ch)
  {
    case LEFT_KEY:
...
bool isDisturbed(int x, int y)
{
  ...
  for (i = 0; i < DIRECTION_COUNT; i++)
  {
    ...
    if (monst && !(monst->creatureState == MONSTER_ALLY) && (canSeeMonster(monst) || monsterRevealed(monst)))
    {
      // Do not trigger for submerged or invisible or unseen monsters.
      return true;
    }
...
bool search(int searchStrength)
{
  ...
  for (i = x - radius; i <= x + radius; i++)
  {
    ...
    {
      if (coordinatesAreInMap(i, j) && playerCanDirectlySee(i, j) && cellHasTMFlag(i, j, TM_IS_SECRET))
      {
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (pmap[i][j].flags & IN_FIELD_OF_VIEW &&
          (max(0, tmap[i][j].light[0]) + max(0, tmap[i][j].light[1]) + max(0, tmap[i][j].light[2]) >
      ...
      {  // if the cell became visible this move
        if (!(pmap[i][j].flags & DISCOVERED) && rogue.automationActive)
        {
          if (pmap[i][j].flags & HAS_ITEM)
          {
            ...
            {
              itemName(theItem, name, false, true, nullptr);
          ...
          if (!(pmap[i][j].flags & MAGIC_MAPPED) && cellHasTMFlag(i, j, TM_INTERRUPT_EXPLORATION_WHEN_SEEN))
          {
            strcpy(
                name,
                tileCatalog[pmap[i][j].layers[layerWithTMFlag(i, j, TM_INTERRUPT_EXPLORATION_WHEN_SEEN)]].description);
            sprintf(buf, "you see %s.", name);
      ...
      else if (!(pmap[i][j].flags & VISIBLE) && (pmap[i][j].flags & WAS_VISIBLE))
      ...
      else if (!(pmap[i][j].flags & TELEPATHIC_VISIBLE) && (pmap[i][j].flags & WAS_TELEPATHIC_VISIBLE))
      ...
      else if (!(pmap[i][j].flags & WAS_TELEPATHIC_VISIBLE) && (pmap[i][j].flags & TELEPATHIC_VISIBLE))
      {  // became telepathically visible
        if (!(pmap[i][j].flags & DISCOVERED) && !cellHasTerrainFlag(i, j, T_PATHING_BLOCKER))
        {
          rogue.xpxpThisTurn++;
        }
        ...
        if (refreshDisplay)
        {
          refreshDungeonCell(i, j);
               ...
               (tmap[i][j].light[0] != tmap[i][j].oldLight[0] || tmap[i][j].light[1] != tmap[i][j].oldLight[1] ||
                tmap[i][j].light[2] != tmap[i][j].oldLight[2]))
      {  // if the cell's light color changed this move
      ...
      else if (updateDancingTerrain && playerCanSee(i, j) &&
               (!rogue.automationActive || !(rogue.playerTurnNumber % 5)) &&
               ((tileCatalog[pmap[i][j].layers[DUNGEON]].backColor) &&
                    tileCatalog[pmap[i][j].layers[DUNGEON]].backColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[DUNGEON]].foreColor) &&
                    tileCatalog[pmap[i][j].layers[DUNGEON]].foreColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[LIQUID]].backColor) &&
                    tileCatalog[pmap[i][j].layers[LIQUID]].backColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[LIQUID]].foreColor) &&
                    tileCatalog[pmap[i][j].layers[LIQUID]].foreColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[SURFACE]].backColor) &&
                    tileCatalog[pmap[i][j].layers[SURFACE]].backColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[SURFACE]].foreColor) &&
                    tileCatalog[pmap[i][j].layers[SURFACE]].foreColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[GAS]].backColor) &&
                    tileCatalog[pmap[i][j].layers[GAS]].backColor->colorDances ||
                (tileCatalog[pmap[i][j].layers[GAS]].foreColor) &&
                    tileCatalog[pmap[i][j].layers[GAS]].foreColor->colorDances ||
                ...
                unsigned long forbiddenFlags, bool cautiousOnWalls)
{
  ...
  for (i = 1; i <= 8; i++)
  {
    scanOctantFOV(grid, xLoc, yLoc, i, maxRadius, 1, LOS_SLOPE_GRANULARITY * -1, 0, forbiddenTerrain, forbiddenFlags,
                  cautiousOnWalls);
  }
}
...
void scanOctantFOV(char grid[DCOLS][DROWS], int xLoc, int yLoc, int octant, float maxRadius,
                   ...
                   unsigned long forbiddenFlags, bool cautiousOnWalls)
{
  if (columnsRightFromOrigin >= maxRadius)
    return;
  ...
  for (i = iStart; i <= iEnd; i++)
  {
    ...
    {  // next column slope starts here
      newStartSlope =
          (long int)((LOS_SLOPE_GRANULARITY * (i)-LOS_SLOPE_GRANULARITY / 2) / (columnsRightFromOrigin + 0.5));
      currentlyLit = true;
    }
    else if (cellObstructed && currentlyLit)
    {  // next column slope ends here
      newEndSlope =
          (long int)((LOS_SLOPE_GRANULARITY * (i)-LOS_SLOPE_GRANULARITY / 2) / (columnsRightFromOrigin - 0.5));
