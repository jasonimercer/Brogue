...
void updateFlavorText()
{
  ...
  if (rogue.disturbed && !rogue.gameHasEnded)
  {
    if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && rogue.armor->enchant2 == A_RESPIRATION &&
        tileCatalog[pmap[player.xLoc][player.yLoc].layers[highestPriorityLayer(player.xLoc, player.yLoc, false)]]
                .flags &
...
{
  return (!(monst->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst->xLoc, monst->yLoc, T_AUTO_DESCENT) &&
...
{
  char buf[COLS], buf2[COLS];
  int *x = &(monst->xLoc), *y = &(monst->yLoc), damage;
  ...
  else if (!player.status[STATUS_HALLUCINATING] && !monst->status[STATUS_LEVITATING] && canSeeMonster(monst) &&
  ...
  if (monst == &player && !(monst->status[STATUS_LEVITATING]) && cellHasTMFlag(*x, *y, TM_IS_SECRET) &&
      playerCanSee(*x, *y))
  {
    discover(*x, *y);
  }
  ...
  if ((monst->bookkeepingFlags & MB_SEIZING) && (cellHasTerrainFlag(*x, *y, T_OBSTRUCTS_PASSABILITY)) &&
  ...
  // lava
  if (!(monst->status[STATUS_LEVITATING]) && !(monst->status[STATUS_IMMUNE_TO_FIRE]) &&
  ...
  // Water puts out fire.
  if (cellHasTMFlag(*x, *y, TM_EXTINGUISHES_FIRE) && monst->status[STATUS_BURNING] &&
      !monst->status[STATUS_LEVITATING] && !(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) &&
  ...
  // Pressure plates.
  if (!(monst->status[STATUS_LEVITATING]) && !(monst->bookkeepingFlags & MB_SUBMERGED) &&
      (!cellHasTMFlag(*x, *y, TM_ALLOWS_SUBMERGING) || !(monst->info.flags & MONST_SUBMERGES)) &&
      cellHasTerrainFlag(*x, *y, T_IS_DF_TRAP) && !(pmap[*x][*y].flags & PRESSURE_PLATE_DEPRESSED))
  {
  ...
  if (cellHasTerrainFlag(*x, *y, T_ENTANGLES) && !monst->status[STATUS_STUCK] &&
      !(monst->info.flags & (MONST_IMMUNE_TO_WEBS | MONST_INVULNERABLE)) && !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
      ...
      !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
    ...
    if (monst == &player)
    {
      ...
      for (layer = 0; layer < NUMBER_TERRAIN_LAYERS &&
                      !(tileCatalog[pmap[*x][*y].layers[layer]].flags & T_CAUSES_EXPLOSIVE_DAMAGE);
           layer++)
        ;
    ...
    {  // it's a monster
      ...
      monsterName(buf, monst, true);
      if (inflictDamage(nullptr, monst, damage, &yellow, false))
      {
        // if killed
        sprintf(buf2, "%s %s %s.", buf, (monst->info.flags & MONST_INANIMATE) ? "is destroyed by" : "dies in",
  ...
  if (monst == &player && cellHasTerrainFlag(*x, *y, T_RESPIRATION_IMMUNITIES) && rogue.armor &&
      (rogue.armor->flags & ITEM_RUNIC) && rogue.armor->enchant2 == A_RESPIRATION)
  {
  ...
  {
    // zombie gas
    if (cellHasTerrainFlag(*x, *y, T_CAUSES_NAUSEA) && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) &&
        !(monst->bookkeepingFlags & MB_SUBMERGED))
    {
      if (monst == &player)
      {
      ...
      if (canDirectlySeeMonster(monst) && !(monst->status[STATUS_NAUSEOUS]))
      {
        ...
        monsterName(buf, monst, true);
        sprintf(buf2, "%s choke%s and gag%s on the overpowering stench of decay.", buf, (monst == &player ? "" : "s"),
                (monst == &player ? "" : "s"));
        message(buf2, false);
      }
    ...
    if (cellHasTerrainFlag(*x, *y, T_CAUSES_PARALYSIS) &&
        !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) && !(monst->bookkeepingFlags & MB_SUBMERGED))
    {
  ...
  // poisonous lichen
  if (cellHasTerrainFlag(*x, *y, T_CAUSES_POISON) && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) &&
  ...
  else if (cellHasTerrainFlag(*x, *y, T_IS_FLAMMABLE) && !cellHasTerrainFlag(*x, *y, T_IS_FIRE) &&
           monst->status[STATUS_BURNING] && !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
...
void applyGradualTileEffectsToCreature(Creature* monst, int ticks)
{
  int itemCandidates, randItemIndex;
  int x = monst->xLoc, y = monst->yLoc, damage;
  char buf[COLS * 5], buf2[COLS * 3];
  Item* theItem;
  enum dungeonLayers layer;

  if (!(monst->status[STATUS_LEVITATING]) && cellHasTerrainFlag(x, y, T_IS_DEEP_WATER) &&
      !cellHasTerrainFlag(x, y, (T_ENTANGLES | T_OBSTRUCTS_PASSABILITY)) &&
      ...
      if (!(pmap[x][y].flags & HAS_ITEM) && rand_percent(ticks * 50 / 100))
      {
        ...
        if (itemCandidates)
        {
          ...
          if (theItem)
          {
            ...
            sprintf(buf, "%s float%s away in the current!", buf2, (theItem->quantity == 1 ? "s" : ""));
            messageWithColor(buf, &itemMessageColor, false);
          }
        }
      }
    }
    else if (monst->carriedItem && !(pmap[x][y].flags & HAS_ITEM) && rand_percent(ticks * 50 / 100))
    {  // it's a monster with an item
      makeMonsterDropItem(monst);
    }
  }

  if (cellHasTerrainFlag(x, y, T_CAUSES_DAMAGE) && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) &&
      !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
    ...
    if (monst == &player)
    {
      ...
      else
      {
        ...
        if (inflictDamage(nullptr, &player, damage, tileCatalog[pmap[x][y].layers[layer]].backColor, true))
        {
    ...
    {  // it's a monster
      ...
      if (inflictDamage(nullptr, monst, damage, tileCatalog[pmap[x][y].layers[layer]].backColor, true))
      {
        if (canSeeMonster(monst))
        {
          monsterName(buf, monst, true);
  ...
  if (cellHasTerrainFlag(x, y, T_CAUSES_HEALING) && !(monst->info.flags & MONST_INANIMATE) &&
      !(monst->bookkeepingFlags & MB_SUBMERGED))
  {
    damage = (monst->info.maxHP / 15) * ticks / 100;
    damage = max(1, damage);
...
{
  int i, j, clairvoyanceRadius, dx, dy;
  bool cursed;
...
{
  if (!theArmor || !(theArmor->category & ARMOR))
  {
    return 0;
  }
...
void checkNutrition()
{
    ...
    for (theItem = packItems->nextItem; theItem != nullptr; theItem = theItem->nextItem)
    {
      if (theItem->category == FOOD)
      {
        sprintf(buf, "unable to control your hunger, you eat a %s.",
                (theItem->kind == FRUIT ? "mango" : "ration of food"));
  ...
  {                                       // Didn't manage to eat any food above.
...
void burnItem(Item* theItem)
{
  ...
  sprintf(buf2, "%s burn%s up!", buf1, theItem->quantity == 1 ? "s" : "");
  x = theItem->xLoc;
  y = theItem->yLoc;
...
{
  int i, currentPercent, thresholds[] = { 5, 10, 25, 40 }, pThresholds[] = { 100, 90, 50 };
  char buf[DCOLS];

  assureCosmeticRNG;

  const int healthThresholdsCount = 4, poisonThresholdsCount = 3;
...
void addXPXPToAlly(int XPXP, Creature* monst)
{
  char theMonsterName[100], buf[200];
  if (!(monst->info.flags & (MONST_INANIMATE | MONST_IMMOBILE)) &&
      !(monst->bookkeepingFlags & MB_TELEPATHICALLY_REVEALED) && monst->creatureState == MONSTER_ALLY &&
      monst->spawnDepth <= rogue.depthLevel && rogue.depthLevel <= AMULET_LEVEL)
  {
    monst->xpxp += XPXP;
    // printf("\n%i xpxp added to your %s this turn.", rogue.xpxpThisTurn, monst->info.monsterName);
    if (monst->xpxp >= XPXP_NEEDED_FOR_TELEPATHIC_BOND && !(monst->bookkeepingFlags & MB_TELEPATHICALLY_REVEALED))
    {
...
void handleXPXP()
{
  ...
  {
    for (monst = levels[rogue.depthLevel - 2].monsters; monst != nullptr; monst = monst->nextCreature)
    {
      addXPXPToAlly(rogue.xpxpThisTurn, monst);
    }
  }
  ...
  {
    for (monst = levels[rogue.depthLevel].monsters; monst != nullptr; monst = monst->nextCreature)
    {
      addXPXPToAlly(rogue.xpxpThisTurn, monst);
    }
  }
...
void playerFalls()
{
  ...
  if (cellHasTMFlag(player.xLoc, player.yLoc, TM_IS_SECRET) && playerCanSee(player.xLoc, player.yLoc))
  {
  ...
  if (rogue.depthLevel < DEEPEST_LEVEL)
  {
    ...
    if (inflictDamage(nullptr, &player, damage, &red, false))
    {
...
{
  int i, j, x, y, layer, sRows[DROWS], sCols[DCOLS], monsterCount, maxMonsters;
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      x = sCols[i];
      y = sRows[j];
      if ((pmap[x][y].flags & IS_IN_MACHINE) && pmap[x][y].machineNumber == machineNumber &&
          !(pmap[x][y].flags & IS_POWERED) && cellHasTMFlag(x, y, TM_IS_WIRED))
      {
  ...
  activatedMonsterList = nullptr;
  for (monst = monsters->nextCreature; monst != nullptr; monst = monst->nextCreature)
  {
    if (monst->machineHome == machineNumber && monst->spawnDepth == rogue.depthLevel &&
...
bool circuitBreakersPreventActivation(int machineNumber)
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (pmap[i][j].machineNumber == machineNumber && cellHasTMFlag(i, j, TM_IS_CIRCUIT_BREAKER))
      {
        return true;
      }
    }
...
void promoteTile(int x, int y, enum dungeonLayers layer, bool useFireDF)
{
  ...
  if ((tile->mechFlags & TM_VANISHES_UPON_PROMOTION))
  {
    ...
    pmap[x][y].layers[layer] =
        (layer == DUNGEON ? FLOOR : NOTHING);  // even the dungeon layer implicitly has floor underneath it
    if (layer == GAS)
    {
      pmap[x][y].volume = 0;
    }
...
{
  enum dungeonLayers layer;
  int ignitionChance = 0, bestExtinguishingPriority = 1000, explosiveNeighborCount = 0;
  int newX, newY;
  enum Directions dir;
  ...
  // Pick the fire type of the most flammable layer that is either gas or equal-or-better priority than the best
  // extinguishing layer.
  for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++)
  {
  ...
  if (alwaysIgnite || (ignitionChance && rand_percent(ignitionChance)))
  {  // If it ignites...
    fireIgnited = true;
    ...
    if (cellHasTMFlag(x, y, TM_EXPLOSIVE_PROMOTE))
    {
      for (dir = 0, explosiveNeighborCount = 0; dir < DIRECTION_COUNT; dir++)
      {
        newX = x + nbDirs[dir][0];
        newY = y + nbDirs[dir][1];
        if (coordinatesAreInMap(newX, newY) && (cellHasTerrainFlag(newX, newY, T_IS_FIRE | T_OBSTRUCTS_GAS) ||
                                                cellHasTMFlag(newX, newY, TM_EXPLOSIVE_PROMOTE)))
        {
...
{
  int i, j, newX, newY, numSpaces;
  unsigned long highestNeighborVolume;
  unsigned long sum;
  enum TileType gasType;
  enum Directions dir;
  unsigned int newGasVolume[DCOLS][DROWS];
    ...
    for (j = 0; j < DROWS; j++)
    {
      if (!cellHasTerrainFlag(i, j, T_OBSTRUCTS_GAS))
      {
        ...
        for (dir = 0; dir < DIRECTION_COUNT; dir++)
        {
          newX = i + nbDirs[dir][0];
          newY = j + nbDirs[dir][1];
          if (coordinatesAreInMap(newX, newY) && !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_GAS))
          {
        ...
        if (cellHasTerrainFlag(i, j, T_AUTO_DESCENT))
        {               // if it's a chasm tile or trap door,
      ...
      else if (pmap[i][j].volume > 0)
      {  // if has gas but can't hold gas,
...
void updateYendorWardenTracking()
{
  ...
  {
    levels[rogue.yendorWarden->depth - 1].mapStorage[rogue.yendorWarden->xLoc][rogue.yendorWarden->yLoc].flags &=
        ~HAS_MONSTER;
  }
  ...
  {
    for (prevMonst = levels[n].monsters; prevMonst->nextCreature != rogue.yendorWarden;
         prevMonst = prevMonst->nextCreature)
      ;
...
void monstersFall()
{
  ...
  for (monst = monsters->nextCreature; monst != nullptr; monst = nextCreature)
  {
    ...
    if ((monst->bookkeepingFlags & MB_IS_FALLING) || monsterShouldFall(monst))
    {
      ...
      else if (!inflictDamage(nullptr, monst, randClumpedRange(6, 12, 2), &red, false))
      {
        demoteMonsterFromLeadership(monst);

        // remove from monster chain
        for (previousCreature = monsters; previousCreature->nextCreature != monst;
...
{
  int i, j, direction, newX, newY, promotions[DCOLS][DROWS];
  long promoteChance;
  enum dungeonLayers layer;
  FloorTileType* tile;
...
{
  int i, j;
  int **playerCostMap, **monsterCostMap;
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      ...
      {
        playerCostMap[i][j] = monsterCostMap[i][j] =
            cellHasTerrainFlag(i, j, T_OBSTRUCTS_DIAGONAL_MOVEMENT) ? PDS_OBSTRUCTION : PDS_FORBIDDEN;
      }
...
void resetDistanceCellInGrid(int** grid, int x, int y)
{
  enum Directions dir;
  int newX, newY;
  for (dir = 0; dir < 4; dir++)
  {
    newX = x + nbDirs[dir][0];
    newY = y + nbDirs[dir][1];
    if (coordinatesAreInMap(newX, newY) && grid[x][y] > grid[newX][newY] + 1)
    {
...
void updateSafetyMap()
{
    ...
    for (j = 0; j < DROWS; j++)
    {
      ...
      else
      {
        if (pmap[i][j].flags & HAS_MONSTER)
        {
          ...
          if ((monst->creatureState == MONSTER_SLEEPING || monst->turnsSpentStationary > 2 ||
               (monst->info.flags & MONST_GETS_TURN_ON_ACTIVATION) || monst->creatureState == MONSTER_ALLY) &&
        ...
        else if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY) && cellHasTMFlag(i, j, TM_IS_SECRET) &&
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY) && cellHasTMFlag(i, j, TM_IS_SECRET) &&
          !(discoveredTerrainFlagsAtLoc(i, j) & T_OBSTRUCTS_PASSABILITY) && !(pmap[i][j].flags & IN_FIELD_OF_VIEW))
      {
...
void processIncrementalAutoID()
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    if (theItem && theItem->charges > 0 && !(theItem->flags & ITEM_IDENTIFIED))
    {
      theItem->charges--;
      if (theItem->charges <= 0)
      {
        ...
        itemName(theItem, theItemName, true, true, nullptr);
...
void rechargeItemsIncrementally(int multiplier)
{
  Item* theItem;
  char buf[DCOLS * 3], theItemName[DCOLS * 3];
  int rechargeIncrement, staffRechargeDuration;

  if (rogue.wisdomBonus)
  {
    rechargeIncrement =
        ringWisdomMultiplier(rogue.wisdomBonus);  // at level 27, you recharge anything to full in one turn
  }
  else
  {
...
void monsterEntersLevel(Creature* monst, int n)
{
  Creature* prevMonst;
  ...
  else if (monst->bookkeepingFlags & MB_APPROACHING_PIT)
  {  // jumping down pit
    pit = true;
  ...
  if (!pit && (pmap[monst->xLoc][monst->yLoc].flags & (HAS_PLAYER | HAS_MONSTER)) &&
      !(terrainFlags(monst->xLoc, monst->yLoc) & avoidedFlagsForMonster(&(monst->info))))
  {
    ...
    getQualifyingPathLocNear(&(prevMonst->xLoc), &(prevMonst->yLoc), monst->xLoc, monst->yLoc, true,
                             ...
                             avoidedFlagsForMonster(&(prevMonst->info)), (HAS_MONSTER | HAS_PLAYER | HAS_STAIRS),
                             false);
    ...
    // DEBUG printf("\nBumped a creature (%s) from (%i, %i) to (%i, %i).", prevMonst->info.monsterName, monst->xLoc,
    // monst->yLoc, prevMonst->xLoc, prevMonst->yLoc);
  }
  ...
  monst->bookkeepingFlags |= MB_PREPLACED;
  monst->bookkeepingFlags &= ~MB_IS_FALLING;
  restoreMonster(monst, nullptr, nullptr);
  ...
  if (pit)
  {
    ...
    {
      if (inflictDamage(nullptr, monst, randClumpedRange(6, 12, 2), &red, false))
      {
        if (canSeeMonster(monst))
        {
...
void monstersApproachStairs()
{
  ...
  for (n = rogue.depthLevel - 2; n <= rogue.depthLevel; n += 2)
  {  // cycle through previous and next level
    if (n >= 0 && n < DEEPEST_LEVEL && levels[n].visited)
    {
      for (monst = levels[n].monsters; monst != nullptr;)
      {
  ...
  if (rogue.yendorWarden && abs(rogue.depthLevel - rogue.yendorWarden->depth) > 1)
  {
    updateYendorWardenTracking();
  }
}
...
void autoRest()
{
  ...
  if ((player.currentHP < player.info.maxHP || player.status[STATUS_HALLUCINATING] || player.status[STATUS_CONFUSED] ||
       player.status[STATUS_NAUSEOUS] || player.status[STATUS_POISONED] || player.status[STATUS_DARKNESS] ||
       initiallyEmbedded) &&
      !rogue.disturbed)
  {
    while (i++ < TURNS_FOR_FULL_REGEN &&
           (player.currentHP < player.info.maxHP || player.status[STATUS_HALLUCINATING] ||
            player.status[STATUS_CONFUSED] || player.status[STATUS_NAUSEOUS] || player.status[STATUS_POISONED] ||
            player.status[STATUS_DARKNESS] || cellHasTerrainFlag(player.xLoc, player.yLoc, T_OBSTRUCTS_PASSABILITY)) &&
           !rogue.disturbed &&
           (!initiallyEmbedded || cellHasTerrainFlag(player.xLoc, player.yLoc, T_OBSTRUCTS_PASSABILITY)))
    {
...
{
  int soonestTurn, damage, turnsRequiredToShore, turnsToShore;
  char buf[COLS], buf2[COLS];
  ...
  do
  {
    ...
    else if (player.currentHP < player.info.maxHP && !player.status[STATUS_POISONED])
    {
    ...
    for (monst = monsters->nextCreature; monst != nullptr; monst = nextMonst)
    {
      ...
      if ((monst->bookkeepingFlags & MB_BOUND_TO_LEADER) &&
          (!monst->leader || !(monst->bookkeepingFlags & MB_FOLLOWER)) && (monst->creatureState != MONSTER_ALLY))
      {
        killCreature(monst, false);
        if (canSeeMonster(monst))
        {
    ...
    if (player.status[STATUS_BURNING] > 0)
    {
      ...
      if (!(player.status[STATUS_IMMUNE_TO_FIRE]) && inflictDamage(nullptr, &player, damage, &orange, true))
      {
    ...
    if (player.status[STATUS_POISONED] > 0)
    {
      ...
      if (inflictDamage(nullptr, &player, player.poisonAmount, &green, true))
      {
    ...
    {
      if (D_SAFETY_VISION ||
          monst->creatureState == MONSTER_FLEEING && pmap[monst->xLoc][monst->yLoc].flags & IN_FIELD_OF_VIEW)
      {
    ...
    while (player.ticksUntilTurn > 0)
    {
      ...
      if (rogue.ticksTillUpdateEnvironment <= 0)
      {
        ...
        {
          nextMonst = monst->nextCreature;
          if (monst->info.DFChance && !(monst->info.flags & MONST_GETS_TURN_ON_ACTIVATION) &&
      ...
      for (monst = monsters->nextCreature; (monst != nullptr) && (rogue.gameHasEnded == false);
           monst = monst->nextCreature)
      {
        if (monst->ticksUntilTurn <= 0)
        {
          ...
          if ((monst->info.flags & MONST_GETS_TURN_ON_ACTIVATION) || monst->status[STATUS_PARALYZED] ||
              monst->status[STATUS_ENTRANCED] || (monst->bookkeepingFlags & MB_CAPTIVE))
          {
          ...
          {
            if (monst2 == monst)
            {  // monst still alive and on the level
    ...
    for (monst = monsters->nextCreature; monst != nullptr; monst = monst->nextCreature)
    {
      if (canSeeMonster(monst) && !(monst->bookkeepingFlags & (MB_WAS_VISIBLE | MB_ALREADY_SEEN)))
      {
        ...
        if (monst->creatureState != MONSTER_ALLY)
        {
          ...
          if (rogue.cautiousMode || rogue.automationActive)
          {
            ...
            sprintf(buf, "you %s a%s %s", playerCanDirectlySee(monst->xLoc, monst->yLoc) ? "see" : "sense",
                    (isVowelish(buf2) ? "n" : ""), buf2);
        ...
        {
          if (rogue.weapon && rogue.weapon->flags & ITEM_RUNIC && rogue.weapon->enchant2 == W_SLAYING &&
              !(rogue.weapon->flags & ITEM_RUNIC_HINTED) && monsterIsInClass(monst, rogue.weapon->vorpalEnemy))
          {
            rogue.weapon->flags |= ITEM_RUNIC_HINTED;
            itemName(rogue.weapon, buf2, false, false, nullptr);
            sprintf(buf, "the runes on your %s gleam balefully.", buf2);
            messageWithColor(buf, &itemMessageColor, true);
          }
          if (rogue.armor && rogue.armor->flags & ITEM_RUNIC && rogue.armor->enchant2 == A_IMMUNITY &&
              !(rogue.armor->flags & ITEM_RUNIC_HINTED) && monsterIsInClass(monst, rogue.armor->vorpalEnemy))
          {
      ...
      else if (!canSeeMonster(monst) && (monst->bookkeepingFlags & MB_WAS_VISIBLE) &&
  ...
  if ((player.status[STATUS_LEVITATING] &&
       cellHasTerrainFlag(player.xLoc, player.yLoc, T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER | T_AUTO_DESCENT)) ||
      (player.status[STATUS_IMMUNE_TO_FIRE] && cellHasTerrainFlag(player.xLoc, player.yLoc, T_LAVA_INSTA_DEATH)))
  {
    if (!rogue.receivedLevitationWarning)
    {
      ...
      {
        turnsToShore =
            max(player.status[STATUS_LEVITATING], player.status[STATUS_IMMUNE_TO_FIRE]) * 100 / player.movementSpeed;
      }
      else
      {
...
{  // don't want player.scentTurnNumber to roll over the int maxint!
  int i, j, d;
