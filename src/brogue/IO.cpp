...
// Populates path[][] with a list of coordinates starting at origin and traversing down the map. Returns the number of
// steps in the path.
...
void clearCursorPath()
{
  ...
  {  // There are no cursor paths during playback.
    for (i = 1; i < DCOLS; i++)
    {
      for (j = 1; j < DROWS; j++)
      {
...
void getClosestValidLocationOnMap(int loc[2], int** map, int x, int y)
{
  int i, j, dist, closestDistance, lowestMapScore;
  ...
  for (i = 1; i < DCOLS - 1; i++)
  {
    for (j = 1; j < DROWS - 1; j++)
    {
      if (map[i][j] >= 0 && map[i][j] < 30000)
      {
        ...
        if (dist < closestDistance || dist == closestDistance && map[i][j] < lowestMapScore)
        {
          loc[0] = i;
          loc[1] = j;
    ...
    for (j = 0; j < DROWS; j++)
    {
      if (cellHasTMFlag(i, j, TM_INVERT_WHEN_HIGHLIGHTED))
      {
        for (dir = 0; dir < 4; dir++)
        {
          newX = i + nbDirs[dir][0];
          newY = j + nbDirs[dir][1];
          if (coordinatesAreInMap(newX, newY) && map[newX][newY] >= 0 && map[newX][newY] < map[i][j])
          {
...
int actionMenu(int x, bool playingBack)
{
  int buttonCount;
  int y;
  ...
  int i, j, longestName = 0, buttonChosen;
  ...
  do
  {
    ...
    if (KEYBOARD_LABELS)
    {
      sprintf(buttons[buttonCount].text, "  %s\\: %s%s color effects  ", yellowColorEscape, whiteColorEscape,
              rogue.trueColorMode ? "Show" : "Hide");
    }
    else
    {
    ...
    if (KEYBOARD_LABELS)
    {
      sprintf(buttons[buttonCount].text, "  %s]: %s%s stealth range  ", yellowColorEscape, whiteColorEscape,
              rogue.displayAggroRangeMode ? "Hide" : "Display");
    }
    else
    {
    ...
    buttonCount++;
    if (KEYBOARD_LABELS)
    {  // No help button if we're not in keyboard mode.
    ...
    if (KEYBOARD_LABELS)
    {
      sprintf(buttons[buttonCount].text, "  %sQ: %sQuit %s  ", yellowColorEscape, whiteColorEscape,
              (playingBack ? "to title screen" : "without saving"));
    }
    else
    {
    ...
    buttonChosen = buttonInputLoop(buttons, buttonCount, x - 1, y, longestName + 2, buttonCount, nullptr);
    overlayDisplayBuffer(rbuf, nullptr);
    if (buttonChosen == -1)
    {
      return -1;
    }
...
void initializeMenuButtons(ButtonState* state, BrogueButton buttons[5])
{
  ...
  initializeButtonState(state, buttons, 5, mapToWindowX(0), ROWS - 1, COLS - mapToWindowX(0), 1);
  ...
  for (i = 0; i < COLS; i++)
  {  // So the buttons stay (but are dimmed and desaturated) when inactive.
...
void mainInputLoop()
{
  int originLoc[2], pathDestination[2], oldTargetLoc[2], path[1000][2], steps, oldRNG, dir, newX, newY;
  Creature* monst;
  Item* theItem;
  cellDisplayBuffer rbuf[COLS][ROWS];

  bool canceled, targetConfirmed, tabKey, focusedOnMonster, focusedOnItem, focusedOnTerrain, playingBack, doEvent,
      textDisplayed;

  RogueEvent theEvent;
  int **costMap, **playerPathingMap, **cursorSnapMap;
  ...
  while (!rogue.gameHasEnded && (!playingBack || !canceled))
  {  // repeats until the game ends
    ...
    if (!playingBack && player.xLoc == cursor[0] && player.yLoc == cursor[1] && oldTargetLoc[0] == cursor[0] &&
    ...
    do
    {
      ...
      if (!playingBack)
      {
        ...
        {
          if (cursorSnapMap[cursor[0]][cursor[1]] >= 0 && cursorSnapMap[cursor[0]][cursor[1]] < 30000)
          {
          ...
          //					steps = getPlayerPathOnMap(path, playerPathingMap, pathDestination[0], pathDestination[1]) - 1; //
          //Get new path. 					reversePath(path, steps);   // Flip it around, back-to-front.
          ...
          if (playerPathingMap[player.xLoc][player.yLoc] != 1 || pathDestination[0] != cursor[0] ||
              pathDestination[1] != cursor[1])
          {
      ...
      if (coordinatesAreInMap(cursor[0], cursor[1]))
      {
        hiliteCell(cursor[0], cursor[1], &white,
                   (steps <= 0 || (path[steps - 1][0] == cursor[0] && path[steps - 1][1] == cursor[1]) ||
                            (!playingBack && distanceBetween(player.xLoc, player.yLoc, cursor[0], cursor[1]) <= 1) ?
                        100 :
                        25),
                   true);
        ...
        if (monst != nullptr && (canSeeMonster(monst) || rogue.playbackOmniscience))
        {
        ...
        else if (theItem != nullptr && playerCanSeeOrSense(cursor[0], cursor[1]))
        {
          rogue.playbackMode = playingBack;
          refreshSideBar(cursor[0], cursor[1], false);
          rogue.playbackMode = false;
      ...
      // Get the input!
      rogue.playbackMode = playingBack;
      doEvent = moveCursor(&targetConfirmed, &canceled, &tabKey, cursor, &theEvent, &state, !textDisplayed,
                           rogue.cursorMode, true);
      rogue.playbackMode = false;
      ...
      if (playingBack)
      {
        ...
        if (theEvent.eventType == KEYSTROKE && theEvent.param1 == ACKNOWLEDGE_KEY)
        {  // To unpause by button during playback.
          canceled = true;
        }
        else
        {
      ...
      if (tabKey && !playingBack)
      {  // The tab key cycles the cursor through monsters, items and terrain features.
    ...
    else if (targetConfirmed && !playingBack && coordinatesAreInMap(cursor[0], cursor[1]))
    {
      if (theEvent.eventType == MOUSE_UP && theEvent.controlKey && steps > 1)
      {
        ...
        for (dir = 0; dir < DIRECTION_COUNT &&
                      (player.xLoc + nbDirs[dir][0] != path[0][0] || player.yLoc + nbDirs[dir][1] != path[0][1]);
             dir++)
          ;
        playerMoves(dir);
      }
      ...
      {
        confirmMessages();
        if (originLoc[0] == cursor[0] && originLoc[1] == cursor[1])
        {
          confirmMessages();
        }
                 ...
                 (distanceBetween(player.xLoc, player.yLoc, cursor[0], cursor[1]) == 1  // includes diagonals
                  && (!diagonalBlocked(player.xLoc, player.yLoc, cursor[0], cursor[1], !rogue.playbackOmniscience) ||
                      ((pmap[cursor[0]][cursor[1]].flags & HAS_MONSTER) &&
                       (monsterAtLoc(cursor[0], cursor[1])->info.flags & MONST_ATTACKABLE_THRU_WALLS))  // there's a
                                                                                                        // turret there
                      || ((terrainFlags(cursor[0], cursor[1]) & T_OBSTRUCTS_PASSABILITY) &&
                          (terrainMechFlags(cursor[0], cursor[1]) & TM_PROMOTES_ON_PLAYER_ENTRY)))))
        {  // there's a lever there
           // Clicking one space away will cause the player to try to move there directly irrespective of path.
          for (dir = 0; dir < DIRECTION_COUNT &&
                        (player.xLoc + nbDirs[dir][0] != cursor[0] || player.yLoc + nbDirs[dir][1] != cursor[1]);
               dir++)
            ;
          playerMoves(dir);
        }
...
{
  int rand = rand_range(0, theColor->rand);
...
void bakeTerrainColors(Color* foreColor, Color* backColor, int x, int y)
{
  const int* vals;
  if (rogue.trueColorMode)
  {
    const int nf = 1000;
    const int nb = 0;
    const int neutralColors[8] = { nf, nf, nf, nf, nb, nb, nb, nb };
    vals = neutralColors;
  }
  else
  {
  ...
  const int foreRand = foreColor->rand * vals[6] / 1000;
  const int backRand = backColor->rand * vals[7] / 1000;
...
void shuffleTerrainColors(int percentOfCells, bool refreshCells)
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (playerCanSeeOrSense(i, j) && (!rogue.automationActive || !(rogue.playerTurnNumber % 5)) &&
...
void normColor(Color* baseColor, const int aggregateMultiplier, const int colorTranslation)
{
  ...
  const int vectorLength = baseColor->red + baseColor->green + baseColor->blue;
...
{
  int bestBCPriority, bestFCPriority, bestCharPriority;
  int distance;
  uchar cellChar = 0;
  ...
  int gasAugmentWeight = 0;
  Creature* monst = nullptr;
  Item* theItem = nullptr;
  enum TileType tile = NOTHING;
  const uchar itemChars[] = { POTION_CHAR, SCROLL_CHAR, FOOD_CHAR,   WAND_CHAR, STAFF_CHAR,
                              GOLD_CHAR,   ARMOR_CHAR,  WEAPON_CHAR, RING_CHAR, CHARM_CHAR };
  ...
  if (!playerCanSeeOrSense(x, y) && !(pmap[x][y].flags & (ITEM_DETECTED | HAS_PLAYER)) &&
      (!monst || !monsterRevealed(monst)) && !monsterWithDetectedItem &&
      (pmap[x][y].flags & (DISCOVERED | MAGIC_MAPPED)) && (pmap[x][y].flags & STABLE_MEMORY))
  {
    // restore memory
  ...
  else
  {
    ...
    for (layer = 0; layer < maxLayer; layer++)
    {
      ...
      if (pmap[x][y].layers[layer] && layer != GAS)
      {
        ...
        if (tileCatalog[tile].drawPriority < bestFCPriority && tileCatalog[tile].foreColor)
        {
          cellForeColor = *(tileCatalog[tile].foreColor);
          bestFCPriority = tileCatalog[tile].drawPriority;
        }
        if (tileCatalog[tile].drawPriority < bestBCPriority && tileCatalog[tile].backColor)
        {
          cellBackColor = *(tileCatalog[tile].backColor);
          bestBCPriority = tileCatalog[tile].drawPriority;
        }
        if (tileCatalog[tile].drawPriority < bestCharPriority && tileCatalog[tile].displayChar)
        {
    ...
    else if (((pmap[x][y].flags & HAS_ITEM) && (pmap[x][y].flags & ITEM_DETECTED) && itemMagicChar(theItem) &&
    ...
    {
      needDistinctness = true;
      if (player.status[STATUS_HALLUCINATING] > 0 && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) &&
      ...
      else
      {
        ...
        else
        {
          if (monst->creatureState == MONSTER_ALLY &&
              (monst->info.displayChar >= 'a' && monst->info.displayChar <= 'z' ||
               monst->info.displayChar >= 'A' && monst->info.displayChar <= 'Z'))
          {
            if (rogue.trueColorMode)
            {
    ...
    else if (monst && monsterRevealed(monst) && !canSeeMonster(monst))
    {
      if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience)
      {
      ...
      if (!(pmap[x][y].flags & DISCOVERED))
      {
        ...
        gasAugmentColor = black;
             ...
             (playerCanSeeOrSense(x, y) ||
              ((pmap[x][y].flags & DISCOVERED) && !cellHasTerrainFlag(x, y, T_MOVES_ITEMS))))
    {
      needDistinctness = true;
      if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience)
      {
    ...
    if (gasAugmentWeight && ((pmap[x][y].flags & DISCOVERED) || rogue.playbackOmniscience))
    {
      ...
      if ((pmap[x][y].flags & HAS_MONSTER) && monst->status[STATUS_INVISIBLE] && playerCanSeeOrSense(x, y) &&
          !monsterRevealed(monst) && !monsterHiddenBySubmersion(monst, &player))
      {
        if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience)
        {
    ...
    if (!(pmap[x][y].flags & (ANY_KIND_OF_VISIBLE | ITEM_DETECTED | HAS_PLAYER)) && !playerCanSeeOrSense(x, y) &&
        (!monst || !monsterRevealed(monst)) && !monsterWithDetectedItem)
    {
      if (rogue.trueColorMode)
      {
  ...
  if (((pmap[x][y].flags & ITEM_DETECTED) || monsterWithDetectedItem || (monst && monsterRevealed(monst))) &&
      !playerCanSeeOrSense(x, y))
  {
    // do nothing
  }
  ...
  else if (!(pmap[x][y].flags & VISIBLE) && !rogue.playbackOmniscience)
  {
    ...
    {
      if (!cellHasTMFlag(x, y, TM_BRIGHT_MEMORY) && (!rogue.trueColorMode || !needDistinctness))
      {
...
{
  int weightComplement = 100 - averageWeight;
...
int randomizeByPercent(int input, int percent)
{
...
void blendAppearances(const Color* fromForeColor, const Color* fromBackColor, const uchar fromChar,
                      const Color* toForeColor, const Color* toBackColor, const uchar toChar, Color* retForeColor,
                      Color* retBackColor, uchar* retChar, const int percent)
{
  ...
  *retBackColor = *fromBackColor;
  ...
  {
    // If it is changing, the first half blends to the current back color, and the second half blends to the final back
    // color.
    if (percent >= 50)
    {
...
void irisFadeBetweenBuffers(cellDisplayBuffer fromBuf[COLS][ROWS], cellDisplayBuffer toBuf[COLS][ROWS], int x,
                            int y, int frameCount, bool outsideIn)
{
  int i, j, frame, percentBasis, thisCellPercent;
  bool fastForward;
  ...
  int completionMap[COLS][ROWS], maxDistance;
  ...
  do
  {
    ...
    for (i = 0; i < COLS; i++)
    {
      for (j = 0; j < ROWS; j++)
      {
        ...
        blendAppearances(&fromForeColor, &fromBackColor, fromChar, &toForeColor, &toBackColor, toChar,
                         &currentForeColor, &currentBackColor, &currentChar, clamp(thisCellPercent, 0, 100));
  ...
  overlayDisplayBuffer(toBuf, nullptr);
}
...
void plotCharWithColor(uchar inputChar, int xLoc, int yLoc, const Color* cellForeColor, const Color* cellBackColor)
{
  ...
  int foreRed = cellForeColor->red, foreGreen = cellForeColor->green, foreBlue = cellForeColor->blue,
  ...
  if (inputChar != ' ' && foreRed == backRed && foreGreen == backGreen && foreBlue == backBlue)
  {
    inputChar = ' ';
  }

  if (inputChar != displayBuffer[xLoc][yLoc].character || foreRed != displayBuffer[xLoc][yLoc].foreColorComponents[0] ||
                      ...
                      cellDisplayBuffer dbuf[COLS][ROWS])
{
  ...
  rogue.RNG = RNG_COSMETIC;
  // assureCosmeticRNG;
  dbuf[x][y].foreColorComponents[0] =
      foreColor->red + rand_range(0, foreColor->redRand) + rand_range(0, foreColor->rand);
  dbuf[x][y].foreColorComponents[1] =
      foreColor->green + rand_range(0, foreColor->greenRand) + rand_range(0, foreColor->rand);
  dbuf[x][y].foreColorComponents[2] =
      foreColor->blue + rand_range(0, foreColor->blueRand) + rand_range(0, foreColor->rand);
  dbuf[x][y].backColorComponents[0] =
      backColor->red + rand_range(0, backColor->redRand) + rand_range(0, backColor->rand);
  dbuf[x][y].backColorComponents[1] =
      backColor->green + rand_range(0, backColor->greenRand) + rand_range(0, backColor->rand);
  dbuf[x][y].backColorComponents[2] =
      backColor->blue + rand_range(0, backColor->blueRand) + rand_range(0, backColor->rand);
...
void commitDraws()
{
  ...
  for (i = 0; i < COLS; i++)
  {
    for (j = 0; j < ROWS; j++)
    {
      ...
      {
        plotChar(displayBuffer[i][j].character, i, j, displayBuffer[i][j].foreColorComponents[0],
                 displayBuffer[i][j].foreColorComponents[1], displayBuffer[i][j].foreColorComponents[2],
                 displayBuffer[i][j].backColorComponents[0], displayBuffer[i][j].backColorComponents[1],
...
void dumpLevelToScreen()
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (pmap[i][j].layers[DUNGEON] != GRANITE || (pmap[i][j].flags & DISCOVERED))
      {
        backup = pmap[i][j];
...
{
  int i, j, k, intensity, currentRadius, fadeOut;
  int localRadius[DCOLS][DROWS];
  ...
  for (n = (invert ? stepCount - 1 : 0); (invert ? n >= 0 : n <= stepCount); n += (invert ? -1 : 1))
  {
    for (i = 0; i < COLS; i++)
    {
      for (j = 0; j < ROWS; j++)
      {
        ...
        if (j < MESSAGE_LINES && i >= mapToWindowX(0) &&
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (chokeMap[i][j] < CHOKEMAP_DISPLAY_CUTOFF)
      {
        ...
        else if (chokeMap[i][j] < CHOKEMAP_DISPLAY_CUTOFF)
        {
          getCellAppearance(i, j, &dchar, &foreColor, &backColor);
...
void exploreKey(const bool controlKey)
{
  ...
  if (dir == NO_DIRECTION)
  {
    for (dir = 0; dir < DIRECTION_COUNT; dir++)
    {
      ...
      if (coordinatesAreInMap(x, y) && !(pmap[x][y].flags & DISCOVERED))
      {
        tooDark = true;
        break;
      }
    ...
    if (!tooDark)
    {
      ...
      do
      {
        dir = nextStep(exploreMap, x, y, nullptr, false);
        if (dir != NO_DIRECTION)
        {
          x += nbDirs[dir][0];
          y += nbDirs[dir][1];
...
void executeKeystroke(signed long keystroke, bool controlKey, bool shiftKey)
{
  ...
  switch (keystroke)
  {
    ...
    case TRUE_COLORS_KEY:
      ...
      if (rogue.trueColorMode)
      {
        messageWithColor(KEYBOARD_LABELS ? "Color effects disabled. Press '\\' again to enable." :
                                           "Color effects disabled.",
                         &teal, false);
      }
      else
      {
        messageWithColor(KEYBOARD_LABELS ? "Color effects enabled. Press '\\' again to disable." :
    ...
    case AGGRO_DISPLAY_KEY:
      ...
      {
        messageWithColor(KEYBOARD_LABELS ? "Stealth range displayed. Press ']' again to hide." :
                                           "Stealth range displayed.",
                         &teal, false);
      }
      else
      {
        messageWithColor(KEYBOARD_LABELS ? "Stealth range hidden. Press ']' again to display." :
                           ...
                           const char* promptSuffix, int textEntryType, bool useDialogBox)
{
  ...
  const int textEntryBounds[TEXT_INPUT_TYPES][2] = { { ' ', '~' }, { ' ', '~' }, { '0', '9' } };
  cellDisplayBuffer dbuf[COLS][ROWS], rbuf[COLS][ROWS];
  ...
  if (useDialogBox)
  {
    ...
    y = ROWS / 2 - 1;
    clearDisplayBuffer(dbuf);
    rectangularShading(x - 1, y - 2, max(maxLength, strLenWithoutEscapes(prompt)) + 2, 4, &interfaceBoxColor,
                       INTERFACE_OPACITY, dbuf);
    overlayDisplayBuffer(dbuf, rbuf);
    printString(prompt, x, y - 1, &white, &interfaceBoxColor, nullptr);
  ...
  do
  {
    ...
    else if (keystroke >= textEntryBounds[textEntryType][0] && keystroke <= textEntryBounds[textEntryType][1])
    {  // allow only permitted input

      if (textEntryType == TEXT_INPUT_FILENAME && characterForbiddenInFilename(keystroke))
      {
        keystroke = '-';
      }
...
bool confirm(char* prompt, bool alsoDuringPlayback)
{
  ...
  {  // If they canceled or pressed no.
    return false;
  }
  else
  {
...
{
  Creature* monst;
  int x[100], y[100], strength[100], count = 0;
...
void displayMessageArchive()
{
  ...
  for (totalMessageCount = 0; totalMessageCount < MESSAGE_ARCHIVE_LINES && messageArchive[totalMessageCount][0];
       totalMessageCount++)
    ;
  ...
  if (totalMessageCount > MESSAGE_LINES)
  {
    ...
    for (reverse = 0; reverse <= 1; reverse++)
    {
           ...
           currentMessageCount += (reverse ? -1 : 1))
      {
        ...
        {
          printString(messageArchive[(messageArchivePosition - currentMessageCount + MESSAGE_ARCHIVE_LINES + j) %
                                     MESSAGE_ARCHIVE_LINES],
                      mapToWindowX(0), j, &white, &black, dbuf);
        }

        // Set the dbuf opacity, and do a fade from bottom to top to make it clear that the bottom messages are the most
        // recent.
        for (j = 0; j < currentMessageCount && j < ROWS; j++)
        {
          ...
          for (i = 0; i < DCOLS; i++)
          {
            ...
            if (dbuf[mapToWindowX(i)][j].character != ' ')
            {
              for (k = 0; k < 3; k++)
              {
                dbuf[mapToWindowX(i)][j].foreColorComponents[k] =
                    dbuf[mapToWindowX(i)][j].foreColorComponents[k] * fadePercent / 100;
              }
            }
          }
        }
...
void updateMessageDisplay()
{
  ...
  for (i = 0; i < MESSAGE_LINES; i++)
  {
    ...
    for (j = m = 0; displayedMessage[i][m] && j < DCOLS; j++, m++)
    {
      ...
      plotCharWithColor(displayedMessage[i][m], mapToWindowX(j), MESSAGE_LINES - i - 1, &messageColor, &black);
    }
    for (; j < DCOLS; j++)
    {
...
{
  const unsigned int newKey = *keystroke - ('A' - 'a');
  if (newKey == LEFT_KEY || newKey == RIGHT_KEY || newKey == DOWN_KEY || newKey == UP_KEY || newKey == UPLEFT_KEY ||
      newKey == UPRIGHT_KEY || newKey == DOWNLEFT_KEY || newKey == DOWNRIGHT_KEY)
  {
    *keystroke -= 'A' - 'a';
  }
}
...
{
  int printY, oldPrintY, shortestDistance, i, j, k, px, py, x, y, displayEntityCount, indirectVision;
  Creature *monst, *closestMonst;
  Item *theItem, *closestItem;
  char buf[COLS];
  void *entityList[ROWS] = { 0 }, *focusEntity = nullptr;
  enum entityDisplayTypes entityType[ROWS] = { 0 }, focusEntityType = EDT_NOTHING;
  int terrainLocationMap[ROWS][2];
  ...
  else
  {
    ...
    if (!focusEntity && cellHasTMFlag(focusX, focusY, TM_LIST_IN_SIDEBAR) && playerCanSeeOrSense(focusX, focusY))
    {
      focusEntity =
          tileCatalog[pmap[focusX][focusY].layers[layerWithTMFlag(focusX, focusY, TM_LIST_IN_SIDEBAR)]].description;
  ...
  // Now list the monsters that we'll be displaying in the order of their proximity to player (listing the focused first
  // if required).
  ...
  for (indirectVision = 0; indirectVision < 2; indirectVision++)
  {
    ...
    do
    {
      ...
      for (monst = monsters->nextCreature; monst != nullptr; monst = monst->nextCreature)
      {
        if ((canDirectlySeeMonster(monst) || (indirectVision && (canSeeMonster(monst) || rogue.playbackOmniscience))) &&
            !addedEntity[monst->xLoc][monst->yLoc] && !(monst->info.flags & MONST_NOT_LISTED_IN_SIDEBAR) &&
    ...
    } while (shortestDistance < 10000 && displayEntityCount * 2 < ROWS);  // Because each entity takes at least 2 rows
                                                                          // in the sidebar.
    ...
    do
    {
      ...
      {
        if ((playerCanDirectlySee(theItem->xLoc, theItem->yLoc) ||
             (indirectVision && (playerCanSeeOrSense(theItem->xLoc, theItem->yLoc) || rogue.playbackOmniscience))) &&
            !addedEntity[theItem->xLoc][theItem->yLoc] &&
            (px - theItem->xLoc) * (px - theItem->xLoc) + (py - theItem->yLoc) * (py - theItem->yLoc) <
                shortestDistance)
        {
    ...
    for (k = 0; k < max(DROWS, DCOLS); k++)
    {
      for (i = px - k; i <= px + k; i++)
      {
        for (j = py - k; j <= py + k; j++)
        {
          if (coordinatesAreInMap(i, j) && (i == px - k || i == px + k || j == py - k || j == py + k) &&
              !addedEntity[i][j] &&
              (playerCanDirectlySee(i, j) ||
               (indirectVision && (playerCanSeeOrSense(i, j) || rogue.playbackOmniscience))) &&
              cellHasTMFlag(i, j, TM_LIST_IN_SIDEBAR) && displayEntityCount < ROWS - 1)
          {
            addedEntity[i][j] = true;
            entityList[displayEntityCount] =
                tileCatalog[pmap[i][j].layers[layerWithTMFlag(i, j, TM_LIST_IN_SIDEBAR)]].description;
  ...
  for (i = 0; i < displayEntityCount && printY < ROWS - 1; i++)
  {  // Bottom line is reserved for the depth.
    oldPrintY = printY;
    if (entityType[i] == EDT_CREATURE)
    {
      x = ((Creature*)entityList[i])->xLoc;
      y = ((Creature*)entityList[i])->yLoc;
      printY = printMonsterInfo((Creature*)entityList[i], printY, (focusEntity && (x != focusX || y != focusY)),
                                (x == focusX && y == focusY));
    }
    else if (entityType[i] == EDT_ITEM)
    {
      ...
      printY = printItemInfo((Item*)entityList[i], printY, (focusEntity && (x != focusX || y != focusY)),
                             (x == focusX && y == focusY));
    }
    else if (entityType[i] == EDT_TERRAIN)
    {
      ...
      printY = printTerrainInfo(x, y, printY, ((const char*)entityList[i]),
                                (focusEntity && (x != focusX || y != focusY)), (x == focusX && y == focusY));
    }
...
void breakUpLongWordsIn(char* sourceText, int width, bool useHyphens)
{
  char buf[COLS * ROWS * 2] = "";
  int i, m, nextChar, wordWidth;
  // const int maxLength = useHyphens ? width - 1 : width;
...
{
  int i, w, textLength, lineCount;
  char printString[COLS * ROWS * 2];
  int spaceLeftOnLine, wordWidth;
                              ...
                              cellDisplayBuffer dbuf[COLS][ROWS])
{
  ...
  for (i = 0; printString[i] != '\0'; i++)
  {
    if (printString[i] == '\n')
    {
      ...
      if (py < ROWS - 1)
      {        // don't advance below the bottom of the screen
        py++;  // next line
      }
      else
      {
...
void printHelpScreen()
{
  ...
  char helpText[BROGUE_HELP_LINE_COUNT][DCOLS * 3] = { "",
                                                       ...
                                                       "          -- Commands --",
                                                       "",
                                                       "         mouse  ****move cursor (including to examine monsters "
                                                       "and terrain)",
                                                       "         click  ****travel",
                                                       " control-click  ****advance one space",
                                                       "      <return>  ****enable keyboard cursor control",
                                                       "   <space/esc>  ****disable keyboard cursor control",
                                                       "hjklyubn, arrow keys, or numpad  ****move or attack (control "
                                                       "or shift to run)",
                                                       "",
                                                       " a/e/r/t/d/c/R  ****apply/equip/remove/throw/drop/call/relabel "
                                                       "an item",
                                                       "i, right-click  ****view inventory",
                                                       "             D  ****list discovered items",
                                                       "",
                                                       "             z  ****rest once",
                                                       "             Z  ****rest for 100 turns or until something "
                                                       "happens",
...
void printDiscoveries(int category, int count, unsigned int itemCharacter, int x, int y,
                      cellDisplayBuffer dbuf[COLS][ROWS])
{
  Color *theColor, goodColor, badColor;
  char buf[COLS], buf2[COLS];
  int i, magic, totalFrequency;
  ItemTable* theTable = tableForItemCategory(category, nullptr);
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (!theTable[i].identified && theTable[i].frequency > 0 && totalFrequency > 0)
    {
...
// int createDiscoveriesButtons(int category, int count, unsigned int itemCharacter, int x, int y,
// brogueButton *buttons) { 	color *theColor, goodColor, badColor; 	char whiteColorEscape[20] = "", darkGrayColorEscape[20]
//= "", yellowColorEscape[20] = "", goodColorEscape[20] = "", badColorEscape[20] = ""; 	int i, x2, magic, symbolCount;
...
//
// void printDiscoveriesScreen() {
//	int i, j, y, buttonCount;
//	cellDisplayBuffer dbuf[COLS][ROWS], rbuf[COLS][ROWS];
//	brogueButton buttons[NUMBER_SCROLL_KINDS + NUMBER_WAND_KINDS + NUMBER_POTION_KINDS + NUMBER_STAFF_KINDS +
//NUMBER_RING_KINDS] = {{{0}}};
//
//	clearDisplayBuffer(dbuf);
//	buttonCount = 0;
...
//	buttonCount += createDiscoveriesButtons(SCROLL, NUMBER_SCROLL_KINDS, SCROLL_CHAR, mapToWindowX(3), ++y,
//&(buttons[buttonCount]));
//
//	printString("-- WANDS --", mapToWindowX(3), y += NUMBER_SCROLL_KINDS + 1, &flavorTextColor, &black, dbuf);
//	buttonCount += createDiscoveriesButtons(WAND, NUMBER_WAND_KINDS, WAND_CHAR, mapToWindowX(3), ++y,
//&(buttons[buttonCount]));
//
//	printString("-- POTIONS --", mapToWindowX(29), y = mapToWindowY(1), &flavorTextColor, &black, dbuf);
//	buttonCount += createDiscoveriesButtons(POTION, NUMBER_POTION_KINDS, POTION_CHAR, mapToWindowX(29), ++y,
//&(buttons[buttonCount]));
//
//	printString("-- STAFFS --", mapToWindowX(54), y = mapToWindowY(1), &flavorTextColor, &black, dbuf);
//	buttonCount += createDiscoveriesButtons(STAFF, NUMBER_STAFF_KINDS, STAFF_CHAR, mapToWindowX(54), ++y,
//&(buttons[buttonCount]));
//
//	printString("-- RINGS --", mapToWindowX(54), y += NUMBER_STAFF_KINDS + 1, &flavorTextColor, &black, dbuf);
//	buttonCount += createDiscoveriesButtons(RING, NUMBER_RING_KINDS, RING_CHAR, mapToWindowX(54), ++y,
//&(buttons[buttonCount]));
//
//	for (i=0; i<COLS; i++) {
//		for (j=0; j<ROWS; j++) {
...
{
  int i, hiliteLineNum, maxLength = 0, leftOffset;
  ...
  printString(
      KEYBOARD_LABELS ? "Press space to continue." : "Touch anywhere to continue.",
...
{
  int i, j, score, topRange, bottomRange;
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (cellHasTerrainFlag(i, j, T_WAYPOINT_BLOCKER) || (map[i][j] == map[0][0]) ||
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY | T_LAVA_INSTA_DEATH) || (map[i][j] == map[0][0]) ||
          (i == player.xLoc && j == player.yLoc))
      {
        continue;
      }
                      ...
                      bool dim)
{
  ...
  if (y >= ROWS - 1)
  {  // don't write over the depth number
    return;
  }
...
void highlightScreenCell(int x, int y, Color* highlightColor, int strength)
{
  Color tempColor;
...
int printMonsterInfo(Creature* monst, int y, bool dim, bool highlight)
{
  ...
  int initialY, i, j, highlightStrength, displayedArmor, percent;
  bool inPath;
  int oldRNG;

  const char hallucinationStrings[14][COLS] = {
    "     (Dancing)      ", "     (Singing)      ", "  (Pontificating)   ", "     (Skipping)     ",
    "     (Spinning)     ", "      (Crying)      ", "     (Laughing)     ", "     (Humming)      ",
    "    (Whistling)     ", "    (Quivering)     ", "    (Muttering)     ", "    (Gibbering)     ",
  ...
  // mutation, if any
  if (y < ROWS - 1 && monst->mutationIndex >= 0 && (!player.status[STATUS_HALLUCINATING] || rogue.playbackOmniscience))
  {
    strcpy(buf, "                    ");
  ...
  if (monst == &player)
  {
    ...
    {
      printString("      STARVING      ", 0, y++, &badMessageColor, &black, nullptr);
    }
  }
  ...
  if (!player.status[STATUS_HALLUCINATING] || rogue.playbackOmniscience || monst == &player)
  {
    for (i = 0; i < NUMBER_OF_STATUS_EFFECTS; i++)
    {
      ...
      {
        printProgressBar(0, y++, (monst == &player ? "Levitating" : "Flying"), monst->status[i], monst->maxStatus[i],
                         &redBar, dim);
      }
      else if (i == STATUS_POISONED && monst->status[i] > 0)
      {
        ...
        {
          sprintf(buf2, "%s (x%i)", buf, monst->poisonAmount);
    ...
    {
      printProgressBar(0, y++, monsterText[monst->info.monsterID].absorbStatus, monst->corpseAbsorptionCounter, 20,
                       &redBar, dim);
    }
  }

  if (monst != &player && (!(monst->info.flags & MONST_INANIMATE) || monst->creatureState == MONSTER_ALLY))
  {
    if (y < ROWS - 1)
    {
      ...
      else if ((monst->creatureState == MONSTER_WANDERING) && y < ROWS - 1)
      {
        ...
        else if ((monst->bookkeepingFlags & MB_FOLLOWER) && monst->leader &&
  ...
  else if (monst == &player)
  {
    if (y < ROWS - 1)
    {
      ...
      {
        sprintf(buf, "Str: %s%i%s  Armor: %i", tempColorEscape, rogue.strength - player.weaknessAmount, grayColorEscape,
                displayedArmor);
      }
      else
      {
        sprintf(buf, "Str: %s%i%s  Armor: %i?", tempColorEscape, rogue.strength - player.weaknessAmount,
                grayColorEscape, estimatedArmorValue());
      }
      // buf[20] = '\0';
    ...
    if (y < ROWS - 1)
    {
      ...
      encodeMessageColor(tempColorEscape, 0, &tempColor);
      encodeMessageColor(grayColorEscape, 0, (dim ? &darkGray : &gray));
      sprintf(buf, "%sStealth range: %i%s", tempColorEscape, rogue.aggroRange, grayColorEscape);
      printString("                    ", 0, y, &white, &black, 0);
...
{
  const unsigned int itemCats[10] = { FOOD, WEAPON, ARMOR, POTION, SCROLL, STAFF, WAND, RING, CHARM, GOLD };
  int cat, kind, maxKinds;
  assureCosmeticRNG;
...
int printItemInfo(Item* theItem, int y, bool dim, bool highlight)
{
  ...
  int initialY, i, j, highlightStrength, lineCount;
  bool inPath;
  int oldRNG;
  ...
  if (y < ROWS - 1)
  {
    ...
    upperCase(name);
    lineCount = wrapText(nullptr, name, 20 - 3);
    for (i = initialY + 1; i <= initialY + lineCount + 1 && i < ROWS - 1; i++)
    {
      printString("                    ", 0, i, (dim ? &darkGray : &gray), &black, 0);
    }
    y = printStringWithWrapping(name, 3, y, 20 - 3, (dim ? &gray : &white), &black, nullptr);  // Advances y.
  }
...
int printTerrainInfo(int x, int y, int py, const char* description, bool dim, bool highlight)
{
  ...
  if (py < ROWS - 1)
  {
    ...
    py = printStringWithWrapping(name, 3, py, 20 - 3, &textColor, &black, nullptr);  // Advances y.
  }
                   ...
                   cellDisplayBuffer rbuf[COLS][ROWS], BrogueButton* buttons, int buttonCount)
{
  ...
  int x2, y2, lineCount, i, bx, by, padLines;
  ...
  while (((lineCount = wrapText(nullptr, textBuf, width)) + y2) >= ROWS - 2 && width < COLS - 5)
  {
  ...
  if (buttonCount > 0)
  {
    return buttonInputLoop(buttons, buttonCount, x2, y2, width, by - y2 + 1 + padLines, nullptr);
  }
  else
  {
    return -1;
...
void printMonsterDetails(Creature* monst, cellDisplayBuffer rbuf[COLS][ROWS])
{
  ...
  printTextBox(textBuf, monst->xLoc, 0, 0, &white, &black, rbuf, nullptr, 0);
}
...
void printFloorItemDetails(Item* theItem, cellDisplayBuffer rbuf[COLS][ROWS])
{
  ...
  itemDetails(textBuf, theItem);
  printTextBox(textBuf, theItem->xLoc, 0, 0, &white, &black, rbuf, nullptr, 0);
}
