...
int randClumpedRange(int lowerBound, int upperBound, int clumpFactor)
{
  ...
  int i, total = 0, numSides = (upperBound - lowerBound) / clumpFactor;
...
int rand_range(int lowerBound, int upperBound)
{
  ...
  if (rogue.RNG == RNG_SUBSTANTIVE)
  {
    ...
    if (1)
    {  // randomNumbersGenerated >= 1128397) {
...
unsigned long seedRandomGenerator(unsigned long seed)
{
  if (seed == 0)
  {
    seed = (unsigned long)time(nullptr) - 1352700000;
  }
