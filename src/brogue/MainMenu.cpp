...
void drawMenuFlames(signed int flames[COLS][(ROWS + MENU_FLAME_ROW_PADDING)][3], unsigned char mask[COLS][ROWS])
{
  int i, j, versionStringLength;
  Color tempColor = { 0 };
...
void updateMenuFlames(const Color* colors[COLS][(ROWS + MENU_FLAME_ROW_PADDING)],
                      signed int colorSources[MENU_FLAME_COLOR_SOURCE_COUNT][4],
                      signed int flames[COLS][(ROWS + MENU_FLAME_ROW_PADDING)][3])
{
  int i, j, k, l, x, y;
  signed int tempFlames[COLS][3];
  int colorSourceNumber, rand;
  ...
  for (j = 0; j < (ROWS + MENU_FLAME_ROW_PADDING); j++)
  {
    ...
    for (i = 0; i < COLS; i++)
    {
      // Each cell is the weighted average of the three color values below and
      // itself. Weight of itself: 100 Weight of left and right neighbors:
      // MENU_FLAME_SPREAD_SPEED / 2 each Weight of below cell:
      // MENU_FLAME_RISE_SPEED Divisor: 100 + MENU_FLAME_SPREAD_SPEED +
      ...
      if (colors[i][j])
      {
        ...
        flames[i][j][1] +=
            (colors[i][j]->green + (colors[i][j]->greenRand * colorSources[colorSourceNumber][2] / 1000) + rand) *
            MENU_FLAME_PRECISION_FACTOR;
        flames[i][j][2] +=
            (colors[i][j]->blue + (colors[i][j]->blueRand * colorSources[colorSourceNumber][3] / 1000) + rand) *
            MENU_FLAME_PRECISION_FACTOR;
...
// Takes a grid of values, each of which is 0 or 100, and fills in some middle
// values in the interstices.
void antiAlias(unsigned char mask[COLS][ROWS])
{
  int i, j, x, y, dir, nbCount;
  const int intensity[5] = { 0, 0, 35, 50, 60 };
...
void initializeMenuFlames(bool includeTitle, const Color* colors[COLS][(ROWS + MENU_FLAME_ROW_PADDING)],
                          Color colorStorage[COLS], signed int colorSources[MENU_FLAME_COLOR_SOURCE_COUNT][4],
                          signed int flames[COLS][(ROWS + MENU_FLAME_ROW_PADDING)][3], unsigned char mask[COLS][ROWS])
{
  int i, j, k, colorSourceCount;
  ...
  if (includeTitle)
  {
    ...
    for (i = 0; i < MENU_TITLE_WIDTH; i++)
    {
      for (j = 0; j < MENU_TITLE_HEIGHT; j++)
      {
        if (title[j][i] != ' ')
        {
          colors[(COLS - MENU_TITLE_WIDTH) / 2 + i + MENU_TITLE_OFFSET_X]
                [(ROWS - MENU_TITLE_HEIGHT) / 2 + j + MENU_TITLE_OFFSET_Y] = &flameTitleColor;
          colorSourceCount++;
          mask[(COLS - MENU_TITLE_WIDTH) / 2 + i + MENU_TITLE_OFFSET_X]
              [(ROWS - MENU_TITLE_HEIGHT) / 2 + j + MENU_TITLE_OFFSET_Y] = 100;
        }
      }
    }
...
void titleMenu()
{
  signed int flames[COLS][(ROWS + MENU_FLAME_ROW_PADDING)][3];  // red, green and blue
  signed int colorSources[MENU_FLAME_COLOR_SOURCE_COUNT][4];  // red, green, blue, and rand, one for each color source
                                                                // (no more than MENU_FLAME_COLOR_SOURCE_COUNT).
  ...
  // Empty nextGamePath and nextGameSeed so that the buttons don't try to load
  // an old game path or seed.
  ...
  do
  {
    ...
    overlayDisplayBuffer(shadowBuf, nullptr);
    ...
    if (pauseBrogue(MENU_FLAME_UPDATE_DELAY))
    {
      ...
      // Process the input.
      button = processButtonInput(&state, nullptr, &theEvent);
    }
  ...
  if (button != -1)
  {
    ...
    {
      // Should fix an issue with Linux/Windows ports that require moving the
      // mouse after pressing control to get the button to change.
...
#define FILES_ON_PAGE_MAX                                                                                              \
  (min(26, ROWS - 7))  // Two rows (top and bottom) for flames, two rows for
                       // border, one for prompt, one for heading.
#define MAX_FILENAME_DISPLAY_LENGTH 53
bool dialogChooseFile(char* path, const char* suffix, const char* prompt)
{
  int i, j, count, x, y, width, height, suffixLength, pathLength, maxPathLength, currentPageStart;
  ...
  // First, we want to filter the list by stripping out any filenames that do
  // not end with suffix. i is the entry we're testing, and j is the entry that
  // we move it to if it qualifies.
  for (i = 0, j = 0; i < count; i++)
  {
    pathLength = strlen(files[i].path);
    // printf("\nString 1: %s", &(files[i].path[(max(0, pathLength -
    // suffixLength))]));
    if (stringsExactlyMatch(&(files[i].path[(max(0, pathLength - suffixLength))]), suffix))
    {
      ...
      {
        files[j] = files[i];
        // printf("\nMatching file: %s\twith date: %s", files[j].path,
        // files[j].date);
      }
      j++;
  ...
  {  // Repeat to permit scrolling.
    ...
    for (i = 0; i < min(count - currentPageStart, FILES_ON_PAGE_MAX); i++)
    {
      ...
      // printf("\nFound file: %s, with date: %s",
      // files[currentPageStart+i].path, files[currentPageStart+i].date);
    }
    ...
    if (count)
    {
      ...
      //			for (j=0; j<min(count - currentPageStart,
      // FILES_ON_PAGE_MAX); j++) { 				printf("\nSanity check BEFORE: %s, with date:
      //%s", files[currentPageStart+j].path, files[currentPageStart+j].date);
      //				printf("\n   (button name)Sanity check
      // BEFORE: %s", buttons[j].text);
      //			}

      i = buttonInputLoop(buttons,
                          min(count - currentPageStart, FILES_ON_PAGE_MAX) + (count > FILES_ON_PAGE_MAX ? 2 : 0), x, y,
                          width, height, nullptr);

      //			for (j=0; j<min(count - currentPageStart,
      // FILES_ON_PAGE_MAX); j++) { 				printf("\nSanity check AFTER: %s, with date:
      //%s", files[currentPageStart+j].path, files[currentPageStart+j].date);
      //				printf("\n   (button name)Sanity check
      // AFTER: %s", buttons[j].text);
      //			}
...
void scum(unsigned long startingSeed, int numberOfSeedsToScan, int scanThroughDepth)
{
...
the first %i depths will, of course, make the game significantly easier.",
  ...
  for (theSeed = startingSeed; theSeed < startingSeed + numberOfSeedsToScan; theSeed++)
  {
    ...
    {
      startLevel(rogue.depthLevel == 1 ? 1 : rogue.depthLevel - 1,
                 1);  // descending into level n
    ...
    freeEverything();
    remove(currentFilePath);  // Don't add a spurious LastGame file to the brogue
                              // folder.
  }
  fclose(logFile);
}
...
// we'll do it. The path (rogue.nextGamePath) is essentially a parameter for
// this command, and tells NG_VIEW_RECORDING and NG_OPEN_GAME which file to
// open. If there is a command but no accompanying path, and it's a command that
// should take a path, then pop up a dialog to have the player specify a path.
// If there is no command (i.e. if rogue.nextGame contains NG_NOTHING), then
// we'll display the title screen so the player can choose.
      ...
      case NG_NEW_GAME_WITH_SEED:
        ...
        if (rogue.nextGame == NG_NEW_GAME_WITH_SEED)
        {
          ...
          {  // Prompt for seed; default is the previous game's seed.
            ...
            if (getInputTextString(buf, "Generate dungeon with seed number:", log10(ULONG_MAX) + 1, seedDefault, "",
                                   TEXT_INPUT_NUMBERS, true) &&
                buf[0] != '\0')
            {
              seedTooBig = false;
