...
{
  const uchar ucharTable[] = { UP_ARROW,   LEFT_ARROW, DOWN_ARROW, RIGHT_ARROW, ESCAPE_KEY, RETURN_KEY, ENTER_KEY,
                               DELETE_KEY, TAB_KEY,    NUMPAD_0,   NUMPAD_1,    NUMPAD_2,   NUMPAD_3,   NUMPAD_4,
...
void numberToString(unsigned long number, int numberOfBytes, unsigned char* recordTo)
{
  int i;
  unsigned long n;
...
void recordNumber(unsigned long number, int numberOfBytes)
{
  int i;
  unsigned char c[10];
...
unsigned long recallNumber(int numberOfBytes)
{
  int i;
  unsigned long n;
...
#define OOS_APOLOGY                                                                                                    \
  "Playback of the recording has diverged from the originally recorded game.\n\n\
...
void playbackPanic()
{
  ...
  if (!rogue.playbackOOS)
  {
    ...
    printTextBox(OOS_APOLOGY, 0, 0, 0, &white, &black, rbuf, nullptr, 0);
...
void loadNextAnnotation()
{
  ...
  for (;;)
  {
        ...
        (currentReadTurn <= 1 && rogue.playerTurnNumber <= 1 && currentReadTurn >= rogue.playerTurnNumber))
    {
      ...
      {
        if (rogue.nextAnnotation[i] < ' ' || rogue.nextAnnotation[i] > '~')
        {
...
void displayAnnotation()
{
  ...
  if (rogue.playbackMode && rogue.playerTurnNumber == rogue.nextAnnotationTurn)
  {
    if (!rogue.playbackFastForward)
    {
      refreshSideBar(-1, -1, false);

      printTextBox(rogue.nextAnnotation, player.xLoc, 0, 0, &black, &white, rbuf, nullptr, 0);
...
void initRecording()
{
  ...
  if (rogue.playbackMode)
  {
    ...
    if (strcmp(versionString, BROGUE_VERSION_STRING))
    {
      rogue.playbackMode = false;
      rogue.playbackFastForward = false;
      sprintf(buf, "This file is from version %s and cannot be opened in version %s.", versionString,
...
void printPlaybackHelpScreen()
{
  ...
  char helpText[PLAYBACK_HELP_LINE_COUNT][80] = { "Commands:",
                                                  "",
                                                  ...
                                                  "l or right arrow: ****advance one turn (shift for 5 turns; control "
                                                  "for 20)",
                                                  "",
...
void advanceToLocation(unsigned long destinationFrame)
{
  ...
  while (rogue.playerTurnNumber < destinationFrame && !rogue.gameHasEnded && !rogue.playbackOOS)
  {
    ...
    {
      rogue.playbackFastForward = false;
      printProgressBar((COLS - 20) / 2, ROWS / 2, "[     Loading...   ]", rogue.playerTurnNumber - initialFrameNumber,
...
void promptToAdvanceToLocation(int keystroke)
{
  ...
  if (!rogue.playbackPaused || unpause())
  {
    ...
    rogue.playbackMode = false;
    enteredText =
        getInputTextString(entryText, "Go to turn number: ", log10(ULONG_MAX) - 1, buf, "", TEXT_INPUT_NUMBERS, false);
    confirmMessages();
    rogue.playbackMode = true;
...
void pausePlayback()
{
  ...
  {
    rogue.playbackPaused = true;
    messageWithColor(KEYBOARD_LABELS ? "recording paused. Press space to play." : "recording paused.", &teal, false);
    refreshSideBar(-1, -1, false);
    oldRNG = rogue.RNG;
    // rogue.RNG = RNG_SUBSTANTIVE;
    mainInputLoop();
...
{
  signed long key;
  int newDelay, frameCount, x, y, previousDeepestLevel;
  unsigned long destinationFrame;
      ...
      case DESCEND_KEY:
        ...
        if (!rogue.playbackPaused || unpause())
        {
          if ((unsigned long)rogue.deepestLevel < maxLevelChanges)
          {
            ...
            pauseBrogue(5);
            rogue.playbackFastForward = true;
            while ((rogue.deepestLevel <= previousDeepestLevel || !rogue.playbackBetweenTurns) && !rogue.gameHasEnded)
            {
              rogue.RNG = RNG_COSMETIC;  // dancing terrain colors can't influence recordings
              nextBrogueEvent(&theEvent, false, true, false);
              rogue.RNG = RNG_SUBSTANTIVE;
...
void saveGame()
{
  ...
  {
    askAgain = false;
    if (getInputTextString(filePath, "Save game as (<esc> to cancel): ", BROGUE_FILENAME_MAX - strlen(GAME_SUFFIX),
                           defaultPath, GAME_SUFFIX, TEXT_INPUT_FILENAME, false))
    {
...
void saveRecording()
{
  ...
  {
    askAgain = false;
    if (getInputTextString(filePath,
                           "Save recording as (<esc> to cancel): ", BROGUE_FILENAME_MAX - strlen(RECORDING_SUFFIX),
                           defaultPath, RECORDING_SUFFIX, TEXT_INPUT_FILENAME, false))
    {
    ...
    {  // Declined to save recording; save it anyway as LastRecording, and delete LastRecording if it already exists.
...
void loadSavedGame()
{
  ...
  if (rogue.howManyTurns > 0)
  {
    ...
    while (recordingLocation < lengthOfPlaybackFile && rogue.playerTurnNumber < rogue.howManyTurns &&
           !rogue.gameHasEnded && !rogue.playbackOOS)
    {
      ...
      if (!(recordingLocation % progressBarInterval) && !rogue.playbackOOS)
      {
        ...
        printProgressBar((COLS - 20) / 2, ROWS / 2, "[     Loading...   ]", recordingLocation, lengthOfPlaybackFile,
                         &darkPurple, false);
        pauseBrogue(1);
        rogue.playbackFastForward = true;
      }
...
void describeKeystroke(unsigned char key, char* description)
{
  int i;
  uchar c;
  const uchar ucharList[52] = { UP_KEY, DOWN_KEY, LEFT_KEY, RIGHT_KEY, UP_ARROW, LEFT_ARROW, DOWN_ARROW, RIGHT_ARROW,
                                UPLEFT_KEY, UPRIGHT_KEY, DOWNLEFT_KEY, DOWNRIGHT_KEY, DESCEND_KEY, ASCEND_KEY, REST_KEY,
                                AUTO_REST_KEY, SEARCH_KEY, INVENTORY_KEY, ACKNOWLEDGE_KEY, EQUIP_KEY, UNEQUIP_KEY,
                                APPLY_KEY, THROW_KEY, RELABEL_KEY, DROP_KEY, CALL_KEY,
                                // FIGHT_KEY, FIGHT_TO_DEATH_KEY,
                                HELP_KEY, DISCOVERIES_KEY, RETURN_KEY, EXPLORE_KEY, AUTOPLAY_KEY, SEED_KEY,
                                EASY_MODE_KEY, ESCAPE_KEY, RETURN_KEY, ENTER_KEY, DELETE_KEY, TAB_KEY, PERIOD_KEY,
                                VIEW_RECORDING_KEY, NUMPAD_0, NUMPAD_1, NUMPAD_2, NUMPAD_3, NUMPAD_4, NUMPAD_5,
                                NUMPAD_6, NUMPAD_7, NUMPAD_8, NUMPAD_9, UNKNOWN_KEY };
  const char descList[53][30] = { "up", "down", "left", "right", "up arrow", "left arrow", "down arrow", "right arrow",
                                  "upleft", "upright", "downleft", "downright", "descend", "ascend", "rest",
                                  "auto rest", "search", "inventory", "acknowledge", "equip", "unequip", "apply",
                                  "throw", "relabel", "drop", "call",
                                  //"fight", "fight to death",
                                  "help", "discoveries", "repeat travel", "explore", "autoplay", "seed", "easy mode",
                                  "escape", "return", "enter", "delete", "tab", "period", "open file", "numpad 0",
                                  "numpad 1", "numpad 2", "numpad 3", "numpad 4", "numpad 5", "numpad 6", "numpad 7",
                                  "numpad 8", "numpad 9", "unknown", "ERROR" };
...
void parseFile()
{
  ...
  if (selectFile("Parse recording: ", "Recording.broguerec", ""))
  {
    ...
    fprintf(descriptionFile,
            ...
            "%li\n\tFile length: %li\n",
            currentFilePath, versionString, seed, numTurns, numDepths, fileLength);
    for (i = 0; recordingLocation < fileLength; i++)
    {
      ...
      fprintf(descriptionFile, "\nEvent %li, loc %li, length %li:%s\t%s", i, startLoc, recordingLocation - startLoc,
              (i < 10 ? " " : ""), description);
    }
