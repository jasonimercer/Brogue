...
{
  int i, j, k;
  int colorComponents[3], randComponent, lightMultiplier;
  int fadeToPercent;
  double radius;
  ...
  for (i = max(0, x - (radius + FLOAT_FUDGE)); i < DCOLS && i < x + radius; i++)
  {
    for (j = max(0, y - (radius + FLOAT_FUDGE)); j < DROWS && j < y + radius; j++)
    {
      ...
      {
        lightMultiplier =
            100 - (100 - fadeToPercent) * (sqrt((i - x) * (i - x) + (j - y) * (j - y)) / radius + FLOAT_FUDGE);
        for (k = 0; k < 3; k++)
        {
...
void updateMinersLightRadius()
{
  ...
  rogue.minersLight.lightRadius.upperBound = rogue.minersLight.lightRadius.lowerBound =
      clamp(lightRadius + FLOAT_FUDGE, -30000, 30000);
}
...
{
  int lights[DCOLS][DROWS][3];
  bool inView, fastForward, atLeastOneFlareStillActive;
  int i;  // i iterates through the flare list
