...
{
  int i, textLoc, width, midPercent, symbolNumber, opacity, oldRNG;
...
int processButtonInput(ButtonState* state, bool* canceled, RogueEvent* event)
{
  ...
  if (event->eventType == MOUSE_DOWN || event->eventType == MOUSE_UP || event->eventType == MOUSE_ENTERED_CELL)
  {
    ...
    for (i = 0; i < state->buttonCount; i++)
    {
      if ((state->buttons[i].flags & B_DRAW) && (state->buttons[i].flags & B_ENABLED) &&
          (state->buttons[i].y == y ||
           ((state->buttons[i].flags & B_WIDE_CLICK_AREA) && abs(state->buttons[i].y - y) <= 1)) &&
          x >= state->buttons[i].x && x < state->buttons[i].x + strLenWithoutEscapes(state->buttons[i].text))
      {
...
{
  int x, y, button;  // (x, y) keeps track of the mouse location
  bool canceled;
  RogueEvent theEvent;
  ButtonState state = { 0 };
