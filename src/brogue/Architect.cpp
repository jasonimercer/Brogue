...
int topBlobMinX, topBlobMinY, blobWidth, blobHeight;
...
bool checkLoopiness(int x, int y)
{
  bool inString;
  int newX, newY, dir, sdir;
  int numStrings, maxStringLength, currentStringLength;
  ...
  // starting on this unloopy neighbor, work clockwise and count up (a) the
  // number of strings of loopy neighbors, and (b) the length of the longest
  // such string.
...
void auditLoop(int x, int y, char grid[DCOLS][DROWS])
{
  int dir, newX, newY;
  if (coordinatesAreInMap(x, y) && !grid[x][y] && !(pmap[x][y].flags & IN_LOOP))
  {
    grid[x][y] = true;
    for (dir = 0; dir < DIRECTION_COUNT; dir++)
    {
...
// Assumes it is called with respect to a passable (startX, startY), and that
// the same is not already included in results. Returns 10000 if the area
// included an area machine.
int floodFillCount(char results[DCOLS][DROWS], char passMap[DCOLS][DROWS], int startX, int startY)
{
  int dir, newX, newY, count;
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && passMap[newX][newY] && !results[newX][newY])
    {
...
// Rotates around the cell, counting up the number of distinct strings of
// passable neighbors in a single revolution.
//		Zero means there are no impassable tiles adjacent.
//		One means it is adjacent to a wall.
//		Two means it is in a hallway or something similar.
//		Three means it is the center of a T-intersection or something
// similar. 		Four means it is in the intersection of two hallways. 		Five or more
// means there is a bug.
int passableArcCount(int x, int y)
{
  int arcCount, dir, oldX, oldY, newX, newY;
  ...
  for (dir = 0; dir < DIRECTION_COUNT; dir++)
  {
    ...
    newX = x + cDirs[dir][0];
    newY = y + cDirs[dir][1];
    // Counts every transition from passable to impassable or vice-versa on the
    // way around the cell:
  ...
  return arcCount / 2;  // Since we added one when we entered a wall and another
                        // when we left.
}
...
{
  int i, j, i2, j2, dir, newX, newY, oldX, oldY, passableArcCount, cellCount;
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && !cellHasTMFlag(i, j, TM_IS_SECRET))
      {
    ...
    for (j = 0; j < DROWS; j++)
    {
      if (pmap[i][j].flags & IN_LOOP)
      {
        ...
        for (dir = 0; dir < DIRECTION_COUNT; dir++)
        {
          newX = i + nbDirs[dir][0];
          newY = j + nbDirs[dir][1];
          if (coordinatesAreInMap(newX, newY) && !grid[newX][newY] && !(pmap[newX][newY].flags & IN_LOOP))
          {
  ...
  if (calculateChokeMap)
  {
    ...
    // The chokepoint map is a number for each passable tile. If the tile is a
    // chokepoint, then the number indicates the number of tiles that would be
    // rendered unreachable if the chokepoint were blocked. If the tile is not a
    // chokepoint, then the number indicates the number of tiles that would be
    // rendered unreachable if the nearest exit chokepoint were blocked. The
    // cost of all of this is one depth-first flood-fill per open point that is
    // adjacent to a chokepoint.

    // Start by setting the chokepoint values really high, and roping off room
    // machines.
    for (i = 0; i < DCOLS; i++)
    {
      for (j = 0; j < DROWS; j++)
      ...
      for (j = 0; j < DROWS; j++)
      {
        if (passMap[i][j] && (pmap[i][j].flags & IS_CHOKEPOINT))
        {
          for (dir = 0; dir < 4; dir++)
          {
            newX = i + nbDirs[dir][0];
            newY = j + nbDirs[dir][1];
            if (coordinatesAreInMap(newX, newY) && passMap[newX][newY] && !(pmap[newX][newY].flags & IS_CHOKEPOINT))
            {
              // OK, (newX, newY) is an open point and (i, j) is a chokepoint.
              // Pretend (i, j) is blocked by changing passMap, and run a
              // flood-fill cell count starting on (newX, newY). Keep track of
              // the flooded region in grid[][].
              zeroOutGrid(grid);
              passMap[i][j] = false;
              cellCount = floodFillCount(grid, passMap, newX, newY);
              passMap[i][j] = true;

              // CellCount is the size of the region that would be obstructed if
              // the chokepoint were blocked. CellCounts less than 4 are not
              // useful, so we skip those cases.

              if (cellCount >= 4)
              {
                // Now, on the chokemap, all of those flooded cells should take
                // the lesser of their current value or this resultant number.
                ...
                // The chokepoint itself should also take the lesser of its
                // current value or the flood count.
...
void addLoops(int** grid, int minimumPathingDistance)
{
  int newX, newY, oppX, oppY;
  int **pathMap, **costMap;
  int i, d, x, y, sCoord[DCOLS * DROWS];
  const int dirCoords[2][2] = { { 1, 0 }, { 0, 1 } };
  ...
  for (i = 0; i < DCOLS * DROWS; i++)
  {
    ...
    if (!grid[x][y])
    {
      ...
      {  // Try a horizontal door, and then a vertical door.
        ...
        if (coordinatesAreInMap(newX, newY) && coordinatesAreInMap(oppX, oppY) && grid[newX][newY] > 0 &&
            grid[oppX][oppY] > 0)
        {  // If the tile being inspected has floor on both sides,
          ...
          {                     // and if the pathing distance between
                                // the two flanking floor tiles exceeds
...
// Returns true if everything went well, and false if we ran into a machine
// component that was already there, as we don't want to build a machine around
// it.
bool addTileToMachineInteriorAndIterate(char interior[DCOLS][DROWS], int startX, int startY)
{
  int dir, newX, newY;
  bool goodSoFar = true;
  ...
  for (dir = 0; dir < 4 && goodSoFar; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY))
    {
          ...
          ((pmap[newX][newY].flags & IS_IN_MACHINE) && !(pmap[newX][newY].flags & IS_GATE_SITE)))
      {
        ...
        // Items haven't been populated yet, so the only way this could happen
        // is if another machine previously placed an item here. Also abort if
        // we're touching another machine at any point other than a gate tile.
        return false;
      }
      if (!interior[newX][newY] && chokeMap[newX][newY] <= chokeMap[startX][startY]  // don't have to worry about walls
                                                                                     // since they're all 30000
          && !(pmap[newX][newY].flags & IS_IN_MACHINE))
      {
        // goodSoFar = goodSoFar && addTileToMachineInteriorAndIterate(interior,
        // newX, newY);
        if (goodSoFar)
        {
...
bool itemIsADuplicate(Item* theItem, Item** spawnedItems, int itemCount)
{
  int i;
  if (theItem->category & (STAFF | WAND | POTION | SCROLL | RING | WEAPON | ARMOR | CHARM))
  {
    ...
    {
      if (spawnedItems[i]->category == theItem->category && spawnedItems[i]->kind == theItem->kind)
      {
        return true;
      }
    }
...
bool blueprintQualifies(int i, unsigned long requiredMachineFlags)
{
...
void abortItemsAndMonsters(Item* spawnedItems[MACHINES_BUFFER_LENGTH],
                           Creature* spawnedMonsters[MACHINES_BUFFER_LENGTH])
{
  ...
  for (i = 0; i < MACHINES_BUFFER_LENGTH && spawnedItems[i]; i++)
  {
    ...
    removeItemFromChain(spawnedItems[i],
                        packItems);  // just in case; can't imagine why this would arise.
    for (j = 0; j < MACHINES_BUFFER_LENGTH && spawnedMonsters[j]; j++)
    {
      // Remove the item from spawned monsters, so it doesn't get double-freed
      // when the creature is killed below.
      if (spawnedMonsters[j]->carriedItem == spawnedItems[i])
      {
        spawnedMonsters[j]->carriedItem = nullptr;
        break;
      }
    }
...
bool cellIsFeatureCandidate(int x, int y, int originX, int originY, int distanceBound[2],
                               char interior[DCOLS][DROWS], char occupied[DCOLS][DROWS], char viewMap[DCOLS][DROWS],
                               int** distanceMap, int machineNumber, unsigned long featureFlags,
                               unsigned long bpFlags)
{
  int newX, newY, dir, distance;

  // No building in the hallway if it's prohibited.
  // This check comes before the origin check, so an area machine will fail
  // altogether if its origin is in a hallway and the feature that must be built
  // there does not permit as much.
  if ((featureFlags & MF_NOT_IN_HALLWAY) && passableArcCount(x, y) > 1)
  {
    return false;
  }

  // No building along the perimeter of the level if it's prohibited.
  if ((featureFlags & MF_NOT_ON_LEVEL_PERIMETER) && (x == 0 || x == DCOLS - 1 || y == 0 || y == DROWS - 1))
  {
    return false;
  }

  // The origin is a candidate if the feature is flagged to be built at the
  // origin. If it's a room, the origin (i.e. doorway) is otherwise NOT a
  // candidate.
  ...
  if ((featureFlags & (MF_IN_VIEW_OF_ORIGIN | MF_IN_PASSABLE_VIEW_OF_ORIGIN)) && !viewMap[x][y])
  {
    return false;
  }
  ...
  {  // distance falls int of min
    return false;
  }
  if (featureFlags & MF_BUILD_IN_WALLS)
  {  // If we're supposed to build in a wall...
    if (!interior[x][y] && (pmap[x][y].machineNumber == 0 || pmap[x][y].machineNumber == machineNumber) &&
        cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY))
    {  // ...and this location is a wall
       // that's not already machined...
      for (dir = 0; dir < 4; dir++)
      {
        newX = x + nbDirs[dir][0];
        newY = y + nbDirs[dir][1];
        if (coordinatesAreInMap(newX, newY)  // ...and it's next to an interior spot or permitted
                                             // elsewhere and next to passable spot...
            && ((interior[newX][newY] && !(newX == originX && newY == originY)) ||
                ((featureFlags & MF_BUILD_ANYWHERE_ON_LEVEL) && !cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER) &&
  ...
  {  // Can't build in a wall unless
     // instructed to do so.
    return false;
  }
  else if (featureFlags & MF_BUILD_ANYWHERE_ON_LEVEL)
  {
    if ((featureFlags & MF_GENERATE_ITEM) && (cellHasTerrainFlag(x, y, T_OBSTRUCTS_ITEMS | T_PATHING_BLOCKER) ||
                                              (pmap[x][y].flags & (IS_CHOKEPOINT | IN_LOOP | IS_IN_MACHINE))))
    {
      return false;
    }
    else
...
void addLocationToKey(Item* theItem, int x, int y, bool disposableHere)
{
  int i;
...
void addMachineNumberToKey(Item* theItem, int machineNumber, bool disposableHere)
{
  int i;
...
void expandMachineInterior(char interior[DCOLS][DROWS], int minimumInteriorNeighbors)
{
  bool madeChange;
  int nbcount, newX, newY, i, j, layer;
  enum Directions dir;
  ...
  do
  {
    ...
    for (i = 1; i < DCOLS - 1; i++)
    {
      for (j = 1; j < DROWS - 1; j++)
      {
        if (cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && pmap[i][j].machineNumber == 0)
        {
          ...
          for (nbcount = dir = 0; dir < DIRECTION_COUNT; dir++)
          {
            newX = i + nbDirs[dir][0];
            newY = j + nbDirs[dir][1];
            if (interior[newX][newY] && !cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER))
            {
              nbcount++;
            }
          }
          if (nbcount >= minimumInteriorNeighbors)
          {
            ...
            if (!nbcount)
            {
              // Eliminate this obstruction; welcome its location into the
              // machine.
              madeChange = true;
              interior[i][j] = true;
        ...
        else if (pmap[i][j].layers[DUNGEON] == DOOR || pmap[i][j].layers[DUNGEON] == SECRET_DOOR)
        {
...
bool fillInteriorForVestibuleMachine(char interior[DCOLS][DROWS], int bp, int originX, int originY)
{
  int **distanceMap, **costMap, qualifyingTileCount, totalFreq, sRows[DROWS], sCols[DCOLS], i, j, k;
  bool success = true;
  ...
  totalFreq = rand_range(blueprintCatalog[bp].roomSize[0],
                         blueprintCatalog[bp].roomSize[1]);  // Keeps track of the goal size.
      ...
      for (j = 0; j < DROWS && qualifyingTileCount < totalFreq; j++)
      {
        if (distanceMap[sCols[i]][sRows[j]] == k)
        {
          ...
          if (pmap[sCols[i]][sRows[j]].flags & HAS_ITEM)
          {
            ...
            success = false;
            qualifyingTileCount = totalFreq;  // This is a hack to drop out of
  ...
  if ((blueprintCatalog[bp].flags & BP_TREAT_AS_BLOCKING) && levelIsDisconnectedWithBlockingMap(interior, false))
  {
    success = false;
  }
...
void redesignInterior(char interior[DCOLS][DROWS], int originX, int originY, int theProfileIndex)
{
  int i, j, n, newX, newY;
  enum Directions dir;
  int orphanList[20][2];
  int orphanCount = 0;
  int **grid, **pathingGrid, **costGrid;
  grid = allocGrid();
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      ...
      {
        grid[i][j] = 1;  // Treat existing level as already built (though
                         // shielded by a film of -1s).
        for (dir = 0; dir < 4; dir++)
        {
          newX = i + nbDirs[dir][0];
          newY = j + nbDirs[dir][1];
          if (coordinatesAreInMap(newX, newY) && interior[newX][newY] && (newX != originX || newY != originY))
          {
  ...
  // Connect to preexisting rooms that were orphaned (mostly preexisting machine
  // rooms).
  if (orphanCount > 0)
  {
    ...
    for (n = 0; n < orphanCount; n++)
    {
      ...
      while (pathingGrid[i][j] > 0)
      {
        for (dir = 0; dir < 4; dir++)
        {
          ...
          if (coordinatesAreInMap(newX, newY) && pathingGrid[newX][newY] < pathingGrid[i][j])
          {
            grid[i][j] = 1;
            i = newX;
            j = newY;
...
void prepareInteriorWithMachineFlags(char interior[DCOLS][DROWS], int originX, int originY, unsigned long flags,
                                     int dungeonProfileIndex)
{
  int i, j, newX, newY;
  enum dungeonLayers layer;
  enum Directions dir;

  // If requested, clear and expand the room as far as possible until either
  // it's convex or it bumps into surrounding rooms
      ...
      for (j = 0; j < DROWS; j++)
      {
        if (interior[i][j] && !(pmap[i][j].flags & IS_GATE_SITE))
        {
          for (dir = 0; dir < DIRECTION_COUNT; dir++)
          {
            newX = i + nbDirs[dir][0];
            newY = j + nbDirs[dir][1];
            if (coordinatesAreInMap(newX, newY) && !interior[newX][newY] &&
                !cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) && !(pmap[newX][newY].flags & IS_GATE_SITE) &&
                !pmap[newX][newY].machineNumber && cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER))
            {
              for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++)
              {
  ...
  // Completely clear the interior, fill with granite, and cut entirely new
  // rooms into it from the gate site. Then zero out any portion of the interior
  // that is still wall.
  ...
  // Reinforce surrounding tiles and interior tiles if requested to prevent
  // tunneling in or through.
      ...
      for (j = 0; j < DROWS; j++)
      {
        if (interior[i][j] && !(pmap[i][j].flags & IS_GATE_SITE))
        {
          ...
          for (dir = 0; dir < DIRECTION_COUNT; dir++)
          {
            newX = i + nbDirs[dir][0];
            newY = j + nbDirs[dir][1];
            if (coordinatesAreInMap(newX, newY) && !interior[newX][newY] && !(pmap[newX][newY].flags & IS_GATE_SITE))
            {
...
// If empty array parentSpawnedItems or parentSpawnedMonsters is given, will
// pass those back for deletion if necessary.
bool buildAMachine(enum machineTypes bp, int originX, int originY, unsigned long requiredMachineFlags,
                      Item* adoptiveItem, Item* parentSpawnedItems[MACHINES_BUFFER_LENGTH],
                      Creature* parentSpawnedMonsters[MACHINES_BUFFER_LENGTH])
{
  int i, j, k, feat, randIndex, totalFreq, gateCandidates[50][2], instance, instanceCount, qualifyingTileCount, featX,
      featY, itemCount, monsterCount, sRows[DROWS], sCols[DCOLS], **distanceMap, distance25, distance75, distances[100],
      distanceBound[2], personalSpace, failsafe, locationFailsafe, machineNumber;
  const unsigned long alternativeFlags[2] = { MF_ALTERNATIVE, MF_ALTERNATIVE_2 };
  bool success;

  // Our bool grids:
  //	Interior:		This is the master grid for the machine. All area
  // inside the machine are set to true. 	Occupied:		This keeps track
  // of what is within the personal space of a previously built feature in the
  // same machine. 	Candidates:		This is calculated at the start of each
  // feature, and is true where that feature is eligible for building.
  //	BlockingMap:	Used during terrain/DF placement in features that are
  // flagged not to tolerate blocking, to see if they block.
  //	ViewMap:		Used for features with MF_IN_VIEW_OF_ORIGIN, to
  // calculate which cells are in view of the origin.
  ...
  Creature *monst, *nextMonst, *torchBearer = nullptr, *leader = nullptr;

  Item *theItem = nullptr, *torch = nullptr, *spawnedItems[MACHINES_BUFFER_LENGTH] = { 0 },
       *spawnedItemsSub[MACHINES_BUFFER_LENGTH] = { 0 };
  ...
  do
  {
    ...
    if (--failsafe <= 0)
    {
      ...
      if (D_MESSAGE_MACHINE_GENERATION)
      {
        ...
        {
          printf("\nDepth %i: Failed to build a machine; gave up after 10 "
                 "unsuccessful attempts to find a suitable blueprint and/or "
                 "location.",
                 rogue.depthLevel);
        }
        else
        {
          printf("\nDepth %i: Failed to build a machine; requested blueprint "
                 "and location did not work.",
                 rogue.depthLevel);
        }
      }
      return false;
    ...
    if (chooseBP)
    {  // If no blueprint is given, then pick one:

      // First, choose the blueprint. We choose from among blueprints
      // that have the required blueprint flags and that satisfy the depth
      // requirements.
      totalFreq = 0;
      ...
      if (!totalFreq)
      {  // If no suitable blueprints are in the library, fail.
        if (distanceMap)
        {
          freeGrid(distanceMap);
        }
        if (D_MESSAGE_MACHINE_GENERATION)
          printf("\nDepth %i: Failed to build a machine because no suitable "
    ...
    if (blueprintCatalog[bp].flags & BP_ROOM)
    {
      ...
      // choke size and remember where they are. The origin of the room will be
      // the gate location.
      zeroOutGrid(interior);
      ...
      if (chooseLocation)
      {
        ...
        for (i = 0; i < DCOLS; i++)
        {
          ...
          {
            if ((pmap[i][j].flags & IS_GATE_SITE) && !(pmap[i][j].flags & IS_IN_MACHINE) &&
                chokeMap[i][j] >= blueprintCatalog[bp].roomSize[0] &&
                chokeMap[i][j] <= blueprintCatalog[bp].roomSize[1])
            {
              // DEBUG printf("\nDepth %i: Gate site qualified with interior
              // size of %i.", rogue.depthLevel, chokeMap[i][j]);
        ...
        else
        {
          ...
          if (D_MESSAGE_MACHINE_GENERATION)
            printf("\nDepth %i: Failed to build a machine; there was no eligible "
                   "door candidate for the chosen room machine from blueprint %i.",
                   rogue.depthLevel, bp);
          return false;
        }
      }
      ...
      // Start at the gate location and do a depth-first floodfill to grab all
      // adjoining tiles with the same or lower choke value, ignoring any tiles
      // that are already part of a machine. If we get false from this, try
      // again. If we've tried too many times already, abort.
    ...
    else if (blueprintCatalog[bp].flags & BP_VESTIBULE)
    {
      if (chooseLocation)
      {
        // Door machines must have locations passed in. We can't pick one
        // ourselves.
        if (distanceMap)
        {
          freeGrid(distanceMap);
        ...
        if (D_MESSAGE_MACHINE_GENERATION)
          printf("\nDepth %i: ERROR: Attempted to build a door machine from "
                 "blueprint %i without a location being provided.",
                 rogue.depthLevel, bp);
        return false;
      }
      if (!fillInteriorForVestibuleMachine(interior, bp, originX, originY))
      {
        ...
        if (D_MESSAGE_MACHINE_GENERATION)
          printf("\nDepth %i: Failed to build a door machine from blueprint "
                 "%i; not enough room.",
                 rogue.depthLevel, bp);
        return false;
      }
    ...
    else
    {
      ...
      // expand it along a pathing map by one space in all directions until the
      // size reaches the chosen size, and then make sure the resulting space
      // qualifies. If not, try again. If we've tried too many times already,
      // abort.
      ...
      do
      {
        ...
        calculateDistances(distanceMap, originX, originY, T_PATHING_BLOCKER, nullptr, true, false);
        qualifyingTileCount = 0;  // Keeps track of how many interior cells we've added.
        totalFreq = rand_range(blueprintCatalog[bp].roomSize[0],
                               blueprintCatalog[bp].roomSize[1]);  // Keeps track of the goal size.
          ...
          for (i = 0; i < DCOLS && qualifyingTileCount < totalFreq; i++)
          {
            for (j = 0; j < DROWS && qualifyingTileCount < totalFreq; j++)
            {
              if (distanceMap[sCols[i]][sRows[j]] == k)
              {
                ...
                {
                  // Abort if we've entered another machine or engulfed another
                  // machine's item or monster.
                  tryAgain = true;
                  qualifyingTileCount = totalFreq;  // This is a hack to drop out
                                                    // of these three for-loops.
                }
              }
            }
          }
        ...
        // Now make sure the interior map satisfies the machine's
        // qualifications.
        if ((blueprintCatalog[bp].flags & BP_TREAT_AS_BLOCKING) && levelIsDisconnectedWithBlockingMap(interior, false))
        {
          tryAgain = true;
                 ...
                 levelIsDisconnectedWithBlockingMap(interior, true) < 100)
        {
          tryAgain = true;  // BP_REQUIRE_BLOCKING needs some work to make sure
                            // the disconnect is interesting.
        }
        // If locationFailsafe runs out, tryAgain will still be true, and we'll
        // try a different machine. If we're not choosing the blueprint, then
        // don't bother with the locationFailsafe; just use the higher-level
        // failsafe.
      } while (chooseBP && tryAgain && --locationFailsafe);
    }

    // If something went wrong, but we haven't been charged with choosing
    // blueprint OR location, then there is nothing to try again, so just fail.
  ...
  // This is the point of no return. Back up the level so it can be restored if
  // we have to abort this machine after this point.
  copyMap(pmap, levelBackup);

  // Perform any transformations to the interior indicated by the blueprint
  // flags, including expanding the interior if requested.
  prepareInteriorWithMachineFlags(interior, originX, originY, blueprintCatalog[bp].flags,
                                  blueprintCatalog[bp].dungeonProfileType);

  // If necessary, label the interior as IS_IN_AREA_MACHINE or
  // IS_IN_ROOM_MACHINE and mark down the number.
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (interior[i][j])
      {
        ...
        // also clear any secret doors, since they screw up distance mapping and
        // aren't fun inside machines
  ...
  //	DEBUG printf("\n\nWorking on blueprint %i, with origin at (%i, %i).
  // Here's the initial interior map:", bp, originX, originY); 	DEBUG
  // logBuffer(interior);

  // Calculate the distance map (so that features that want to be close to or
  // far from the origin can be placed accordingly) and figure out the 33rd and
  // 67th percentiles for features that want to be near or far from the origin.
  if (!distanceMap)
  {
    distanceMap = allocGrid();
  }
  fillGrid(distanceMap, 0);
  calculateDistances(distanceMap, originX, originY, T_PATHING_BLOCKER, nullptr, true, true);
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (interior[i][j] && distanceMap[i][j] < 100)
      {
        distances[distanceMap[i][j]]++;  // create a histogram of distances --
                                         // poor man's sort function
        qualifyingTileCount++;
      }
    }
  }
  ...
  // DEBUG printf("\nDistances calculated: 33rd percentile of distance is %i,
  // and 67th is %i.", distance25, distance75);

  // Now decide which features will be skipped -- of the features marked
  // MF_ALTERNATIVE, skip all but one, chosen randomly. Then repeat and do the
  // same with respect to MF_ALTERNATIVE_2, to provide up to two independent
  // sets of alternative features per machine.
    ...
    if (totalFreq > 0)
    {
      ...
      for (i = 0; i < blueprintCatalog[bp].featureCount; i++)
      {
        if (blueprintCatalog[bp].feature[i].flags & alternativeFlags[j])
        {
          if (randIndex == 1)
          {
            skipFeature[i] = false;  // This is the alternative that gets built.
                                     // The rest do not.
            break;
          }
          else
  ...
  // Keep track of all monsters and items that we spawn -- if we abort, we have
  // to go back and delete them all.
  itemCount = monsterCount = 0;

  // Zero out occupied[][], and use it to keep track of the personal space
  // around each feature that gets placed.
  ...
  for (feat = 0; feat < blueprintCatalog[bp].featureCount; feat++)
  {
    if (skipFeature[feat])
    {
      continue;  // Skip the alternative features that were not selected for
                 // building.
    }
    ...
    if (feature->flags & (MF_IN_VIEW_OF_ORIGIN | MF_IN_PASSABLE_VIEW_OF_ORIGIN))
    {
      ...
      {
        getFOVMask(viewMap, originX, originY, max(DCOLS, DROWS), (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION), 0,
                   false);
      }
    ...
    do
    {  // If the MF_REPEAT_UNTIL_NO_PROGRESS flag is set, repeat until we fail
       // to build the required number of instances.
      ...
      for (i = 0; i < DCOLS; i++)
      {
        for (j = 0; j < DROWS; j++)
        {
          if (cellIsFeatureCandidate(i, j, originX, originY, distanceBound, interior, occupied, viewMap, distanceMap,
      ...
      {
        // Generate everywhere that qualifies -- instead of randomly picking
        // tiles, keep spawning until we run out of eligible tiles.
      ...
      for (instance = 0; (generateEverywhere || instance < instanceCount) && qualifyingTileCount > 0;)
      {
        ...
        {
          // Does the feature want to be at the origin? If so, put it there.
        ...
        else
        {
          ...
          // the candidates map so that subsequent instances of this same
          // feature can't choose it.
          featX = -1;
        ...
        // Don't waste time trying the same place again whether or not this
        // attempt succeeds.
        ...
        // Try to build the DF first, if any, since we don't want it to be
        // disrupted by subsequently placed terrain.
        ...
        // OK, if placement was successful, clear some personal space around the
        // feature so subsequent features can't be generated too close. Personal
        // space of 0 means nothing gets cleared, 1 means that only the tile
        // itself gets cleared, and 2 means the 3x3 grid centered on it.

        if (DFSucceeded && terrainSucceeded)
        {
          for (i = featX - personalSpace + 1; i <= featX + personalSpace - 1; i++)
          {
            for (j = featY - personalSpace + 1; j <= featY + personalSpace - 1; j++)
            {
              if (coordinatesAreInMap(i, j))
              {
                if (candidates[i][j])
                {
                  brogueAssert(!occupied[i][j] || (i == originX && j == originY));  // Candidates[][] should never be
                                                                                    // true where occupied[][] is true.
                  candidates[i][j] = false;
                  qualifyingTileCount--;
                }
                occupied[i][j] = true;
          ...
          // DEBUG printf("\nPlaced instance #%i of feature %i at (%i, %i).",
          // instance, feat, featX, featY);
        }

        if (DFSucceeded && terrainSucceeded)
        {  // Proceed only if the terrain stuff for this
           // instance succeeded.
          ...
          pmap[featX][featY].flags |=
              ((blueprintCatalog[bp].flags & BP_ROOM) ? IS_IN_ROOM_MACHINE : IS_IN_AREA_MACHINE);
              ...
              (adoptiveItem && (feature->flags & MF_ADOPT_ITEM) && (blueprintCatalog[bp].flags & BP_ADOPT_ITEM)))
          {
            ...
            if (adoptiveItem && (feature->flags & MF_ADOPT_ITEM) && (blueprintCatalog[bp].flags & BP_ADOPT_ITEM))
            {
              ...
              adoptiveItem = nullptr;  // can be adopted only once
            }
            else
            {
              ...
              while ((theItem->flags & ITEM_CURSED) ||
                     ((feature->flags & MF_REQUIRE_GOOD_RUNIC) && (!(theItem->flags & ITEM_RUNIC)))  // runic if
                                                                                                     // requested
                     || ((feature->flags & MF_NO_THROWING_WEAPONS) && theItem->category == WEAPON &&
                         theItem->quantity > 1)  // no throwing weapons if prohibited
                     || itemIsADuplicate(theItem, spawnedItems, itemCount))
              {  // don't want to duplicates
                 // of rings, staffs, etc.
                deleteItem(theItem);
                theItem = generateItem(feature->itemCategory, feature->itemKind);
                if (failsafe <= 0)
              ...
              spawnedItems[itemCount] = theItem;  // Keep a list of generated items so that we can
                                                  // delete them all if construction fails.
            ...
            if (!(feature->flags & MF_OUTSOURCE_ITEM_TO_MACHINE) && !(feature->flags & MF_MONSTER_TAKE_ITEM))
            {
          ...
          if (feature->flags & (MF_OUTSOURCE_ITEM_TO_MACHINE | MF_BUILD_VESTIBULE))
          {
            ...
            // If we fail 10 times, abort the entire machine (including any
            // sub-machines already built). Also, if we build a sub-machine, and
            // it succeeds, but this (its parent machine) fails, we pass the
            // monsters and items that it spawned back to the parent, so that if
            // the parent fails, they can all be freed.
            for (i = 10; i > 0; i--)
            {
              // First make sure our adopted item, if any, is not on the floor
              // or in the pack already. Otherwise, a previous attempt to place
              // it may have put it on the floor in a different machine, only to
              // have that machine fail and be deleted, leaving the item
              // remaining on the floor where placed.
              ...
              {
                success = buildAMachine(-1, featX, featY, BP_VESTIBULE, nullptr, spawnedItemsSub, spawnedMonstersSub);
              }

              // Now put the item up for adoption.
              if (success)
              {
                // Success! Now we have to add that machine's items and monsters
                // to our own list, so they all get deleted if this machine or
                // its parent fails.
            ...
            {
              if (D_MESSAGE_MACHINE_GENERATION)
                printf("\nDepth %i: Failed to place blueprint %i because it "
                       "requires an adoptive machine and we couldn't place one.",
                       rogue.depthLevel, bp);
              // failure! abort!
          ...
          if ((feature->flags & MF_GENERATE_HORDE) || feature->monsterID)
          {
            if (feature->flags & MF_GENERATE_HORDE)
            {
              monst = spawnHorde(0, featX, featY, ((HORDE_IS_SUMMONED | HORDE_LEADER_CAPTIVE) & ~(feature->hordeFlags)),
                                 feature->hordeFlags);
              if (monst)
              {
            ...
            if (feature->monsterID)
            {
              ...
              if (monst)
              {
                killCreature(monst, true);  // If there's already a monster here,
                                            // quietly bury the body.
              }
            ...
            if (monst)
            {
              ...
              // Actually just remember that we have to give it to this monster;
              // the actual hand-off happens after we're sure that the machine
              // will succeed.
            ...
            {
              // Have to cache the next monster, as the chain can get disrupted
              // by making a monster dormant below.
              ...
              if (monst->bookkeepingFlags & MB_JUST_SUMMONED)
              {
                ...
                monst->machineHome = machineNumber;  // Monster remembers the
                                                     // machine that spawned it.
              }
            }
          }
        }
    ...
    if (instance < feature->minimumInstanceCount && !(feature->flags & MF_REPEAT_UNTIL_NO_PROGRESS))
    {
      ...
      if (D_MESSAGE_MACHINE_GENERATION)
        printf("\nDepth %i: Failed to place blueprint %i because of feature "
               "%i; needed %i instances but got only %i.",
  ...
  if (blueprintCatalog[bp].flags & BP_NO_INTERIOR_FLAG)
  {
    for (i = 0; i < DCOLS; i++)
    {
      for (j = 0; j < DROWS; j++)
      {
        if (pmap[i][j].machineNumber == machineNumber && !cellHasTMFlag(i, j, (TM_IS_WIRED | TM_IS_CIRCUIT_BREAKER)))
        {
  ...
  freeGrid(distanceMap);
  if (D_MESSAGE_MACHINE_GENERATION)
    printf("\nDepth %i: Built a machine from blueprint %i with an origin at "
           "(%i, %i).",
           rogue.depthLevel, bp, originX, originY);
  return true;
}
...
void addMachines()
{
  int machineCount, failsafe;
  int randomMachineFactor;
  ...
  if (rogue.depthLevel == AMULET_LEVEL)
  {
    ...
    {
      if (buildAMachine(MT_AMULET_AREA, -1, -1, nullptr, nullptr, nullptr, nullptr))
      {
        break;
      }
    }
  ...
  {
    if (buildAMachine(-1, -1, -1, BP_REWARD, nullptr, nullptr, nullptr))
    {
      machineCount--;
...
// Add terrain, DFs and flavor machines. Includes traps, torches, funguses,
// flavor machines, etc. If buildAreaMachines is true, build ONLY the
// autogenerators that include machines. If false, build all EXCEPT the
// autogenerators that include machines.
  ...
  for (AG = 1; AG < NUMBER_AUTOGENERATORS; AG++)
  {
    ...
    if (gen->machine > 0 == buildAreaMachines)
    {
      ...
      for (i = 0; i < count; i++)
      {
        ...
        // if (randomMatchingLocation(&x, &y,
        // gen->requiredDungeonFoundationType, NOTHING, -1)) { if
        // (randomMatchingLocation(&x, &y, -1, -1,
        ...
        if (randomMatchingLocation(&x, &y, gen->requiredDungeonFoundationType, gen->requiredLiquidFoundationType, -1))
        {
          ...
          // Spawn the terrain if it's got the priority to spawn there and won't
        ...
        // Machines will find their own locations, so it will not be at the same
        // place as terrain and DF.
        if (gen->machine > 0)
        {
          buildAMachine(gen->machine, -1, -1, 0, nullptr, nullptr, nullptr);
        }
      }
    }
  }
...
void cleanUpLakeBoundaries()
{
  ...
  do
  {
    ...
    for (i = (reverse ? DCOLS - 2 : 1); (reverse ? i > 0 : i < DCOLS - 1); (reverse ? i-- : i++))
    {
      for (j = (reverse ? DROWS - 2 : 1); (reverse ? j > 0 : j < DROWS - 1); (reverse ? j-- : j++))
      {
        // assert(i >= 1 && i <= DCOLS - 2 && j >= 1 && j <= DROWS - 2);

        // if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY)
        if (cellHasTerrainFlag(i, j, T_LAKE_PATHING_BLOCKER | T_OBSTRUCTS_PASSABILITY) &&
            !cellHasTMFlag(i, j, TM_IS_SECRET) && !(pmap[i][j].flags & IMPREGNABLE))
        {
          ...
          if ((terrainFlags(i - 1, j) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) &&
              !cellHasTMFlag(i - 1, j, TM_IS_SECRET) && !cellHasTMFlag(i + 1, j, TM_IS_SECRET) &&
              (terrainFlags(i - 1, j) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) ==
                  (terrainFlags(i + 1, j) & T_LAKE_PATHING_BLOCKER & ~subjectFlags))
          {
            x = i + 1;
            y = j;
                   ...
                   !cellHasTMFlag(i, j - 1, TM_IS_SECRET) && !cellHasTMFlag(i, j + 1, TM_IS_SECRET) &&
                   (terrainFlags(i, j - 1) & T_LAKE_PATHING_BLOCKER & ~subjectFlags) ==
                       (terrainFlags(i, j + 1) & T_LAKE_PATHING_BLOCKER & ~subjectFlags))
          {
            x = i;
            y = j + 1;
...
void insertRoomAt(int** dungeonMap, int** roomMap, const int roomToDungeonX, const int roomToDungeonY,
                  const int xRoom, const int yRoom)
{
  int newX, newY;
  enum Directions dir;
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && roomMap[newX][newY] &&
...
void designCavern(int** grid, int minWidth, int maxWidth, int minHeight, int maxHeight)
{
  int destX, destY;
  ...
  fillGrid(grid, 0);
  createBlobOnGrid(blobGrid, &caveX, &caveY, &caveWidth, &caveHeight, 5, minWidth, minHeight, maxWidth, maxHeight, 55,
                   "ffffffttt", "ffffttttt");
...
// This is a special room that appears at the entrance to the dungeon on
// depth 1.
void designEntranceRoom(int** grid)
{
...
void designSymmetricalCrossRoom(int** grid)
{
...
void designCircularRoom(int** grid)
{
  int radius;
  ...
  if (radius > 6 && rand_percent(50))
  {
    drawCircleOnGrid(grid, DCOLS / 2, DROWS / 2, rand_range(3, radius - 3), 0);
  }
}

void designChunkyRoom(int** grid)
{
  int i, x, y;
  int minX, maxX, minY, maxY;
  int chunkCount = rand_range(2, 8);
...
// If the indicated tile is a wall on the room stored in grid, and it could be
// the site of a door out of that room, then return the outbound direction that
// the door faces. Otherwise, return NO_DIRECTION.
enum Directions directionOfDoorSite(int** grid, int x, int y)
{
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    oppX = x - nbDirs[dir][0];
    oppY = y - nbDirs[dir][1];
    if (coordinatesAreInMap(oppX, oppY) && coordinatesAreInMap(newX, newY) && grid[oppX][oppY] == 1)
    {
      // This grid cell would be a valid tile on which to place a door that,
      // facing outward, points dir.
...
void chooseRandomDoorSites(int** roomMap, int doorSites[4][2])
{
  int i, j, k, newX, newY;
  enum Directions dir;
  int** grid;
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (!grid[i][j])
      {
        ...
        if (dir != NO_DIRECTION)
        {
          // Trace a ray 10 spaces outward from the door site to make sure it
          // doesn't intersect the room. If it does, it's not a valid door site.
          newX = i + nbDirs[dir][0];
          newY = j + nbDirs[dir][1];
          ...
          if (!doorSiteFailed)
          {
            //                        plotCharWithColor(dirChars[dir],
            //                        mapToWindowX(i), mapToWindowY(j), &black,
            //                        &green);
            grid[i][j] = dir + 2;  // So as not to conflict with 0 or 1, which
                                   // are used to indicate exterior/interior.
          }
        }
      }
    }
...
void attachHallwayTo(int** grid, int doorSites[4][2])
{
  int i, x, y, newX, newY, dirs[4];
  int length;
  ...
  {
    dir = dirs[i];
    if (doorSites[dir][0] != -1 && doorSites[dir][1] != -1 &&
  ...
  y = clamp(y - nbDirs[dir][1], 0,
            DROWS - 1);  // Now (x, y) points at the last interior cell of the hallway.
  ...
  for (dir2 = 0; dir2 < 4; dir2++)
  {
    ...
    if ((dir2 != dir && !allowObliqueHallwayExit) || !coordinatesAreInMap(newX, newY) || grid[newX][newY])
    {
      doorSites[dir2][0] = -1;
      doorSites[dir2][1] = -1;
    }
...
// If attachHallway is true, then it will bolt a perpendicular hallway onto the
// room at one of the four standard door sites, and then relocate three of the
// door sites to radiate from the end of the hallway. (The fourth is defunct.)
// RoomTypeFrequencies specifies the probability of each room type, in the
// following order:
//      0. Cross room
...
void designRandomRoom(int** grid, bool attachHallway, int doorSites[4][2],
                      const int roomTypeFrequencies[ROOM_TYPE_COUNT])
{
  int randIndex, i, sum;
  enum Directions dir;
...
void attachRooms(int** grid, const DungeonProfile* theDP, int attempts, int maxRoomCount)
{
  ...
  for (roomsBuilt = roomsAttempted = 0; roomsBuilt < maxRoomCount && roomsAttempted < attempts; roomsAttempted++)
  {
    ...
    fillGrid(roomMap, 0);
    designRandomRoom(roomMap, roomsAttempted <= attempts - 5 && rand_percent(theDP->corridorChance), doorSites,
    ...
    if (D_INSPECT_LEVELGEN)
    {
      ...
      if (doorSites[0][0] != -1)
        plotCharWithColor('^', mapToWindowX(doorSites[0][0]), mapToWindowY(doorSites[0][1]), &black, &green);
      if (doorSites[1][0] != -1)
        plotCharWithColor('v', mapToWindowX(doorSites[1][0]), mapToWindowY(doorSites[1][1]), &black, &green);
      if (doorSites[2][0] != -1)
        plotCharWithColor('<', mapToWindowX(doorSites[2][0]), mapToWindowY(doorSites[2][1]), &black, &green);
      if (doorSites[3][0] != -1)
        plotCharWithColor('>', mapToWindowX(doorSites[3][0]), mapToWindowY(doorSites[3][1]), &black, &green);
      temporaryMessage("Generating this room:", true);
    }
    ...
    for (i = 0; i < DCOLS * DROWS; i++)
    {
      ...
      if (dir != NO_DIRECTION && doorSites[oppDir][0] != -1 &&
          roomFitsAt(grid, roomMap, x - doorSites[oppDir][0], y - doorSites[oppDir][1]))
      {
        ...
        insertRoomAt(grid, roomMap, x - doorSites[oppDir][0], y - doorSites[oppDir][1], doorSites[oppDir][0],
                     doorSites[oppDir][1]);
...
{
  const int descentPercent = clamp(100 * (rogue.depthLevel - 1) / (AMULET_LEVEL - 1), 0, 100);
...
// On the grid, a 0 denotes granite, a 1 denotes floor, and a 2 denotes a
// possible door site. -1 denotes off-limits areas -- rooms can't be placed
// there and also can't sprout off of there. Parent function will translate this
// grid into pmap[][] to make floors, walls, doors, etc.
void carveDungeon(int** grid)
{
  ...
  designRandomRoom(grid, false, nullptr, theFirstRoomDP.roomFrequencies);
...
void liquidType(int* deep, int* shallow, int* shallowWidth)
{
  int randMin, randMax, rand;
...
// Fills a lake marked in unfilledLakeMap with the specified liquid type,
// scanning outward to reach other lakes within scanWidth. Any wreath of shallow
// liquid must be done elsewhere.
void fillLake(int x, int y, int liquid, int scanWidth, char wreathMap[DCOLS][DROWS], int** unfilledLakeMap)
{
  int i, j;
  ...
  for (i = x - scanWidth; i <= x + scanWidth; i++)
  {
    for (j = y - scanWidth; j <= y + scanWidth; j++)
    {
      if (coordinatesAreInMap(i, j) && unfilledLakeMap[i][j])
      {
        ...
        wreathMap[i][j] = 1;
        fillLake(i, j, liquid, scanWidth, wreathMap,
                 unfilledLakeMap);  // recursive
      }
    }
  }
}

void lakeFloodFill(int x, int y, int** floodMap, int** grid, int** lakeMap, int dungeonToGridX,
                   int dungeonToGridY)
{
  int newX, newY;
  enum Directions dir;
  ...
  for (dir = 0; dir < 4; dir++)
  {
    newX = x + nbDirs[dir][0];
    newY = y + nbDirs[dir][1];
    if (coordinatesAreInMap(newX, newY) && !floodMap[newX][newY] &&
...
bool lakeDisruptsPassability(int** grid, int** lakeMap, int dungeonToGridX, int dungeonToGridY)
{
  bool result;
  int i, j, x, y;
  int** floodMap;
  ...
  for (i = 0; i < DCOLS && x == -1; i++)
  {
    ...
    {
      if (!cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && !lakeMap[i][j] &&
  ...
  for (i = 0; i < DCOLS && result == false; i++)
  {
    ...
    {
      if (!cellHasTerrainFlag(i, j, T_PATHING_BLOCKER) && !lakeMap[i][j] && !floodMap[i][j] &&
          (!coordinatesAreInMap(i + dungeonToGridX, j + dungeonToGridY) ||
           !grid[i + dungeonToGridX][j + dungeonToGridY]))
      {
        //                if (D_INSPECT_LEVELGEN) {
        ...
        //                    plotCharWithColor('X', mapToWindowX(i),
        //                    mapToWindowY(j), &black, &red);
...
void designLakes(int** lakeMap)
{
  int i, j, k;
  int x, y;
  int lakeMaxHeight, lakeMaxWidth;
  int lakeX, lakeY, lakeWidth, lakeHeight;

  int** grid;  // Holds the current lake.
  ...
  {  // lake generations
    ...
    fillGrid(grid, 0);
    createBlobOnGrid(grid, &lakeX, &lakeY, &lakeWidth, &lakeHeight, 5, 4, 4, lakeMaxWidth, lakeMaxHeight, 55,
                     "ffffftttt", "ffffttttt");
    ...
    {  // placement attempts
      ...
      if (!lakeDisruptsPassability(grid, lakeMap, -x, -y))
...
void createWreath(int shallowLiquid, int wreathWidth, char wreathMap[DCOLS][DROWS])
{
  int i, j, k, l;
  for (i = 0; i < DCOLS; i++)
  {
...
void fillLakes(int** lakeMap)
{
  int deepLiquid = CRYSTAL_WALL, shallowLiquid = CRYSTAL_WALL, shallowLiquidWidth = 0;
...
{
  int i, j;
  const int secretDoorChance = clamp((rogue.depthLevel - 1) * 67 / 25, 0, 67);
  for (i = 1; i < DCOLS - 1; i++)
  {
    for (j = 1; j < DROWS - 1; j++)
    {
      if (pmap[i][j].layers[DUNGEON] == DOOR && pmap[i][j].machineNumber == 0)
      {
        if ((!cellHasTerrainFlag(i + 1, j, T_OBSTRUCTS_PASSABILITY) ||
             !cellHasTerrainFlag(i - 1, j, T_OBSTRUCTS_PASSABILITY)) &&
            (!cellHasTerrainFlag(i, j + 1, T_OBSTRUCTS_PASSABILITY) ||
             !cellHasTerrainFlag(i, j - 1, T_OBSTRUCTS_PASSABILITY)))
        {
          // If there's passable terrain to the left or right, and there's
          // passable terrain above or below, then the door is orphaned and must
          // be removed.
          pmap[i][j].layers[DUNGEON] = FLOOR;
        }
                 ...
                 3)
        {
          // If the door has three or more pathing blocker neighbors in the four
          // cardinal directions, then the door is orphaned and must be removed.
          pmap[i][j].layers[DUNGEON] = FLOOR;
        }
...
bool buildABridge()
{
  int i, j, k, l, i2, j2, nCols[DCOLS], nRows[DROWS];
  int bridgeRatioX, bridgeRatioY;
  bool foundExposure;
  ...
  for (i2 = 1; i2 < DCOLS - 1; i2++)
  {
    ...
    {
      j = nRows[j2];
      if (!cellHasTerrainFlag(i, j, (T_CAN_BE_BRIDGED | T_PATHING_BLOCKER)) && !pmap[i][j].machineNumber)
      {
        // try a horizontal bridge
        foundExposure = false;
        for (k = i + 1; k < DCOLS                     // Iterate across the prospective length of the bridge.
                        ...
                        && !cellHasTerrainFlag(k, j,
                                               T_OBSTRUCTS_PASSABILITY)  // Candidate tile cannot be a wall.
                        && cellHasTerrainFlag(k, j - 1,
                                              (T_CAN_BE_BRIDGED | T_OBSTRUCTS_PASSABILITY))  // Only chasms or walls are
                                                                                             // permitted next to the
                                                                                             // length of the bridge.
                        && cellHasTerrainFlag(k, j + 1, (T_CAN_BE_BRIDGED | T_OBSTRUCTS_PASSABILITY));
             k++)
        {
          if (!cellHasTerrainFlag(k, j - 1, T_OBSTRUCTS_PASSABILITY)  // Can't run against a wall
                                                                      // the whole way.
        ...
        if (k < DCOLS && (k - i > 3)  // Can't have bridges shorter than 3 spaces.
            && foundExposure &&
            !cellHasTerrainFlag(k, j,
                                T_PATHING_BLOCKER | T_CAN_BE_BRIDGED)  // Must end on an unobstructed land tile.
            && !pmap[k][j].machineNumber                               // Cannot end in a machine.
            && 100 * pathingDistance(i, j, k, j, T_PATHING_BLOCKER) / (k - i) > bridgeRatioX)
        {  // Must shorten the pathing distance enough.
        ...
        foundExposure = false;
        for (k = j + 1; k < DROWS && !pmap[i][k].machineNumber && cellHasTerrainFlag(i, k, T_CAN_BE_BRIDGED) &&
                        !cellHasTMFlag(i, k, TM_IS_SECRET) && !cellHasTerrainFlag(i, k, T_OBSTRUCTS_PASSABILITY) &&
        ...
        if (k < DROWS && (k - j > 3) && foundExposure &&
...
// Finishing touches -- items, monsters, staircases, etc. -- are handled
// elsewhere.
void digDungeon()
{
  int i, j;
  ...
  // Time to add lakes and chasms. Strategy is to generate a series of blob
  // lakes of decreasing size. For each lake, propose a position, and then check
  // via a flood fill that the level would remain connected with that placement
  // (i.e. that each passable tile can still be reached). If not, make 9 more
  // placement attempts before abandoning that lake and proceeding to generate
  // the next smaller one. Canvas sizes start at 30x15 and decrease by 2x1 at a
  // time down to a minimum of 20x10. Min generated size is always 4x4.

  // DEBUG logLevel();

  // Now design the lakes and then fill them with various liquids (lava, water,
  // chasm, brimstone).
  int** lakeMap = allocGrid();
  designLakes(lakeMap);
  fillLakes(lakeMap);
  freeGrid(lakeMap);
  ...
  // Now finish any exposed granite with walls and revert any unexposed walls to
  // granite
  finishWalls(true);
...
void refreshWaypoint(int wpIndex)
{
  ...
  {
    if ((monst->creatureState == MONSTER_SLEEPING || (monst->info.flags & MONST_IMMOBILE) ||
...
void setUpWaypoints()
{
  int i, j, sCoord[DCOLS * DROWS], x, y;
  char grid[DCOLS][DROWS];
...
// Returns the size of the connected zone, and marks visited[][] with the
// zoneLabel.
int connectCell(int x, int y, int zoneLabel, char blockingMap[DCOLS][DROWS], char zoneMap[DCOLS][DROWS])
{
  enum Directions dir;
  int newX, newY, size;
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    if (coordinatesAreInMap(newX, newY) && zoneMap[newX][newY] == 0 && (!blockingMap || !blockingMap[newX][newY]) &&
...
// Make a zone map of connected passable regions that include at least one
// passable cell that borders the blockingMap if blockingMap blocks. Keep track
// of the size of each zone. Then pretend that the blockingMap no longer blocks,
// and grow these zones into the resulting area (without changing the stored
// zone sizes). If two or more zones now touch, then we block. At that point,
// return the size in cells of the smallest of all of the touching regions (or
// just 1, i.e. true, if countRegionSize is false). If no zones touch, then we
// don't block, and we return zero, i.e. false.
int levelIsDisconnectedWithBlockingMap(char blockingMap[DCOLS][DROWS], bool countRegionSize)
{
  char zoneMap[DCOLS][DROWS];
  int i, j, dir, zoneSizes[200], zoneCount, smallestQualifyingZoneSize, borderingZone;
    ...
    for (j = 1; j < DROWS - 1; j++)
    {
      if (blockingMap[i][j] && zoneMap[i][j] == 0 && cellIsPassableOrDoor(i, j))
      {
        for (dir = 0; dir < 4; dir++)
        {
          ...
          if (borderingZone != 0)
          {
            connectCell(i, j, borderingZone, nullptr, zoneMap);
            break;
          }
        }
      }
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (  // If it's flagged for building in the spawn map,
          ...
          // and the terrain in the layer to be overwritten has a higher
          // priority number (unless superpriority),
          && (superpriority ||
              tileCatalog[pmap[i][j].layers[layer]].drawPriority >= tileCatalog[surfaceTileType].drawPriority)
          // and we won't be painting into the surface layer when that cell
          // forbids it,
          && !(layer == SURFACE && cellHasTerrainFlag(i, j, T_OBSTRUCTS_SURFACE_EFFECTS))
          // and, if requested, the fill won't violate the priority of the most
          // important terrain in this cell:
          && (!blockedByOtherLayers || tileCatalog[pmap[i][j].layers[highestPriorityLayer(i, j, true)]].drawPriority >=
                                           tileCatalog[surfaceTileType].drawPriority))
      {
...
void spawnMapDF(int x, int y, enum TileType propagationTerrain, bool requirePropTerrain, int startProb,
                int probDec, char spawnMap[DCOLS][DROWS])
{
  int i, j, dir, t, x2, y2;
  bool madeChange;
        ...
        if (spawnMap[i][j] == t - 1)
        {
          for (dir = 0; dir < 4; dir++)
          {
            ...
            if (coordinatesAreInMap(x2, y2) &&
                ...
                (!cellHasTerrainFlag(x2, y2, T_OBSTRUCTS_SURFACE_EFFECTS) ||
                 (propagationTerrain > 0 && cellHasTerrainType(x2, y2, propagationTerrain))) &&
...
void evacuateCreatures(char blockingMap[DCOLS][DROWS])
{
  ...
  for (i = 0; i < DCOLS; i++)
  {
    for (j = 0; j < DROWS; j++)
    {
      if (blockingMap[i][j] && (pmap[i][j].flags & (HAS_MONSTER | HAS_PLAYER)))
      {
        monst = monsterAtLoc(i, j);
        getQualifyingLocNear(newLoc, i, j, true, blockingMap, forbiddenFlagsForMonster(&(monst->info)),
                             (HAS_MONSTER | HAS_PLAYER), false, false);
        monst->xLoc = newLoc[0];
        monst->yLoc = newLoc[1];
...
// because of blocking)
bool spawnDungeonFeature(int x, int y, DungeonFeature* feat, bool refreshCell, bool abortIfBlocking)
{
  int i, j, layer;
  ...
  // Blocking keeps track of whether to abort if it turns out that the DF would
  // obstruct the level.
  blocking = ((abortIfBlocking && !(feat->flags & DFF_PERMIT_BLOCKING) &&
               ((tileCatalog[feat->tile].flags & (T_PATHING_BLOCKER)) || (feat->flags & DFF_TREAT_AS_BLOCKING))) ?
                  true :
                  false);
  ...
  if (feat->tile)
  {
    ...
    {
      spawnMapDF(x, y, feat->propagationTerrain, (feat->propagationTerrain ? true : false), feat->startProbability,
                 feat->probabilityDecrement, blockingMap);
      if (!blocking || !levelIsDisconnectedWithBlockingMap(blockingMap, false))
      {
        if (feat->flags & DFF_EVACUATE_CREATURES_FIRST)
        {  // first, evacuate creatures if
           // necessary, so that they do not
           // re-trigger the tile.
          evacuateCreatures(blockingMap);
        }

        // succeeded = fillSpawnMap(feat->layer, feat->tile, blockingMap,
        // (feat->flags & DFF_BLOCKED_BY_OTHER_LAYERS), refreshCell, (feat->flags
        // & DFF_SUPERPRIORITY));
        fillSpawnMap(feat->layer, feat->tile, blockingMap, (feat->flags & DFF_BLOCKED_BY_OTHER_LAYERS), refreshCell,
                     (feat->flags & DFF_SUPERPRIORITY));  // this can tweak the spawn map too
        succeeded = true;                                 // fail ONLY if we blocked the level. We succeed even
                                                          // if, thanks to priority, nothing gets built.
      }
      else
      {
        succeeded = false;
  ...
  //	if (succeeded && feat->description[0] && !feat->messageDisplayed &&
  // playerCanSee(x, y)) { 		feat->messageDisplayed = true;
  //		message(feat->description, false);
  //	}
  if (succeeded)
  {
    ...
    {
      for (monst = dormantMonsters->nextCreature; monst != nullptr; monst = monst->nextCreature)
      {
...
void restoreMonster(Creature* monst, int** mapToStairs, int** mapToPit)
{
  ...
  if (monst->status[STATUS_ENTERS_LEVEL_IN] > 0)
  {
    ...
    {
      turnCount =
          ((theMap[monst->xLoc][monst->yLoc] * monst->movementSpeed / 100) - monst->status[STATUS_ENTERS_LEVEL_IN]);
      for (i = 0; i < turnCount; i++)
      {
        if ((dir = nextStep(theMap, monst->xLoc, monst->yLoc, nullptr, true)) != NO_DIRECTION)
        {
  ...
  if ((pmap[*x][*y].flags & (HAS_PLAYER | HAS_UP_STAIRS | HAS_DOWN_STAIRS)) || (monst->bookkeepingFlags & MB_PREPLACED))
  {
    if (!(monst->bookkeepingFlags & MB_PREPLACED))
    {
      // (If if it's preplaced, it won't have set the HAS_MONSTER flag in the
      // first place, so clearing it might screw up an existing monster.)
    ...
    getQualifyingPathLocNear(x, y, *x, *y, true, T_DIVIDES_LEVEL & avoidedFlagsForMonster(&(monst->info)), 0,
                             ...
                             (HAS_MONSTER | HAS_PLAYER | HAS_UP_STAIRS | HAS_DOWN_STAIRS), true);
  }
  pmap[*x][*y].flags |= HAS_MONSTER;
  monst->bookkeepingFlags &=
      ~(MB_PREPLACED | MB_APPROACHING_DOWNSTAIRS | MB_APPROACHING_UPSTAIRS | MB_APPROACHING_PIT | MB_ABSORBING);
  ...
  if (monst->bookkeepingFlags & MB_FOLLOWER)
  {
    ...
    for (leader = monsters->nextCreature; leader != nullptr; leader = leader->nextCreature)
    {
...
// Returns true iff the location is a plain wall, three of the four cardinal
// neighbors are walls, the remaining cardinal neighbor is not a pathing
// blocker, the two diagonals between the three cardinal walls are also walls,
// and none of the eight neighbors are in machines.
bool validStairLoc(int x, int y)
{
  ...
  for (dir = 0; dir < 4; dir++)
  {
    ...
    else
    {
      ...
      if (cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER) || passableArcCount(newX, newY) >= 2)
      {
        return false;
      }
...
// The walls on either side become torches. Any adjacent granite then becomes
// top_wall. All wall neighbors are un-tunnelable. Grid is zeroed out within 5
// spaces in all directions.
void prepareForStairs(int x, int y, char grid[DCOLS][DROWS])
{
  int newX, newY, dir;
...
void initializeLevel()
{
  ...
  else
  {
    getQualifyingLocNear(downLoc, levels[n].downStairsLoc[0], levels[n].downStairsLoc[1], false, 0,
                         (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_ITEMS | T_AUTO_DESCENT | T_IS_DEEP_WATER |
                          T_LAVA_INSTA_DEATH | T_IS_DF_TRAP),
  ...
  {
    // Run a field of view check from up stairs so that monsters do not spawn
    // within sight of it.
    for (dir = 0; dir < 4; dir++)
    {
  ...
  // Restore creatures that fell from the previous depth or that have been
  // pathing toward the stairs.
  ...
  calculateDistances(mapToPit, levels[rogue.depthLevel - 1].playerExitedVia[0],
                     levels[rogue.depthLevel - 1].playerExitedVia[1], T_PATHING_BLOCKER, nullptr, true, true);
  for (monst = monsters->nextCreature; monst != nullptr; monst = monst->nextCreature)
  {
...
// no creatures, items or stairs and with either a matching liquid and dungeon
// type or at least one layer of type terrainType. A dungeon, liquid type of -1
// will match anything.
bool randomMatchingLocation(int* x, int* y, int dungeonType, int liquidType, int terrainType)
{
  ...
  do
  {
    ...
    *y = rand_range(0, DROWS - 1);
                                   ...
                                   (((dungeonType >= 0 && pmap[*x][*y].layers[DUNGEON] != dungeonType) ||
                                     (liquidType >= 0 && pmap[*x][*y].layers[LIQUID] != liquidType)) &&
                                    terrainType < 0) ||
                                   (pmap[*x][*y].flags & (HAS_PLAYER | HAS_MONSTER | HAS_DOWN_STAIRS | HAS_UP_STAIRS |
                                                          HAS_ITEM | IS_IN_MACHINE)) ||
